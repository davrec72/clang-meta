...
class Preprocessor {
  ...
  friend class ParserBrickWallRAII; //DWR ADDN
...
private:
  ...
  enum CurLexerKind {
    CLK_Lexer,
    CLK_MetaSrcLexer, //DWR ADDN
    CLK_PTHLexer,
    CLK_TokenLexer,
    CLK_CachingLexer,
    CLK_LexAfterModuleImport,
    CLK_TerminatorPretendLexer //DWR ADDN for resolving unterminated metaparse exprs.
  } CurLexerKind = CLK_Lexer;

  unsigned CurTerminator = 0; //DWR ADDN; CLK_TerminatorPretendLexer will cycle through a few
...
//DWR ADDN:
  /// \{
  /// Cache of meta-generated source string lexers to reduce malloc traffic.
  /// (Copied from TokenLexerCache structure.)
  /// Very big cache here because we usually need to push a big stack of these
  /// in each constexpr decl call; then in we have nested calls or templated
  /// calls that's even more.
  // TODO perhaps best to have a smaller Lexer implem for this usage.
  enum { MetaSrcStrLexerCacheSize = 64 };
  unsigned NumCachedMetaSrcStrLexers;
  std::unique_ptr<Lexer> MetaSrcStrLexerCache[MetaSrcStrLexerCacheSize];
  /// \}
//END
...
//               , unsigned long MetaSrcStrVecReserve = 1024  //DWR ADDN/DWR FIXME
               );
...
//DWR ADDN:
//  std::vector<std::string *> MetaSrcStrVec;
//  //^ DWR added this back in to try to address the random crashes that occasionally
//  // occur when trying to parse String Literals (DIDN'T WORK)
private:
  bool ParsingFromMetaSrcStrBool = false;
  bool ErrorWhileParsingFromMetaSrcStrBool = false;
  bool NoMoreMetaparseStrsAvailableBool = false;
  SourceLocation cur_metaparse_loc = SourceLocation();

  /// This variable is used to ensure that, while temporarily lexing a template instatition's expand statement-generated
  /// source strings, you know exactly when those strings have been fully lexed so that upon returning to the
  /// instantiation point the lexer stack is in the same state is was before.
  size_t TotalNumLexersAtRAIIConstruction = 0;
public:

  // These defs are trivial, but we put them into the cpp so we can insert debug messages
  // without incurring huge extra compile times due to altering a much-used header:
  size_t getTotalNumLexers() const;
  bool ParsingFromMetaSrcStr() const;
  bool NoMoreMetaparseStrsAvailable() const;
  bool ErrorWhileParsingFromMetaSrcStr() const;
  void setNoMoreMetaparseStrsAvailable(bool val);
  void setErrorWhileParsingFromMetaSrcStr(bool val);
  SourceLocation curMetaparseLoc() const;
  void setCurMetaparseLoc(SourceLocation loc); //used for diagnostics in case of errors
  bool LexingPretendTerminators() const { return CurLexerKind == CLK_TerminatorPretendLexer; }

  /// Builds a new Lexer to process a std::string and calls EnterSourceFileWithLexer,
  /// thus pushing the string onto the lexer stack to be processed.
  void InsertGeneratedSrcStr(StringRef str, SourceLocation loc);

  /// This will be called before storing (via COPYING)
  /// CachedTokens and other cache info when constructing a PreprocessorBrickWallRAII.
  /// If there is anything to be done to clear out unneeded tokens etc. before this copy,
  /// put those tasks in here.  Definition in PPCaching.cpp.
  void CleanUpCache();

#ifndef NMETAPARSEDEBUG
  bool DebugMode = false; //DWR TEMP
