...
class Lexer : public PreprocessorLexer {
  ...
  // in Lexer::isNextPPTokenLParen/LBrace.
  ...
  bool BufferIsMetaSrcStr = false; //DWR ADDN
...
public:
  //DWR ADDN:
  bool ParsingFromMetaSrcStr() const { return BufferIsMetaSrcStr; }
...
private:
  ...
  /// isNextPPTokenLParen
  /// Return 1 if the next unexpanded token will return a
  /// tok::l_paren token, 0 if it is
  /// something else and 2 if there are no more
  /// tokens in the buffer controlled by this lexer.
  unsigned isNextPPTokenLParen();

//DWR ADDN
  /// isNextPPTokenLBrace-
  /// Same as isNextPPTokenLParen, but for LBrace.
  unsigned isNextPPTokenLBrace();
//END
