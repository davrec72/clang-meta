...
//DWR ADDN
  /// isNextPPTokenLBrace- Same as isNextPPTokenLParen, but for LBrace
  unsigned isNextPPTokenLBrace() {
    // isNextPPTokenLBrace is not on the hot path, and all we care about is
    // whether or not we are at a token with kind tok::eof or tok::l_brace.
    // Just read the first byte from the current token pointer to determine
    // its kind.
    tok::TokenKind x = (tok::TokenKind)*CurPtr;
    return x == tok::eof ? 2 : x == tok::l_brace;
  }
//END
