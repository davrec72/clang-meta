...
#include "clang/Sema/ParsedAttr.h" //DWR ADDN
...
//  class SourceCodeInjector; //ASUTTON ADDN
  class Stmt;
  class StringLiteral;
  class SwitchStmt;
  ...
  class TemplateDecl;
  class TemplateDeclInstantiator; //DWR ADDN (of forward declaration, not class)
...
class Sema {
  //DWR ADDN:
  /// ParsingIntoInstantiation: when you are parsing into a template instantiation
  /// (due to constexpr decls with __queue_metaparse statements in the template), this is
  /// set to either PII_class or PII_func, depending on the kind of template.
  /// This is used when we need to instruct Sema to keep track of things needed
  /// by the Parser (e.g. Scope and IdResolver with the proper instantiated Decls,
  /// for proper Lookup when parsing Stmts when PII==PII_func) that ordinarily Sema
  /// would not keep track of, owing to the fact that in the original implementation
  /// the parser is not used at all during template instantiation.
public:
  ...
  ParsingIntoInstantiation getPII() const { return PII; }
private:
  ParsingIntoInstantiation PII = PII_false;

  friend class TempParseIntoDiffScope; //So it can store and restore CurScope (which should perhaps be a Parser data mem...)
  friend class SemaPIIRAII;
  friend class TemplateDeclInstantiator; //DWR TEMP DEBUG
  //END
...
//DWR ADDN (for __queue_metaparse):
private:
...
public:
  Parser &getParser() {
    assert(TheParser
           && "Expected setParser to have been "
              "called when the Parser was constructed ");
    return *TheParser;
  }
...
//END
//  //DWR TEMP HACK:
//  class TemplateDeclInstantiator* CurTemplateDeclInstantiator = nullptr;
//  void setCurTemplateDeclInstantiator(class TemplateDeclInstantiator* x) {
//    CurTemplateDeclInstantiator = x;
//  }
//  //END

//DWR ADDN (for __reflect_prop etc):
private:
  DeclContext *ClientReflDC = nullptr; //will be set when first needed
  ClassTemplateDecl *ClientReflPtrwrpCTD = nullptr; //""
  std::map<unsigned/*ReflectionObjKind*/, ClassTemplateDecl *> reflection_cached_lookups = {};
public:
  DeclContext *GetClientReflDC(SourceLocation Loc); //SemaBuildReflection.cpp
  ClassTemplateDecl *getClientReflPtrwrpCTD(SourceLocation Loc);
  ClassTemplateDecl *getClientReflCTD(SourceLocation Loc, unsigned/*ReflectionObjKind*/ RK); //SemaBuildReflection.cpp
  const QualType refl_chunk_qt = Context.getUIntPtrType();
//END
...
public:
  ...
  Decl *ActOnTag(Scope *S, unsigned TagSpec,
////ASUTTON ADDN:
//                 Expr *Metaclass,
////END
                 TagUseKind TUK,
...
//END
  ...
  ParsingClassState PushParsingClass() {
    //DWR TODO: this is called when a ParsingClassDefinition RAII object is created;
    //since we want to limit our state changes to the Parser, should we restrict this action so that
    //it's only performed when !ParsingFromMetaSrcStr?  Need to figure out what DelayedDiagnostics does exactly...
...
////END
  ...
  QualType CheckTemplateIdType(TemplateName Template,
                               ...
                               , bool CacheInstantiation = true //DWR ADDN
                              );
...
//        , Injection(nullptr) //ASUTTON ADDN
        {}
...
//DWR ADDN:
  void
  AddMembersToInstantiation(CXXRecordDecl *Instantiation,
                                  CXXRecordDecl *Pattern,
                                  TemplateSpecializationKind TSK,
                                  SourceLocation PointOfInstantiation,
...
////END
//ASUTTON ADDN:
  /// Overload for when first three args are still dependent or have not
  /// yet been processed (DWR):
  ExprResult ActOnReflectionTrait(SourceLocation KWLoc,
...
//DWR ADDN:
  ...
  ExprResult ActOnReflectionTrait(SourceLocation KWLoc,
                                  ...
                                  ArrayRef<Expr *> RemArgs, /// AFTER the ObjKind, MemNum, and IsPtr args;
                                                            /// i.e. starting with the fourth arg
                                  SourceLocation RParenLoc
//                                  , bool HasNonIntArgs
                                  );

  /// Dependent type overload:
  ExprResult ActOnReflectionTrait(SourceLocation KWLoc,
                                  ReflectionTraitKind TraitKind,
                                  unsigned ObjKind,
                                  unsigned MemNum,
...
//                                  , bool HasNonIntArgs
  ...
  ExprResult ActOnReflectNew(SourceLocation KWLoc,
                             unsigned/*ReflectionObjKind*/ ObjKind, //DWR ADDN
                             ArrayRef<Expr *> Args,
                             SourceLocation RParenLoc);
  ExprResult ActOnReflectDelete(SourceLocation KWLoc,
                                unsigned/*ReflectionObjKind*/ ObjKind, //DWR ADDN
                                ArrayRef<Expr *> Args,
                                SourceLocation RParenLoc);
//END
...
//ASUTTON ADDN, DWR MOD:
  ...
  ExprResult ActOnCompilerMessageExpr(SourceLocation KWLoc,
                                      Expr *MessageExpr,
                                      SourceLocation RParenLoc);
  ExprResult BuildCompilerMessageExpr(SourceLocation KWLoc,
                                      Expr *MessageExpr,
                                      SourceLocation RParenLoc);
  ExprResult ActOnCompilerDiagnosticExpr(SourceLocation KWLoc,
                                         ArrayRef<Expr *> Args,
                                         SourceLocation RParenLoc);
  ExprResult BuildCompilerDiagnosticExpr(SourceLocation KWLoc,
                                         ArrayRef<Expr *> Args,
                                         SourceLocation RParenLoc);
//END
...
//ASUTTON ADDN:
  Decl *ActOnConstexprDecl(Scope *S, SourceLocation ConstexprLoc,
                           unsigned &ScopeFlags //DWR ADDN?
                           , struct CDContextVars &cdcv //DWR ADDN
...
//  StmtResult ActOnCXXInjectionStmt(SourceLocation Loc, Expr *Reflection); //ASUTTON ADDN
//  StmtResult BuildCXXInjectionStmt(SourceLocation Loc, Expr *Reflection); //ASUTTON ADDN

  StmtResult ActOnCXXQueueMetaparseStmt(SourceLocation KWLoc, Expr *NewSrcCode); //DWR ADDN
  StmtResult BuildCXXQueueMetaparseStmt(SourceLocation KWLoc, Expr *NewSrcCode); //DWR ADDN

  ExprResult ActOnCXXMetaparseExpr(SourceLocation KWLoc, QualType T, Expr *ExprSrcCode, Parser &TheParser); //DWR ADDN
  ExprResult BuildCXXMetaparseExpr(SourceLocation KWLoc, QualType T, Expr *ExprSrcCode, Parser &TheParser); //DWR ADDN
...
//                    SmallVectorImpl<EvalEffect> &Injections, //ASUTTON ORIG
...
////END
//  bool ApplyExpand(SourceLocation POI, ExpandInfo &II); //DWR ADDN
////ASUTTON ADDN:
