...
//DWR ADDN:
  /// InstantiationsWillNeedParsing: this will be true for class templates and function
  /// templates with constexpr decls containing __queue_metaparse statements.  It is consulted in
  /// InstantiateClass and InstantiateFunctionDefinition to determine if they need to
  /// set up Parser RAII objects and Sema variables to allow proper parsing into an
  /// instantiation.
  /// For non-dependent decls this is meaningless.
  unsigned InstantiationsWillNeedParsing : 1;
//END
...
//        Injectable(false), Injected(false), //ASUTTON ADDN
        InstantiationsWillNeedParsing(false), //DWR ADDN
...
////ASUTTON ADDN:
//        Injectable(false), Injected(false),
////END
        Access(AS_none), FromASTFile(0),
        IdentifierNamespace(getIdentifierNamespaceForKind(DK)),
        CacheValidAndLinkage(0) {
...
//DWR ADDN:
//DWR FIXME: see if you can move this to TemplateDecl or something,
//then be sure to add it to DeclNodesRefl in the right place
  bool instantiationsWillNeedParsing() const {
    return InstantiationsWillNeedParsing;
  }
  void setInstantiationsWillNeedParsingTrue() {
    InstantiationsWillNeedParsing = true;
  }
//END
...
////DWR COMMENTED OUT because I don't think these have been defined anywhere, nor used.
