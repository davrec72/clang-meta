...
public:
  CXXExpansionStmt(StmtClass SC, DeclStmt *Range, DeclStmt *LoopVar, Stmt *Body,
                   std::function<std::size_t()> CB_getSize, //DWR MOD (was std::size_t N)
  ...
  std::function<std::size_t()> getCB_getSize() const { return CB_getSize; } //DWR ADDN

  /// Whenver there is an error during the CB_getSize call, INVALID_STD_SIZE_T will be returned.
# define INVALID_STD_SIZE_T  std::numeric_limits<std::size_t>::max()

  /// \brief Returns the number of instantiated statements.
  std::size_t getSize() const {
//DWR MOD
    assert(CB_getSize && "Didn't expect this to be called unless the CB_getSize was valid");
    return CB_getSize();
//END
  }
  ...
  ArrayRef<Stmt *> getInstantiatedStatements() const {
    return llvm::makeArrayRef(InstantiatedStmts, getSize());
  }
  ...
  Stmt **end_instantiated_statements() const {
    return InstantiatedStmts + getSize();
  }
  ...
  Sema &getSemaRef() const {
    assert(SemaPtr && "don't call this unless you've set SemaPtr to nonnull");
    return *SemaPtr;
  }
...
//DWR ADDN
  static bool classof(const Stmt *T) {
    return T->getStmtClass() == CXXPackExpansionStmtClass
        || T->getStmtClass() == CXXTupleExpansionStmtClass;
  }
//END
};
...
public:
  CXXTupleExpansionStmt(TemplateParameterList *TP, DeclStmt *RangeVar,
                        ...
                        std::function<std::size_t()> CB_getSize, //DWR MOD
                        SourceLocation FL, SourceLocation EL, SourceLocation CL,
                        SourceLocation RPL,
                        Sema &SemaRef //DWR ADDN
                        );
...
///
class CXXQueueMetaparseStmt : public Stmt {
  SourceLocation IntroLoc;

  /// The generated source code to be introduced.
  Stmt *NewSrcCode;

public:
  CXXQueueMetaparseStmt(SourceLocation IntroLoc, Expr *Ref)
    : Stmt(CXXQueueMetaparseStmtClass), IntroLoc(IntroLoc), NewSrcCode(Ref) {}

  explicit CXXQueueMetaparseStmt(EmptyShell Empty)
      : Stmt(CXXQueueMetaparseStmtClass, Empty), IntroLoc(), NewSrcCode() {}

  /// \brief The introduced source code as an expression (of a string literal/static char array).
  Expr *getNewSrcCode() const { return reinterpret_cast<Expr *>(NewSrcCode); }
  ...
  SourceLocation getLocEnd() const LLVM_READONLY {
    return NewSrcCode->getLocEnd();
  }

  static bool classof(const Stmt *T) {
    return T->getStmtClass() == CXXQueueMetaparseStmtClass;
  }

  child_range children() {
    return child_range(&NewSrcCode, &NewSrcCode + 1);
  }
