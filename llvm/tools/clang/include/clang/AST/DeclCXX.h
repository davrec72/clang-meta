...
//DWR ADDN:
    /// This is used by Sema::getContainingDC() to detect when it
    /// is parsing into a class template instantiation.
    unsigned IsInstantiation : 1;
//END
...
//DWR ADDN:
  bool isInstantiation() const {
    return data().IsInstantiation;
  }
  void setIsInstantiationTrue() {
    data().IsInstantiation = true;
  }
//END
...
public:
  ...
  /// declared already.
  CXXConstructorDecl *getConstructor() const { return {}; } //FIXME define properly, DWR added stupid def to avoid linker error
...
//DWR ADDN:
  struct CDContextVars *cdcv_p;
  StmtResult sr = StmtEmpty();
  // ^ used for constexpr "decls" within function bodies,
  //  i.e. constexpr blocks that need to be parsed as
  //  both declarations AND statements
  //  (Will be accessed in ActOnDeclStmt)
  Expr *LambdaExpr = nullptr;
  bool IsDependent = false;
//END

//DWR MODS: added attrs param to latter two constructors:
  ConstexprDecl(DeclContext *DC, SourceLocation ConstexprLoc)
      : Decl(Constexpr, DC, ConstexprLoc), Representation(), Call(nullptr)
      , cdcv_p(nullptr) //DWR ADDN
      {
  }

  ConstexprDecl(DeclContext *DC, SourceLocation ConstexprLoc, CDContextVars &cdcv, FunctionDecl *Fn);
  ConstexprDecl(DeclContext *DC, SourceLocation ConstexprLoc, CDContextVars &cdcv, CXXRecordDecl *Class);
public:
  ...
  //DWR FIXME: if storing the LambdaExpr works, then work on removing the Closure stuff
  // and substitute the Expr * everywhere instead

//END DWR MODS
//DWR ADDN:
  ~ConstexprDecl();
  CDContextVars &getcdcv();
  StmtResult getStmtResult() const;
  void setStmtResult(StmtResult newsr);
  Expr *getLambdaExpr() const;
  void setLambdaExpr(Expr *E);
  bool isDependent() const { return IsDependent; }
  void setDependentTrue() { IsDependent = true; }
//END DWR ADDN
...
/////     constexpr void <m`etafunction>(T target, S source);
/////
