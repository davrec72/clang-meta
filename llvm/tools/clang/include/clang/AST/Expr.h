...
////DWR ADDN:
//struct ExpandInfo {
//  /// The type of the expression being expanded.
//  QualType NewSrcCodeType;

//  /// The actual value computed by the expand statement.
////  APValue NewSrcCodeValue; //OLD
//  const StringLiteral *NewSrcCodeValue;
//};
////END
////ASUTTON ADDN + DWR ADDN/MOD:
...
//  /// FIXME maybe -- this should ideally go in the above union, but having memory allocation issues so trying this out.
//  ///---ACTUALLY, I think it's critical you do NOT use pointers here, because the delete statements in teh destructor below
//  ///will be problematic when a vector of EvalEffects needs to be moved to accomodate a larger number: the original batch will
//  ///be deleted before being moved, which will call the delete pointers, such that the new batch will be invalid suddenly.
//  ///So, any data types need to be non-pointers, or smart pointers, or the logic needs to be improved otherwise.
//  ///Best bet: leave as is; I think DiagnosticArg is unused now but if you get errors also make that a non-pointers;
//  /// then when you remove ASUTTON's stuff, the issue with the Injection pointer will go away.


//  ExpandInfo SrcCodeToExpand; //DWR ADDN (not in above union because I got errors doing that)

//  EvalEffect()
////    : DiagnosticArg(nullptr)
//  { }
...
////  ~EvalEffect() {
////    if (Kind == DiagnosticEffect)
////      delete DiagnosticArg;
//////    else if (Kind == InjectionEffect)
//////      delete Injection;
////  }
//};
////END
...
////END
//DWR ADDN/REPLACEMENT FOR ABOVE
    SmallVectorImpl<const StringLiteral *> *MetaCodeChunks;
//END

    EvalStatus()
        : HasSideEffects(false), HasUndefinedBehavior(false), Diag(nullptr)
//        , Effects(nullptr) //ASUTTON ADDN
        , MetaCodeChunks(nullptr) //DWR ADDN/REPLACEMENT
        {}
...
//DWR ADDN
  ...
  bool EvaluateAsString(const StringLiteral *&StrLitResult, const ASTContext &Ctx,
                        SideEffectsKind AllowSideEffects = SE_NoSideEffects) const;
...
public:
  ...
  static bool classof(const Stmt *T); //cpp
...
//  class ReflectionTraitExpr;

//} //namespace clang: temporarily exiting...

//namespace llvm {

//// Special cast<...>(...) overloads for ReflectionTraitExpr->StringLiteral.
//// Defined in Expr.cpp.

//// The main specialization; evaluates the reflection trait
//// and returns the StringLiteral result:
//template<>
//clang::StringLiteral *cast<clang::StringLiteral>(clang::ReflectionTraitExpr *E);

//// Version for base clang::Stmt type (Expr * should overload here too).
//template<>
//clang::StringLiteral *cast<clang::StringLiteral>(clang::Stmt *T);

//// Trivial const versions:
//template<>
//const clang::StringLiteral *cast<clang::StringLiteral>(const clang::ReflectionTraitExpr *E);
//template<>
//const clang::StringLiteral *cast<clang::StringLiteral>(const clang::Stmt *T);

//} //namespace llvm

///// Can't do diags during ReflectionTrait->StringLiteral conversion; best we can
///// do is return a StringLiteral whose string has this prefix (possibly followed by
///// additional info).  HACK.
//#define BAD_REFLTRAIT_STRLIT_PFX "!!ERROR_Reflection_trait_eval_failed"

//namespace clang { //...and we're back in clang
...
StringLiteral * MakeString(ASTContext& Ctx, StringRef Str, SourceLocation Loc); //Expr.cpp
//END
