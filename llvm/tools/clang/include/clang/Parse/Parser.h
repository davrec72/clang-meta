...
//DWR ADDN:
  ...
  friend class TempParseIntoClassInstantiation; //so it can use ParsingClassDefinition
  //So that InstantiateClass can call e.g. ParseLexedMethodDeclarations etc. for processing __queue_metaparse statements:
  //DWR FIXME: would be better to isolate all the called functions into public Parser functions)
  friend bool Sema::InstantiateClass(SourceLocation PointOfInstantiation,
                                     CXXRecordDecl *Instantiation, CXXRecordDecl *Pattern,
                                     const MultiLevelTemplateArgumentList &TemplateArgs,
                                     TemplateSpecializationKind TSK,
                                     bool Complain);
  //For non-templated constexpr decls the following needs access:
  friend void Sema::ActOnFinishConstexprDecl(Scope *S, Decl *D, Stmt *Body);
  //So DoQueuedMetaparsing can make looped calls to e.g. ParseCXXClassMemberDeclarationWithPragmas:
  friend bool Sema::DoQueuedMetaparsing(SourceLocation POI,
...
//END
  ...
  bool ParsingFromMetaSrcStrBool = false; //DWR ADDN
...
public:
  ...
  //DWR ADDN:
#ifndef NDEBUG
  static unsigned DEBUGnumParsersConstructed;
#endif
...
//DWR ADDN:
  /// The vector of class instantiations, each of a class template containing
  /// __queue_metaparse statements, that have yet to have their initializers, inline defs,
  /// attributes, etc. be late-parsed.
  ...
  ///   struct A {
  ///     constexpr {
  ///       __queue_metaparse "int i = 0;";
  ///     }
  ///   };
  ///   template<typename T>
  ///   struct B {
  ///     constexpr {
  ///       __queue_metaparse "A<int> a = A<int>();";
  ///     }
  ///   };
  ///   int main() {
  ///     B<int> b;
  ...
  /// \endcode
  ///
  /// When we try to instantiate B<int>, the data member "A<int> a" will be added without issue, but
  /// if we immediately try to late-parse the initializer "= A<int>()", we will have a problem...
  ///
  /// DWR FIXME FIGURE THIS OUT, maybe this isn't necessary...
  ///
  std::vector<ParsingClass *> ExpandInstClassVec;
//END
...
////END
  ...
  ParsingClass& StealParsingClass(Sema::ParsingClassState); //ASUTTON ADDN (Used by DWR too)
...
//END
  ...
  //DWR NOTE: moved ParsedAttributesWithRange def to ParsedAttr.h
...
//ASUTTON ADDN:
  ...
  ExprResult ParseReflectionTrait(ReflectionTraitKind rtk);
  ExprResult ParseReflectNew(); //DWR ADDN
  ExprResult ParseReflectDelete(); //DWR ADDN
...
//
////END
//  StmtResult ParseCXXInjectionStatement(); //ASUTTON ADDN
  StmtResult ParseCXXQueueMetaparseStmt(); //DWR ADDN
  ExprResult ParseCXXMetaparseExpr(); //DWR ADDN
////ASUTTON ADDN:
...
//ASUTTON ADDN, DWR MOD:
  ///Nested: if this is being parsed while processing an outer constexpr decl
  DeclGroupPtrTy ParseConstexprDeclaration(struct CDContextVars &cdcv, bool Nested);
//END
//ASUTTON ADDN & DWR ADDN/MOD:
  ExprResult ParseCompilerDiagnosticExpression();
  ExprResult ParseCompilerMessageExpression();
//END
////ASUTTON ADDN:
