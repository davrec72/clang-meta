...
void Preprocessor::CleanUpCache() {
  //DWR TEST: if you get errors, erase the following code;
  // it's just to try to save memory/copy operations while
  // constructing PreprocessorBrickWallRAII.
  if (!isBacktrackEnabled() && CachedLexPos == CachedTokens.size()) {
    //NB: so far this ALWAYS seems to be the case when
    // entering a constexpr decl or instantiation.
    METAPARSE_DEBUG("In CleanUpCache, clearing out cache entirely...")
    CachedTokens.clear();
    CachedLexPos = 0;
    assert(BacktrackPositions.empty());
    CachedTokenRangeToErase = None;
  }
  else_METAPARSE_DEBUG("In CleanUpCache; conditions not met, so NOT clearing cache")
}
//END