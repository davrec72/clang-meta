...
void TokenLexer::Init(Token &Tok, SourceLocation ELEnd, MacroInfo *MI,
  ...
  if (NumTokens > 0) {
    ...
    assert((Tokens[0].getLocation().isFileID()
           || Tokens[0].is(tok::comment)
//           || PP.ParsingFromMetaSrcStr() //DWR ADDN TEMP TEST
...
/// isNextTokenLParenOrLBrace (DWR MOD: added LBrace) -
...
/// expansion stack, return 2.  If the next unexpanded token is a '(', return
/// 1, otherwise return 0.
unsigned TokenLexer::isNextTokenLBrace() const {
  // Out of tokens?
  if (isAtEnd())
    return 2;
  return Tokens[CurTokenIdx].is(tok::l_brace);
}
//END
...
//         || PP.ParsingFromMetaSrcStr() //DWR ADDN
         );

  SourceManager &SM = PP.getSourceManager();
  assert(SM.isInSLocAddrSpace(loc, MacroDefStart, MacroDefLength)
         || PP.ParsingFromMetaSrcStr() //DWR TEST ADDN
