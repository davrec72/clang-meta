...
bool Preprocessor::ReadMacroParameterList(MacroInfo *MI, Token &Tok
  ...
  const auto endtokkind = (OpenWasLBrace ? tok::r_brace : tok::r_paren); //DWR ADDN
  assert((MI->paramsInBraces() == OpenWasLBrace)); //DWR ADDN

  while (true) {
    LexUnexpandedToken(Tok);
//DWR moved following out of switch:
    if (Tok.getKind() == endtokkind) {
...
//END
    ...
    case tok::ellipsis:  // #define X(... -> C99 varargs
      ...
      LexUnexpandedToken(Tok);
      if (Tok.isNot(endtokkind)) { //DWR MOD
        Diag(Tok,
             endtokkind == tok::r_paren ? //DWR ADDN
               diag::err_pp_missing_rparen_in_macro_def
                 : diag::err_pp_missing_rbrace_in_macro_def //DWR ADDN
                   );
        return true;
      }
...
// DWR ADDN: I've gotten crashes from trying to use function-like macros defined
...
//END
  ...
  if (Tok.is(tok::eod)) {
    ...
    Tok.clearFlag(Token::LeadingSpace);
             ...
             || Tok.is(tok::l_brace) //DWR ADDN: lets macro definitions can also use braces instead of parens.
             ) {

    // This is a function-like macro definition.  Read the argument list.
    MI->setIsFunctionLike();
//DWR ADDN (this has nothing to do with metaparsing like the above)
    if (Tok.is(tok::l_brace)) {
      MI->setParamsInBraces();
      assert(MI->paramsInBraces());
    } else
      assert(!MI->paramsInBraces());
//END
    if (ReadMacroParameterList(MI, LastTok
                               , Tok.is(tok::l_brace) //DWR ADDN
                               )) {
