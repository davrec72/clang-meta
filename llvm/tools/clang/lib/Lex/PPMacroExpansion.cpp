...
//DWR ADDN (TODO extract common func)
/// isNextPPTokenLParen - Determine whether the next preprocessor token to be
/// lexed is a '('.  If so, consume the token and return true, if not, this
/// method should have no observable side-effect on the lexed tokens.
bool Preprocessor::isNextPPTokenLBrace() {
  // Do some quick tests for rejection cases.
  unsigned Val;
  if (CurLexer)
    Val = CurLexer->isNextPPTokenLBrace();
  else if (CurPTHLexer)
    Val = CurPTHLexer->isNextPPTokenLBrace();
  else
    Val = CurTokenLexer->isNextTokenLBrace();
  ...
  if (Val == 2) {
    ...
    for (const IncludeStackInfo &Entry : llvm::reverse(IncludeMacroStack)) {
      if (Entry.TheLexer)
        Val = Entry.TheLexer->isNextPPTokenLBrace();
      else if (Entry.ThePTHLexer)
        Val = Entry.ThePTHLexer->isNextPPTokenLBrace();
      else
        Val = Entry.TheTokenLexer->isNextTokenLBrace();
...
bool Preprocessor::HandleMacroExpandedIdentifier(Token &Identifier,
  ...
  // DWR ADDN: I've gotten crashes from trying to use macros defined
  // within a metaparse statement, hence this:
  if (ParsingFromMetaSrcStr()) {
    Diag(cur_metaparse_loc, diag::err_metaparse_unsupported_macro_use)
        << "(Cannot expand a macro within a metaparse statement.)";
    setErrorWhileParsingFromMetaSrcStr(true);
    return true; //true = error I think
  }
  //END
...
// (DWR NOTE that this already was keeping track of braces for some reason,
// so do NOT mess with this should you decide to abandon the idea of allowing
// braces in the macros, or condition it on a command line input.)
...
MacroArgs *Preprocessor::ReadMacroCallArgumentList(Token &MacroName,
                                                   ...
                                                   , bool ParamsUsedBraces //DWR ADDN
                                                   ) {
  ...
  assert(Tok.is(ParamsUsedBraces ? tok::l_brace : tok::l_paren) //DWR MOD
         && "Error computing l-paren/brace-ness?");
  ...
  const auto begtokkind = ParamsUsedBraces ? tok::l_brace : tok::l_paren; //DWR ADDN
  const auto endtokkind = ParamsUsedBraces ? tok::r_brace : tok::r_paren; //DWR ADDN

  unsigned NumActuals = 0;
  while (Tok.isNot(endtokkind)) { //DWR MOD
    if (ContainsCodeCompletionTok && Tok.isOneOf(tok::eof, tok::eod))
      break;

    assert(Tok.isOneOf(begtokkind, tok::comma) && //DWR MOD
    ...
    unsigned NumParensOrBraces = 0; //DWR MOD: keeps tracks of braces instead of parens
                                    // if that's what the end tok kind is.
    ...
    while (true) {
      ...
      if (Tok.isOneOf(tok::eof, tok::eod)) { // "#if f(<eof>" & "#if f(\n"
        ...
        break;
      } else if (Tok.is(endtokkind)) { //DWR MOD
        // If we found the ) or } token, the macro arg list is done. (DWR MOD)
        if (NumParensOrBraces-- == 0) { //DWR MOD
      ...
      } else if (Tok.is(begtokkind)) { //DWR MOD
        ++NumParensOrBraces; //DWR MOD
      } else if (Tok.is(tok::comma) && NumParensOrBraces == 0 && //DWR MOD
    ...
    // argument.
    if (ArgTokens.empty() && Tok.getKind() == endtokkind) //DWR ADDN
      break;
  ...
  // Okay, we either found the r_paren (DWR or r_brace).
  // Check to see if we parsed too few arguments.
  ...
  if (!isVariadic && NumActuals > MinArgsExpected &&
    ...
    for (SourceRange ParenLocation : ParenHints) {
      DB << FixItHint::CreateInsertion(ParenLocation.getBegin(), ParamsUsedBraces ? "{" : "("); //DWR MOD
      DB << FixItHint::CreateInsertion(ParenLocation.getEnd(), ParamsUsedBraces ? "}" : ")"); //DWR MOD
    }
