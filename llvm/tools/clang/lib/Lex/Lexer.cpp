...
//DWR ADDN:
/// Lexer constructor - Create a new lexer object for lexing generated source
/// string (which don't have an associated file object, nor SourceLocations
/// associated with each lexed character).
...
SourceLocation Lexer::getSourceLocation(const char *Loc,
  ...
  if (FileLoc.isFileID()) {
    //DWR FIXME: would be great if we could point to a location within a string literal
    // with a SourceLocation, but not implemented yet, so when we get an error instead a
    // __queue_metaparse statement, it will point to the beginning of that statement.
//DWR ADDN
    if (BufferIsMetaSrcStr)
      return FileLoc;
//END
...
bool Lexer::LexEndOfFile(Token &Result, const char *CurPtr) {
  ...
  // a pedwarn.
  //DWR ADDN: only do this check if !BufferIsMetaSrcStr (since if BufferIsMetaSrcStr, it's not a file)
  if (!BufferIsMetaSrcStr && CurPtr != BufferStart && (CurPtr[-1] != '\n' && CurPtr[-1] != '\r')) {
...
/// isNextPPTokenLParen
/// Return 1 if the next unexpanded token lexed from
/// the specified lexer will return a tok::l_paren token
...
//DWR ADDN
// TODO: extract out common function
/// Same as LParen version, but for LBrace
unsigned Lexer::isNextPPTokenLBrace() {
  assert(!LexingRawMode && "How can we expand a macro from a skipping buffer?");
...
//DWR ADDN:
    if (Result.getKind() != tok::identifier
        && Result.getKind() != tok::raw_identifier) {
      Kind = tok::dollar;
      break;
    }
//END
