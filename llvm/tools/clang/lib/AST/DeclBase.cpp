...
#define DECL_CONTEXT_BASE(NAME)                  \
      if (DK >= first##NAME && DK <= last##NAME) \
        return static_cast<NAME##Decl *>(const_cast<DeclContext *>(D));
#include "clang/AST/DeclNodes.inc"
      llvm::outs() << "[DWR ERROR in castFromDeclContext!] DeclKind name: " << D->getDeclKindName() << "\n"; //DWR TEMP
      reinterpret_cast<Decl *>(const_cast<DeclContext *>(D))->dump(); //DWR TEMP
...
#ifndef NDEBUG
  if (Access == AS_none) {
    llvm::outs() << "Access sanity assert about to fail, dumping Decl with access=AS_none:\n";
    dump();
  }
#endif
//END
...
//  assert(D->getDeclContext() == this && "Decl is not a member of this context");
//  assert(containsDecl(D) && "Decl is not contained within this context");
//
