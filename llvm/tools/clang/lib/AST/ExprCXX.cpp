...
//DWR ADDN:
CXXMetaparseExpr::CXXMetaparseExpr(SourceLocation KWLoc, QualType T, Expr *ExprSrcCode, Parser &TheParser)
  : Expr(CXXMetaparseExprClass, T, VK_RValue, OK_Ordinary,
          /*isTypeDependent:*/T->isDependentType(),
          ExprSrcCode->isValueDependent(),
          ExprSrcCode->isInstantiationDependent(),
          ExprSrcCode->containsUnexpandedParameterPack()
         )
  , KWLoc(KWLoc)
  , ExprSrcCode(ExprSrcCode)
  , TheParser(&TheParser)
{}
//END
...
//END DWR MOD
//DWR ADDN
static bool AnyContainsUnexpandedParameterPackExprs(ArrayRef<Expr *> Args) {
  for (unsigned i = 0; i < Args.size(); ++i)
    if (Args[i]->containsUnexpandedParameterPack())
      return true;
  return false;
...
bool clang::AnyDependentExprs(ArrayRef<Expr *> Args) {
  for (unsigned i = 0; i < Args.size(); ++i) {
    if (Args[i]->isTypeDependent() || Args[i]->isValueDependent())
      return true;
#ifndef NDEBUG
    else {
      assert(!Args[i]->isInstantiationDependent() && "DWR didn't expect, add it in");
      assert(!Args[i]->containsUnexpandedParameterPack() && "DWR didn't expect, add it in");
    }
#endif
  }
  return false;
...
ReflectionTraitExpr::ReflectionTraitExpr(ASTContext &C,
                                         SourceLocation KWLoc,
                                         ReflectionTraitKind  TraitKind,
                                         ArrayRef<Expr *> InitArgs,
                                         SourceLocation RParenLoc
//                                         , bool HasNonIntArgs
                                         )
  : Expr(ReflectionTraitExprClass, C.DependentTy, VK_RValue, OK_Ordinary,
////ASUTTON ORIG:
...
////END
//DWR REPLACEMENT:
         /*isTypeDependent:*/true, //for this ctor, type will always be dependent
         /*isValueDependent:*/true,
         /*isInstantiationDependent*/false, //I think this one is rare
         AnyContainsUnexpandedParameterPackExprs(InitArgs)
//END
         )
  , TraitKind(TraitKind)
  ...
  , RParenLoc(RParenLoc)
  , Ctx(&C)
  , ObjKind(0), MemNum(0), IsPtr(0)
//  , HasNonIntArgs(HasNonIntArgs)
  , CB_getTypeFromArgs(nullptr)
  , CB_getResultFromArgs(nullptr)
{
...
//                                         , bool HasNonIntArgs
  ...
  : Expr(ReflectionTraitExprClass, NonDepTy, VK_RValue, OK_Ordinary,
//DWR MOD:
          /*isTypeDependent:*/false, //for this ctor, type will always be non-dependent
          /*isValueDependent:*/AnyValueDependentExprs(RemArgs),
          /*isInstantiationDependent*/false, //not 100% sure bout this one
          AnyContainsUnexpandedParameterPackExprs(RemArgs)
//END
         )
  , TraitKind(TraitKind)
  , NumArgs(RemArgs.size())
  , KWLoc(KWLoc)
  , RParenLoc(RParenLoc)
  , Ctx(&C)
  , ObjKind(ObjKind), MemNum(MemNum), IsPtr(IsPtr)
//  , HasNonIntArgs(HasNonIntArgs)
  , CB_getTypeFromArgs(nullptr)
  , CB_getResultFromArgs(CB_getResultFromArgs)
{
  Args = new (C) Expr *[NumArgs];
  for (unsigned i = 0; i != NumArgs; ++i) {
    Args[i] = RemArgs[i];
  ...
  assert(!NonDepTy->isDependentType()
         && "Use the other constructor for dependent types -- i.e. provide a "
            "callback to generate the proper type from the args, instead of the type");
  assert(!CB_getTypeFromArgs); // sanity check
}

/// Ctor for a dependent type (provide a callback instead of the type):
ReflectionTraitExpr::ReflectionTraitExpr(ASTContext &C,
                                         SourceLocation KWLoc,
                                         ReflectionTraitKind  TraitKind,
                                         unsigned ObjKind, unsigned MemNum, bool IsPtr,
...
//                                         , bool HasNonIntArgs
    ...
    : Expr(ReflectionTraitExprClass, C.DependentTy, VK_RValue, OK_Ordinary,
//DWR MOD:
          /*isTypeDependent:*/true, //always type dependent for this ctor
          /*isValueDependent:*/true, //always value dependent for this ctor
          /*isInstantiationDependent*/false, //not 100% sure bout this one
          AnyContainsUnexpandedParameterPackExprs(RemArgs)
//END
           )
    ...
    , ObjKind(ObjKind), MemNum(MemNum), IsPtr(IsPtr)
//    , HasNonIntArgs(HasNonIntArgs)
    , CB_getTypeFromArgs(CB_getTypeFromArgs)
    , CB_getResultFromArgs(CB_getResultFromArgs)
{
  Args = new (C) Expr *[NumArgs];
  for (unsigned i = 0; i != NumArgs; ++i) {
  ...
  assert(CB_getTypeFromArgs &&
         "Must provide a nonnull callback here (so we can properly set"
         "the type of this expression once we have sufficient args evaluated)");
}

bool ReflectionTraitExpr::callbacksAreSet() const {
#ifndef NDEBUG
  //Just making sure if one of these is set, all of them are:
  if (getCB_getResultFromArgs()) {
    assert(!getType()->isDependentType() || getCB_getTypeFromArgs());
  } else {
    assert(getType()->isDependentType());
  }
#endif
  return getCB_getResultFromArgs();
}

Stmt::child_range ReflectionTraitExpr::children() {
...
/*
 ...
 --Implement these functions by porting of the LoadIntArgExprs etc. stuff, removing the Sema/EvalInfo params
 so that all diagnostics must be handled outisde of these.
 --Set up the ExprConstant.cpp stuff to make use of the EvaluateResult.
 --Fix the Sema ActOn stuff so that it makes use of EvaluateType in the Dependent typed one.
 --In the non-dependent typed one, add in some asserts about isa and cast to StringLiteral where appropriate.
    ---If these fail -- and they might -- then lose the conversion operator, and instead define a specialization of cast/dyn_cast for pointer
    types, that allocates the new object.  Do that in the header.

 Then, see how it does.  Once you are sure casting/isa is working, that's about the best you can do -- I don't see any places where you'd need to
 take advantage of slicing/overloading into a StringLiteral (not many VisitStringLiteral implems in meaningful places),
  so anything that's dependent on that must operate through isa/cast.
 */



/// @returns whether we are ready to evaluate and the type is a string literal type.
bool ReflectionTraitExpr::readyToConvertToStringLiteral() const {
  assert(isValueDependent() || callbacksAreSet()
         && "Expected that if all the args were non-value dependent, "
            "the callbacks would have been set by now");
  return !isValueDependent() && isStrLitType(getType());
}
...
bool clang::isConstCharPtrType(QualType QT) {
  if (QT->isPointerType()) {
    auto PointeeType = QT->getPointeeType();
    return (PointeeType->isCharType()
        && PointeeType.isConstQualified());
  }
  //Not sure if needed:
  if (!QT.isCanonical())
    return isConstCharPtrType(QT.getCanonicalType());
  return false;
}
/// Determines whether QT is a const char * or const char[n]
bool clang::isStrLitType(QualType QT) {
  if (QT->isPointerType()) {
    auto PointeeType = QT->getPointeeType();
    return (PointeeType->isCharType()
        && PointeeType.isConstQualified());
  ...
  if (QT->isArrayType()) {
    auto ElemType = cast<ArrayType>(QT.getTypePtr())->getElementType();
    return (ElemType->isCharType()
        && ElemType.isConstQualified());
  }
  //Not sure if needed:
  if (!QT.isCanonical())
    return isStrLitType(QT.getCanonicalType());
  return false;
}
...
//DWR ADDN:
ReflectNewExpr::ReflectNewExpr(const ASTContext &C,
                               unsigned/*ReflectionObjKind*/ ObjKind,
                               ArrayRef<Expr *> InitArgs,
                               SourceLocation KWLoc,
                               SourceLocation RParenLoc)
      // (DWR NOTE mostly copied from the ReflectionTraitExpr ctor above...)
    : Expr(ReflectNewExprClass, C.DependentTy, VK_RValue, OK_Ordinary,
           //DWR FIXME: set up these args like you did above, think it through.
           // A reflection trait is type dependent when its operand is
           // value dependent.
           AnyValueDependentExprs(InitArgs),
           // FIXME: When is this expression value dependent? When the node
...
ReflectDeleteExpr::ReflectDeleteExpr(const ASTContext &C,
                                     unsigned/*ReflectionObjKind*/ ObjKind,
                                     ArrayRef<Expr *> InitArgs,
                                     SourceLocation KWLoc,
                                     SourceLocation RParenLoc)
      // (DWR NOTE mostly copied from above...)
    : Expr(ReflectDeleteExprClass, C.VoidTy, VK_RValue, OK_Ordinary,
           // A reflection trait is type dependent when its operand is
           // value dependent.
           AnyValueDependentExprs(InitArgs),
           // FIXME: When is this expression value dependent? When the node
...
CompilerMessageExpr(QualType VoidType,
    ...
    : Expr(CompilerMessageExprClass, VoidType, VK_RValue, OK_Ordinary, false,
...
CompilerMessageExpr *CompilerMessageExpr::
Create(const ASTContext &C, Expr *Message, SourceLocation KWLoc, SourceLocation RParenLoc) {
  return new (C) CompilerMessageExpr(C.VoidTy, Message, KWLoc, RParenLoc);
}

CompilerMessageExpr *CompilerMessageExpr::CreateEmpty(const ASTContext &C, EmptyShell Empty) {
  return new (C) CompilerMessageExpr(Empty);
}
//END

//DWR ADDN (modeled on CXXUnresolvedConstructExpr, plus used ASUTTON's Any_DependentExprs static functions)
CompilerDiagnosticExpr::CompilerDiagnosticExpr(QualType VoidType,
                                               SourceLocation KWLoc,
                                               ArrayRef<Expr*> Args,
                                               SourceLocation RParenLoc)
    : Expr(CompilerDiagnosticExprClass, VoidType, VK_RValue, OK_Ordinary,
           AnyTypeDependentExprs(Args),
           AnyValueDependentExprs(Args),
           AnyInstantiationDependentExprs(Args),
           AnyContainsUnexpandedParameterPackExprs(Args)), //is this right?
      KWLoc(KWLoc), RParenLoc(RParenLoc),
      NumArgs(Args.size()) {
  auto **StoredArgs = getTrailingObjects<Expr *>();
  for (unsigned I = 0; I != Args.size(); ++I) {
...
////DWR same as above, but with SmallVectorImpl input for Args (TODO combine these)
//CompilerDiagnosticExpr::CompilerDiagnosticExpr(QualType VoidType,
//                                               SourceLocation KWLoc,
...
CompilerDiagnosticExpr::Create(const ASTContext &C,
                               SourceLocation KWLoc,
                               ArrayRef<Expr*> Args,
                               SourceLocation RParenLoc) {
  void *Mem = C.Allocate(totalSizeToAlloc<Expr *>(Args.size()));
  return new (Mem) CompilerDiagnosticExpr(C.VoidTy, KWLoc, Args, RParenLoc);
}

CompilerDiagnosticExpr *
CompilerDiagnosticExpr::CreateEmpty(const ASTContext &C, unsigned NumArgs) {
  Stmt::EmptyShell Empty;
  void *Mem = C.Allocate(totalSizeToAlloc<Expr *>(NumArgs));
  return new (Mem) CompilerDiagnosticExpr(Empty, NumArgs);
}
//END
////DWR ADDN:
//CompilerDiagnosticExpr *CompilerDiagnosticExpr::
...
//  return new (C) CompilerDiagnosticExpr(C, warn1err0, KWLoc, RParenLoc, UserInputtedLoc, Message, std::move(FixIts));
//}

//CompilerDiagnosticExpr *CompilerDiagnosticExpr::
//CreateEmpty(const ASTContext &C, EmptyShell Empty) {
//  return new (C) CompilerDiagnosticExpr(Empty);
//}
////END
//ASUTTON ADDN:
...
CXXConcatenateExpr::CXXConcatenateExpr(ASTContext &Ctx,
        ...
        : Expr(CXXConcatenateExprClass, T, VK_RValue, OK_Ordinary,
               ...
               /*isValueDependent:*/AnyValueDependentExprs(Parts),
               /*isInstantiationDependent:*/AnyInstantiationDependentExprs(Parts),
               /*DWR MOD (was false):*/AnyContainsUnexpandedParameterPackExprs(Parts)
               ),
