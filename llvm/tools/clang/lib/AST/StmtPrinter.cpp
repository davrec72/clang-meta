...
////END
//DWR ADDN:
void StmtPrinter::VisitCXXQueueMetaparseStmt(CXXQueueMetaparseStmt *Node) {
  // FIXME: Actually print something meaningful.
  Indent() << " __queue_metaparse(";
  PrintExpr(Node->getNewSrcCode());
  OS << ");";
  if (Policy.IncludeNewlines) OS << "\n";
}
//END
////ASUTTON ADDN:
//void StmtPrinter::VisitCXXExtensionStmt(CXXExtensionStmt *Node) {
//  // FIXME: Actually print something meaningful.
//  Indent() << "-> { ... }";
//  if (Policy.IncludeNewlines) OS << "\n";
//}
...
////DWR ADDN:
//#define REFLECTION_TRAIT_3(Spelling, Kind) \
//  case clang::TRT_##Kind: return #Spelling;
////END
//#include "clang/Basic/TokenKinds.def"
//  }
...
//END
//DWR ADDN
static std::string to_string(ReflectionTraitKind TraitKind) {
  switch (TraitKind) {
...
void StmtPrinter::VisitReflectionTraitExpr(ReflectionTraitExpr *E) {
  OS <<
//     getReflectionTraitName(E->getTrait())  //ASUTTON
       to_string(E->getTraitKind()) //DWR REPLACEMENT
     << '(';
//DWR ADDN
  if (E->callbacksAreSet())
    OS << E->getObjKind() << ", " << E->getMemNum() << ", " << E->isPtr() << ", ";
//END
...
//END
//DWR ADDN:
void StmtPrinter::VisitReflectNewExpr(ReflectNewExpr *E) {
  OS << "__reflect_new(";
  for (unsigned i = 0; i < E->getNumArgs(); ++i) {
    if (i)
      OS << ", ";
...
void StmtPrinter::VisitReflectDeleteExpr(ReflectDeleteExpr *E) {
  OS << "__reflect_delete(";
  for (unsigned i = 0; i < E->getNumArgs(); ++i) {
    if (i)
      OS << ", ";
...
//DWR ADDN:
void StmtPrinter::VisitCXXMetaparseExpr(CXXMetaparseExpr *E) {
  // FIXME: Actually print something meaningful.
  OS << "__metaparse_expr(";
  PrintExpr(E->getExprSrcCode());
  OS << ");";
}
//END
//END
////DWR OLD
//void StmtPrinter::VisitVoidReflectionExpr(VoidReflectionExpr *E) {
//  llvm_unreachable("Didn't think you could ever reach here; not implemented");
//}
//void StmtPrinter::VisitNonvoidReflectionExpr(NonvoidReflectionExpr *E) {
//  llvm_unreachable("Didn't think you could ever reach here; unimplemented");
//}
////END
//DWR ADDN:
void StmtPrinter::VisitCompilerDiagnosticExpr(CompilerDiagnosticExpr *E) {
  OS << "__compiler_diag(";
...
//END
//ASUTTON ADDN:
void StmtPrinter::VisitCompilerMessageExpr(CompilerMessageExpr *Node) {
  OS << "__compiler_debug(";
