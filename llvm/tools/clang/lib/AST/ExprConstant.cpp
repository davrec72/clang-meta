...
//#include "clang/AST/PrettyDeclStackTrace.h" //DWR ADDN
...
//#include "clang/Parse/CDContextVars.h" //DWR ADDN, stupid thing
#include "clang/Parse/Parser.h" //DWR ADDN (for metaparsing)
#include "clang/Parse/RAIIObjectsForParser.h" //DWR ADDN ""
#include "clang/Sema/SemaDiagnostic.h" //DWR ADDN (for expr_not_ice)
//#include "clang/Sema/Template.h" // DWR ADDN for instantiating statements in VisitTupleExpansionStmt
//#include "clang/Sema/Sema.h" //DWR ADDN need more stuff from Sema.h
...
////DWR TEST ADDN (not sure if this might help or not)
//  if (isa<ReflectionTraitExpr>(E) && E->getType()->isLiteralType(Info.Ctx)) {
////    llvm::errs() << "DWR TEMP DEBUG in CheckLiteralType, returning true because is RTE; also dumping...\n";
////    E->dump();
//    return true;
//  }
////END
...
struct StmtResultB { //DWR MOD: changed this from StmtResult, just in this file, b/c I started getting puzzling ambiguity issues
...
static EvalStmtResult EvaluateStmt(StmtResultB &Result, EvalInfo &Info,
                                   const Stmt *S,
                                   const SwitchCase *SC = nullptr);

/// Evaluate the body of a loop, and translate the result as appropriate.
static EvalStmtResult EvaluateLoopBody(StmtResultB &Result, EvalInfo &Info,
                                       const Stmt *Body,
...
//DWR ADDN:
/// Differs from Expr::EvaluateAsString in that you can supply
/// an EvalInfo& param, which will handle substitutions.
/// ALWAYS CALL THIS ONE when you have an Info available.
/// I probably should not have added the Expr:: version.
static bool EvaluateAsString(const StringLiteral *&StrLitResult, EvalInfo &Info, const Expr *E) {
  assert(!E->isValueDependent());
  assert(!E->isTypeDependent());

#ifndef NDEBUG
  if (!isStrLitType(E->getType())) {
    E->getType().dump();
    E->getType().getCanonicalType().dump();
      llvm_unreachable("Expression is not a string literal type -- see type and canonical type dumps above");
  }
#endif
//  assert(isStrLitType(E->getType()));
  ...
  assert(Res.isLValue() && "DWR Expected an LValue here, but you can try "
                           "commenting this out to see what happens");

  // 3. Fetch the StringLiteral from the evaluation result:
  ...
  if (Base.is<const Expr *>()) {
    ...
    StrLitResult = cast<StringLiteral>(BaseExpr);
    assert(StrLitResult);
    return true;
  }
  else {
    const ValueDecl *D = Base.get<const ValueDecl *>();
    assert(D && "DWR: Expected the LValueBase to either be an Expr or a ValueDecl");
    D->dump();
    llvm_unreachable("DWR: You probably should have done defaultFunctionArrayLValueConversion "
                     "during the ActOn... for the Expression responsible for this. "
                     "The Evaluation result was just a ValueDecl, e.g. "
                     "a ParmVarDecl, rather than the VALUE OF that Decl.  Not sure what "
                     "to do with it to get the value.  See dump above.");
  }
}
//END
////DWR ADDN:
///// Same idea as the EvaluateAsString above: this version constrasts with Expr::EvaluateAsInt
///// in that you can supply an Info param.  Not 100% we need this, maybe ints are simpler
///// and don't need the Info param, but supplied it just in case that extra Info is needed
///// for some evaluation, as it is often with strings.
//static bool EvaluateAsInt(llvm::APSInt Result, EvalInfo &Info, const Expr *E) {
//  assert(E->getType()->isIntegralOrEnumerationType() &&
//         "Not an int type -- should have checked this before now");
...
//  assert(Res.isInt() && "DWR hmmm, expected an integer-typed expression to return an int here,"
//                        "probably an LValue thing, need to get the LValueBase or whatever here...");
//  Result = Res.getInt();
//  return true;
//}
////END
...
static EvalStmtResult EvaluateSwitch(StmtResultB &Result, EvalInfo &Info,
                                     const SwitchStmt *SS) {
  BlockScopeRAII Scope(Info);
...
////DWR MOD
//    Sema &SemaRef = TES->getSemaRef();
//    auto Owner = SemaRef.CurContext;

//    ASTContext &Context = Info.Ctx;

////    //DWR FIXME this RAII stuff should really be handled in ActOnConstexprDecl etc., stupid
////    // that you have to do it separately here.
////    std::unique_ptr<TempParseIntoDiffScope> parseScopeEtcRAII;
////    if (Owner->isRecord()) {
////      parseScopeEtcRAII = llvm::make_unique<TempParseIntoClassInstantiation>(SemaRef, false/*IsInterface; DWR FIXME I guess set this properly, never used though...*/);
////    } else if (Owner->isFunctionOrMethod()) {
////      parseScopeEtcRAII = llvm::make_unique<TempParseIntoFuncInstantiation>(SemaRef);
////    }



//    auto CCND = dyn_cast<NamedDecl>(Owner);
//    llvm::errs() << "Owner kind/name, BEFORE ActOn... etc: " << Owner->getDeclKindName() << "/" << (CCND ? CCND->getQualifiedNameAsString() : "[unnamed]") << "\n\n";

//    unsigned ScopeFlags;

////    ConstexprDecl *CD = cast<ConstexprDecl>(
////          SemaRef.ActOnConstexprDecl(SemaRef.getCurScope(), BodySourceLoc, ScopeFlags, cdcv));
////    SemaRef.ActOnStartConstexprDecl(SemaRef.getCurScope(), CD);

////    assert(CD->getDeclContext() == Owner);

//    // Create a new compound statement that binds the loop variable with the
//    // parsed body. This is what we're going to instantiate.
//    Stmt *B = const_cast<Stmt *>(TES->getBody());
//    Stmt *VarAndBody[] = { TES->getLoopVarStmt(), B };

//    Stmt *Body = CompoundStmt::Create(Context, VarAndBody, SourceLocation(), SourceLocation());


////DWR ADDN (needed to do the Sema stuff here during Expr eval...)

////    llvm::errs() << "DWR TEMP DEBUG Evaluating for... expansion, SemaRef.CurContext dump:\n";
////    Decl::castFromDeclContext(SemaRef.CurContext)->dump(); //DWR TEMP

////    Sema::ContextRAII Switch(SemaRef, TES->getEnclCD()); //DWR try this out next

////    SemaRef.PushFunctionScope();
////    Sema::FunctionScopeRAII FunctionScopeCleanup(SemaRef);
////      //^ just calls PopFunctionScope or whatever at the end of this block

////    EnterExpressionEvaluationContext EvalContext(
////                    SemaRef, Sema::ExpressionEvaluationContext::ConstantEvaluated);

////    SemaRef.PushExpressionEvaluationContext(Sema::ExpressionEvaluationContext::ConstantEvaluated);
////END

////    unsigned initMaybeODRUseExprsSize = SemaRef.MaybeODRUseExprs.size(); //DWR TEMP DEBUG

////DWR ADDN
//    std::size_t TESsize = TES->getSize(); //NB this calls TES's CB_getSize() callback
//    if (TESsize == INVALID_STD_SIZE_T) //i.e. if there was an error during the callback
//      return ESR_Failed; //DWR is this right?
////END

//    SourceLocation BodySourceLoc = TES->getBody()->getBeginLoc();
//    assert(BodySourceLoc.isValid() && "fix it");



////    //DWR TEST:
////    EnterExpressionEvaluationContext(SemaRef, Sema::ExpressionEvaluationContext::Unevaluated);

////DWR MOVED/ADAPTED FROM Sema::FinishCXXTupleExpansionStmt.
//// Explanation: While I would love to leave this stuff in the Sema implems,
//// we really need to do it here, to implement evaluation of for loop expansions
//// for non-const reflection containers, since otherwise all such containers will
//// always be interpreted as having size = 0, even after pushing elements onto them.
//// I.e. we are hacking around the const-ness imposed by the Sema/ExprConstant
//// structure by inter-mixing their functionality a bit.

//    // Instantiate the loop body for each element of the tuple.
//    llvm::SmallVector<Stmt *, 8> Stmts;
//    for (std::size_t I = 0; I < TES->getSize(); ++I) {
//      llvm::errs() << "top of loop\n";


//      IntegerLiteral *E = IntegerLiteral::Create(
//          Context, llvm::APSInt::getUnsigned(I), Context.getSizeType(), TES->getColonLoc());
...
//      llvm::errs() << "about to call CEDecl stuff within loop body\n";




//      //DWR TODO: this is so stupid that I have to set this up here...FIXME
//      CDContextVars *cdcv_p;
//      ParsedAttributesWithRange attrs(SemaRef.getParser().getAttrFactory());
//      ParsingDeclSpec DS(SemaRef.getParser());

//      if (Owner->isRecord()) {

//        cdcv_p = new (Context) CDContextVars(attrs, AS_public); //DWR TEMP, DWR FIXME!!!! need to get the right access here!!!!!
//  //      SemaRef.getParser().EnterScope(Scope::ClassScope|Scope::DeclScope);
//      } else if (Owner->isFunctionOrMethod()) {
//        cdcv_p = new (Context) CDContextVars();
//  //      SemaRef.getParser().EnterScope(Scope::FnScope|Scope::DeclScope |
//  //                                     Scope::CompoundStmtScope);
//      } else {
//        cdcv_p = new (Context) CDContextVars(attrs, &DS);
//  //      SemaRef.getParser().EnterScope(Scope::DeclScope);
//      }

//      assert(cdcv_p);


//    llvm::errs() << "\n[DWR TEMP DEBUG] SemaRef.CurContext name, from within loop body: ";
//    Decl::castFromDeclContext(SemaRef.CurContext)->dump();
//    if (auto ND = dyn_cast<NamedDecl>(SemaRef.CurContext))
...
//      ConstexprDecl *CD = cast<ConstexprDecl>(
//            SemaRef.ActOnConstexprDecl(SemaRef.getCurScope(), BodySourceLoc, ScopeFlags, *cdcv_p));
//      PrettyDeclStackTraceEntry CrashInfo(Context, CD, BodySourceLoc,
//                                          "evaluating for... tuple expansion");
//      SemaRef.ActOnStartConstexprDecl(SemaRef.getCurScope(), CD);
//      assert(CD->getDeclContext() == Owner);

//      CCND = dyn_cast<NamedDecl>(SemaRef.CurContext);
//      llvm::errs() << "CurContext kind/name, in loop body, after ActOnConstexprDecl and ActOnStartConstexprDecl: " << SemaRef.CurContext->getDeclKindName() << "/" << (CCND ? CCND->getNameAsString() : "[unnamed]") << "\n\n";
//      assert(!SemaRef.CurContext->isRecord());


////DWR TEMP TEST commented out...
...
//      // scope.
//      LocalInstantiationScope Locals(SemaRef, true);
...
//      //        // do stuff
//      //
//      // The loop is instantiated just after parsing (DWR FIXME no longer true...), and the loop variable
...
//      Locals.InstantiatedLocal(TES->getRangeVariable(), const_cast<VarDecl *>(TES->getRangeVariable())
//                               , /*AllowReinstantiation=*/true //DWR ADDN (DWR FIXME try removing)
//                               );

//      Sema::InstantiatingTemplate Inst(SemaRef, B->getLocStart(),
//                                       const_cast<Stmt *>(S), ArrayRef<TemplateArgument>(Args),
//                                       B->getSourceRange());

//      StmtResult Instantiation = SemaRef.SubstForTupleBody(Body, MultiArgs);
//      if (Instantiation.isInvalid())
//        return ESR_Failed; //DWR replacement for StmtError().

//////DWR ADDN: moved this up here from below
////      ESR = EvaluateLoopBody(Result, Info, Instantiation.get());
////      if (ESR != ESR_Continue)
////        return ESR;
//////END
...
//      SemaRef.ActOnFinishConstexprDecl(SemaRef.getCurScope(), CD, Instantiation.get());

////      //Just the minimum of what we need from ActOnFinishConstexprDecl; we don't need to
////      //set up extra Parser RAIIs etc.:
////      assert (!SemaRef.CurContext->isDependentContext());
////      if (CD->hasFunctionRepresentation()) {
////        FunctionDecl *Fn = CD->getFunctionDecl();
////        SemaRef.ActOnFinishFunctionBody(Fn, Instantiation.get());
////        SemaRef.EvaluateConstexprDecl(CD, Fn);
////      } else {
////        ExprResult Lambda = SemaRef.ActOnLambdaExpr(CD->getLocation(), Body, SemaRef.getCurScope());
////        SemaRef.EvaluateConstexprDecl(CD, Lambda.get());
////      }

//      llvm::errs() << "...done calling CEDecl stuff within loop body";

//      Stmts.push_back(Instantiation.get());
//    }

////DWR ADDN:
//    //Combine all the Stmts into a single Stmt.
//    Stmt *AllBodyInsts = CompoundStmt::Create(Context, Stmts, SourceLocation(), SourceLocation());

////DWR TEMP DEBUG
////    llvm::errs() << "\nDump of AllBodyInsts: \n";
////    AllBodyInsts->dump();

////END DEBUG


////    SemaRef.ActOnFinishConstexprDecl(SemaRef.getCurScope(), CD, AllBodyInsts);
//    llvm::errs() << "After ActOnFinishConstexprDecl in for... tuple eval\n";


////    //Just the minimum of what we need from ActOnFinishConstexprDecl:
////    assert (!SemaRef.CurContext->isDependentContext());

////    if (CD->hasFunctionRepresentation()) {
////      FunctionDecl *Fn = CD->getFunctionDecl();
////      SemaRef.ActOnFinishFunctionBody(Fn, Body);
////          llvm::errs() << "\nDump of Fn: \n";
////          AllBodyInsts->dump();
////      SemaRef.EvaluateConstexprDecl(CD, Fn);
////    } else {
...
////    // current context.
////    if (!SemaRef.getCurScope()) //DWR is this ever null in this context?
////      SemaRef.CurContext = CD->getDeclContext();
...
////    //DWR TODO/FIXME: can almost certainly take this stuff out (and all the get/set InstantiatedStmts stuff), it's not doing anything I'm almost certain...
////    Stmt **Results = new (Context) Stmt *[Stmts.size()];
////    std::copy(Stmts.begin(), Stmts.end(), Results);
////    const_cast<CXXTupleExpansionStmt *>(TES)->setInstantiatedStatements(Results);
////    //END

//////DWR TEMP DEBUG
////    unsigned endMaybeODRUseExprsSize = SemaRef.MaybeODRUseExprs.size();
////    llvm::errs() << "DWR TEMP DEBUG init/end MaybeODRUseExprs.size(): "
////                 << initMaybeODRUseExprsSize << "/" << endMaybeODRUseExprsSize << "\n";
////    for (Expr *E: TES->getSemaRef().MaybeODRUseExprs) {
////      E->dump();
////    }
//////END
...
//    parseScopeEtcRAII.reset(); //deletes the RAII.

//    SemaRef.getParser().ExitScope();

////END DWR MOD


//ORIGINAL ExprConstant stuff here:
    for (std::size_t I = 0; I < TES->getSize(); ++I) {
...
//DWR ADDN:
  case Stmt::CXXQueueMetaparseStmtClass: {
    if (Info.checkingPotentialConstantExpression())
      return ESR_Succeeded;

    //if (!Info.EvalStatus.Effects) { //ASUTTON ORIG
    if (!Info.EvalStatus.MetaCodeChunks) {
      // Only metapgrams can produce injection results.
      Info.CCEDiag(S->getLocStart(), diag::note_Metaparse_outside_constexpr_decl);
      return ESR_Failed;
    }

    // Compute the source string to parse, and store it in Result.
    const CXXQueueMetaparseStmt *IS = cast<CXXQueueMetaparseStmt>(S);
    Expr *NewSrcCode = IS->getNewSrcCode();

////DWR OLD:
//    APValue Result;
//    if (!Evaluate(Result, Info, NewSrcCode))
//      return ESR_Failed;
////END
    const StringLiteral *StrLitRes;

    if (!EvaluateAsString(StrLitRes, Info, NewSrcCode))
      return ESR_Failed;

    assert(StrLitRes && "DWR: expected this to be nonnull after EvaluateAsString");

    METAPARSE_DEBUG_VAR(StrLitRes->getString())
//    assert(*StrLitRes->getString().end() == '\0'
//           && "DWR: expected the result of EvaluateAsString to be a null terminated string!"); //DWR: with STRINGIZE sometimes its not

    // Queue the source string in Effects for later re-parsing
    Info.EvalStatus.MetaCodeChunks->push_back(StrLitRes);

////ASUTTON/DWR OLD
//    Info.EvalStatus.Effects->emplace_back(EvalEffect(
////            EvalEffect::ExpandEffect,
//            ExpandInfo{NewSrcCode->getType(), StrLitRes} ));
////END
...
//    }
//
//    const CXXExtensionStmt *ES = cast<CXXExtensionStmt>(S);
//
//    // Compute the value of the injected reflection and its modifications.
//    //
...
//    Expr *Target = ES->getInjectee();
//    APValue R0;
//    if (!Evaluate(R0, Info, Target))
//      return ESR_Failed;
//
//    // Compute the value of the injected reflection and its modifications.
//    Expr *Reflection = ES->getReflection();
//    APValue R1;
...
static bool HandleFunctionCall(SourceLocation CallLoc,
  ...
  StmtResultB Ret = {Result, ResultSlot};
...
static bool HandleConstructorCall(const Expr *E, const LValue &This,
  ...
  // wasteful.
  APValue RetVal;
  StmtResultB Ret = {RetVal, nullptr};
...
//DWR NOTE moved MakeString to Expr.h/Expr.cpp



////DWR TEMP DEBUG ONLY,
//enum ReflectionObjKind {
//# include "../Sema/reflection_incs/ReflectionObjKindList.inc"
//  MAX_REFLOBJKIND
//};
//static std::string to_string(ReflectionObjKind RK) {
//  switch (RK) {

//#   include "../Sema/reflection_incs/ReflKindToStringCases.inc"
...
static ExprResult EvaluateAndMetaparseStrLit(EvalInfo &Info, Expr *NewSrcCode, Parser *TheParser, QualType QT) {

  assert(!Info.checkingForOverflow() && !Info.checkingPotentialConstantExpression()
         && "Didn't expect to be doing this if you're just checking stuff");

  const StringLiteral *StrLitResult;

  //NB we use the Info-taking version here, so all substitutions should occur
  // in calculating the string lit.
  if (!EvaluateAsString(StrLitResult, Info, NewSrcCode)) {
    Info.FFDiag(NewSrcCode->getExprLoc(), diag::err_metaparse_expr_eval_failed);
    return ExprError();
  }
  assert(StrLitResult);

  auto &PP = TheParser->getPreprocessor();

  StringRef exprstr = StrLitResult->getString();

  // Special case; an empty string for a void-typed expression is not an error:
  if (exprstr.empty() && QT->isVoidType()) {
    return ExprResult();
  }



  // DWR Have to tack on a semi for helping our diagnostics,
  // so need to allocate a new string.  Ugh.
  std::string *allocstr = new (Info.Ctx) std::string(exprstr);
  *allocstr += ';'; //Tack on a dummy tok::semi
  auto dummytokkind = tok::semi;
  exprstr = *allocstr;

//  // DWR HACK : still some crashes, so now we're trying
//  // allocating on a vector in the Preprocessor, not giving Info.Ctx any power over it.
//  // We did something like this originally and it worked, changed it because it seemed
//  // inefficient -- but better than crashes. (DWR didn't work!)
//  PP.MetaSrcStrVec.push_back(new std::string(exprstr));
//  assert(*PP.MetaSrcStrVec.back()->end() == '\0' && "Expected std::string to be null terminated");
//  exprstr = *PP.MetaSrcStrVec.back();

  //DWR NOTE: none of the above hacks worked, going back to just using the StringLiteral's data.
  // The bug is in the Lexing/Preprocessing code somewhere.

  assert(*exprstr.end() == '\0' &&
         "Expected every string literal to be null-terminated");

//  llvm::errs() << "DWR TEMP DEBUG meta-parsing this expr: |" << exprstr << "|\n";


  { //scope for METAPARSE_DEBUG
    METAPARSE_DEBUG_HDR("Metaparsing single expression in place (via __metaparse_expr)");
    ...
    Sema &SemaRef = TheParser->getActions();

//    Token OrigToken = TheParser->getCurToken();
    ExprResult res;
    { //scope (for RAII)
      ParserBrickWallRAII SavedParserState(*TheParser);

      // DWR TODO: you really should reenter the proper scope here,
      // so lookup works.
      // As it happens, most of our usages of this don't require
      // nested name lookup, so this works,
      // but need to fix this eventually.

      PP.setCurMetaparseLoc(StrLitResult->getBeginLoc());
      ...
      EnterExpressionEvaluationContext EvalContextRAII(SemaRef, Sema::ExpressionEvaluationContext::ConstantEvaluated);

      // Now for the actual parsing:
      // If we were to only call TheParser->ParseExpression(),
      // EVERY result would be valid -- any mistakes would be
      // TypoExprs.  That prevents us from getting proper diagnostics.
      // So we instead call CorrectDelayedTyposInExpr on the result right away.
      res = SemaRef.CorrectDelayedTyposInExpr(TheParser->ParseExpression());

      METAPARSE_DEBUG_VAR(res.isInvalid())

      if (res.isInvalid()) {
        Info.FFDiag(NewSrcCode->getExprLoc(), diag::note_during_metaparse_expr_of_str) << exprstr.drop_back(); //leave off the dummy tok
        return ExprError();
      }

      // Determine if there are remaining unparsed tokens, and if so return an error with a diagnostic:
      if (!TheParser->getCurToken().is(dummytokkind) ) { //the dummy character we tacked on

        //    llvm::errs() << "DWR TEMP DEBUG calcd firstLeftoverToken getKind/is eof/is unknown: " << firstLeftoverToken.getKind()
        //                 << "/" << (firstLeftoverToken.getKind() == tok::eof) << "/" << (firstLeftoverToken.getKind() == tok::unknown)
        //                 << "\n";
  //      StringRef spelling = PP.getSpelling(TheParser->getCurToken());
        Info.FFDiag(NewSrcCode->getExprLoc(), diag::err_metaparse_extraneous_chars);
        Info.FFDiag(NewSrcCode->getExprLoc(), diag::note_during_metaparse_expr_of_str) << exprstr.drop_back(); //leave off the dummy tok
        return ExprError();
      }
      TheParser->ConsumeAnyToken(); //consume the dummy tok

    } //RAIIs destroyed here

    // Remember, we haven't evaluated the expression yet, so even for
    // void metaparsed expression, res should be set at this point if it wasn't invalid:
    // DWR TODO test whether this applies to e.g. __metaparse_expr(void, "")
    // --if not, make the below an if statement.
    assert(res.get());
    res = SemaRef.DefaultFunctionArrayLvalueConversion(res.get());
    assert(!res.isInvalid());
    return res;

    // DWR TODO figure out how to set up a cast, something like the below, so if the types are different
    // but convertible it gets done automatically:
//    if (QT->isIntegerType() && res.get()->getType()->isIntegerType() && QT != res.get()->getType()) {
//      return ImplicitCastExpr::Create(SemaRef.getASTContext(), QT, CK_IntegralCast, res.get(), nullptr, VK_RValue);
//    }
...
/// Turns integer literal and string literal arguments
/// into intptr_ts.  (For the string literal, discards the size info,
/// and passes the const char * interpreted as a intptr_t).
///
/// You need to use this function, instead of EvaluateAsInt
/// or
///
/// Returns a null option if there were errors -- be sure to do your
/// own diagnostic if this returns null.
///
static llvm::Optional<intptr_t> EvaluateReflectionArg(EvalInfo &Info, Expr *E) {
  assert(!E->isValueDependent());
  assert(!E->isTypeDependent());

  APValue Res;
  if (!Evaluate(Res, Info, E)) {
    Info.FFDiag(E->getBeginLoc(), diag::err_refltrait_arg_eval_failed) << "string literal";
    E->dump();
    return {};
  }

  if (Res.isInt())
    return Res.getInt().getZExtValue();
  ...
  if (Base.is<const Expr *>()) {
    ...
    //Less common: string literal type:
    if (isStrLitType(E->getType())) {
      assert(isa<StringLiteral>(BaseExpr) && "Not a string literal");
      const StringLiteral *StrLitResult = cast<StringLiteral>(BaseExpr);
  //      llvm::outs() << "DWR TEMP DEBUG evaluated this StrLitResult during getUIntData: "
  //                   << StrLitResult->getString() << ";\n";
      StringRef str = StrLitResult->getString();
  //      llvm::outs() << "`-------------- size of StrLitResult (will be discarded, "
  //                     "but make sure it is right, should NOT be plus 1): "
  //                   << std::to_string(str.size()) << ";\n";
      assert(str.data());
      assert(str.data()[str.size()] == '\0'
             && "Expected string literal data to be null terminated");

  //#ifndef NDEBUG //DWR TEMP DEBUG
  //      if (strlen(str.data()) != str.size()) {
  //          llvm::outs() << "DWR DEBUG INFO:\n"
  //                          "`--str = " << str << "; str.data() = "
  //                       << str.data() << "; str.size() = " << str.size()
  //                       << "; strlen(str.data()) = " << strlen(str.data()) << "\n";
  //          llvm_unreachable("Expected str.size() == strlen(str.data()); see details above");
  //      }
  //#endif
      assert(strlen(str.data()) == str.size());

      // Note we DISCARD the length info; when we construct a StringRef out of this
      // later we have to do strlen on it: not ideal, DWR FIXME come up with a better
      // answer; somehow return the pointer and the length and treat strings with special
      // care to pass them both along.  (E.g. use StringRef as the LOAD_VAL type, and set
      // up two args here, the pointer then the length -- I think it will interpret correctly.
      // Or something like that.  Very inefficient to discard this length.)
      return reinterpret_cast<intptr_t>(StrLitResult->getString().data());
    }

//    //Not sure if we'll ever need this, given we test the Res for being an int at top:
//    if (E->getType()->isIntegerType()) {
//      assert(isa<IntegerLiteral>(BaseExpr) && "Not an integer literal");
//      return cast<IntegerLiteral>(BaseExpr)->getValue().getZExtValue();
//    }

    E->getType().dump();
    llvm_unreachable("Unhandled type in argument to reflection trait expression; see dump");
  }
  else {
    const ValueDecl *D = Base.get<const ValueDecl *>();
    assert(D && "DWR: Expected the LValueBase to either be an Expr or a ValueDecl");
    D->dump();
    llvm_unreachable("DWR: You should have done defaultFunctionArrayLValueConversion "
                     " during the ActOn... for the Expression responsible for this "
                     "Arg; right now the Evaluation result was just a ValueDecl, e.g. "
                     "a ParmVarDecl, rather than the VALUE of that Decl.  Not sure what "
                     "to do with it to get the value.  See dump above.");
  }
...
//    if (!E->EvaluateAsInt(theint, Info.Ctx)) {
//      Info.FFDiag(E->getBeginLoc(), diag::err_refltrait_arg_eval_failed) << "integer constant expression";
//      return {};
//    }
...
static bool EvaluateReflectionArgs(EvalInfo &Info,
                                   ArrayRef<Expr *> Args,
                                   intptr_t *IntDataArr) {
  for (unsigned i = 0; i < Args.size(); ++i) {
    Expr *E = Args[i];
    if (auto res = EvaluateReflectionArg(Info, E)) {
...
//template<typename RET, typename... OTHERARGs>
//static RET EvaluateFromArgExprs(EvalInfo &Info,
//                                function_impl<RET(ArrayRef<intptr_t>, OTHERARGs...)> CBfunc,
//                                ArrayRef<Expr *> Args,
//                                OTHERARGs... otherargs) {
...
//        if (!EvaluateReflectionArgs(Info, Args, IntDataRawArr)) \
//          return RET();\
//        /*llvm::errs() << "DWR TEMP DEBUG evaluating from arg exprs, TraitKind/ObjKind/MemNum/IsPtr="\
//                     << TraitKind << "/" << ObjKind << "/" << MemNum << "/" << IsPtr << "...";*/\
//        assert(CBfunc);\
//        return CBfunc(ArrayRef<intptr_t>(IntDataRawArr, NumArgs), std::forward<OTHERARGs>(otherargs)...);\
//        /*llvm::errs() << "done\n";*/\
//      }
//    /**/
...
//static QualType EvaluateTypeFromArgExprs(EvalInfo &Info,
//                                         const ReflectionTraitExpr *E) {
//  assert(!E->isValueDependent()
//         && "did not expect this to be value dependent at this point");
//  assert(E->callbacksAreSet()
//         && "CBs should have been set during ActOnReflectionTraitExpr");
//  assert(E->getType()->isDependentType()
//         && "Expected to call this only on a dependent-typed ReflectionTraitExpr");

////  llvm::errs() << "\nDWR TEMP DEBUG, evaluating reflection trait TYPE, "
////                  "TraitKind/ObjKind/MemNum/IsPtr/type/NumArgs =\n"\
////               << E->getTraitKind() << "/" << to_string((ReflectionObjKind)E->getObjKind())
////               << "/" << E->getMemNum() << "/" << E->isPtr() << "/" << E->getNumArgs()
////               << "/" << E->getType().getAsString() << "...\n";

//  return EvaluateFromArgExprs<QualType>(Info,
//                                        E->getCB_getTypeFromArgs(),
//                                        { E->getArgs(), E->getNumArgs() });
//}

///// Returns true if the operation succeeded or wasn't necessary.
//static bool EvalAndSetRTETypeIfNecc(EvalInfo &Info,
//                                             ReflectionTraitExpr *E) {
//  if (E->getType()->isDependentType()) {
//    QualType ClientTempType = EvaluateTypeFromArgExprs(Info, E);
//    if (ClientTempType.isNull())
//      return false;
//    E->setType(ClientTempType);
//  }
//  return true;
//}

///NB this version is only called when we did not call EvaluateTypeFromArgExprs to get QT.
/// Exactly the same as above
static ExprResult EvaluateResultFromArgExprs(EvalInfo &Info,
                                             const ReflectionTraitExpr *E) {
  assert(!E->isValueDependent()
         && "did not expect this to be value dependent at this point");
  assert(E->callbacksAreSet()
         && "CBs should have been set during ActOnReflectionTraitExpr");
  assert(!E->getType()->isDependentType()
         && "Expected to call this only on a NON-dependent-typed ReflectionTraitExpr");

//  llvm::errs() << "\nDWR TEMP DEBUG, evaluating reflection trait RESULT, "
//                  "TraitKind/ObjKind/MemNum/IsPtr/type/NumArgs =\n"\
//               << E->getTraitKind() << "/" << to_string((ReflectionObjKind)E->getObjKind())
//               << "/" << E->getMemNum() << "/" << E->isPtr() << "/" << E->getNumArgs()
//               << "/" << E->getType().getAsString() << "...\n";

//  return EvaluateFromArgExprs<ExprResult>(Info,
//                                          E->getCB_getResultFromArgs(),
//                                          { E->getArgs(), E->getNumArgs() },
//                                          E->getType());

  ArrayRef<Expr *> Args = { E->getArgs(), E->getNumArgs() };
  ...
  #   define GET_FROM_INTARGS_LEQ(N) \
      if (NumArgs <= N) { \
        intptr_t IntDataRawArr[N];/*stack allocate*/\
        if (!EvaluateReflectionArgs(Info, { E->getArgs(), NumArgs }, IntDataRawArr)) \
          return ExprError();\
        /*llvm::errs() << "DWR TEMP DEBUG evaluating from arg exprs, TraitKind/ObjKind/MemNum/IsPtr="\
                     << TraitKind << "/" << ObjKind << "/" << MemNum << "/" << IsPtr << "...";*/\
        assert(E->getCB_getResultFromArgs());\
        return E->getCB_getResultFromArgs()(ArrayRef<intptr_t>(IntDataRawArr, NumArgs), E->getType());\
        /*llvm::errs() << "done\n";*/\
      }
    /**/
...
//static ExprResult EvaluateResultFromArgExprs(EvalInfo &Info, const ReflectionTraitExpr* E) {
//  assert(!E->getType()->isDependentType()
//         && "Expected type to be non-dependent by this point");

//  ExprResult res;

//  if (unsigned NumArgs = E->getNumArgs()) {
//    // NumArgs > 0: this implies this is a primitive-typed reflection
//    // (e.g. a reflected method that returns a bool or an int or a string etc.)
//    // For such cases, we have not yet evaluated the arguments into integers;
//    // we will do that now.

//    ArrayRef<Expr *> RemArgs = { E->getArgs(), NumArgs };

//    unsigned ProblemArgIfAny = 1000;
//    assert(ProblemArgIfAny > NumArgs);

//    res = EvaluateResultFromArgExprs(Info.Ctx, E->getCB_getResultFromArgs(), E->getType(), RemArgs, ProblemArgIfAny);
//    if (ProblemArgIfAny < NumArgs) {
//      assert(res.isUnset()); //it won't be invalid yet because of the way we implemented EvaluateResultFromArgExprs
//      res = ExprError();
//      assert(res.isInvalid());
//      llvm::errs() << "DWR TEMP DEBUG better see refltrait_arg_eval_failed\n";
//      Info.FFDiag(RemArgs[ProblemArgIfAny]->getLocStart(), diag::err_refltrait_arg_eval_failed);
//      return res;
//    }
//  }
//  else {
//    // NumArgs = 0: this implies this is a class-typed reflection,
//    // such that we already determined all we needed to about the reflection
//    // in our GET_TYPE_FROM_INTARGS_LEQ(...)... calls back in the dependent-type
//    // ActOnReflectionTraitTyped overload.
//    // All that information is in E->getType() -- i.e. that type is the client reflection
//    // template instance with the evaluated pointer/object data template args.
//    // Bottom line, no need to futz with the args at all here;
//    // we'll pass ane empty ArrayRef<intptr_t> to the CB_getResultFromArgs
//    // function, and it will be unused, because all that function
//    // will do is default-construct E->getType().
//    res = E->getCB_getResultFromArgs()(ArrayRef<intptr_t>(), E->getType());
//  }

//  if (res.isInvalid()) {
//    // If eval fails above, we use the captured SemaRef reference and call
//    // Diag on that, just because that's the only diagnostic handler we had
//    // available when defining the callbacks.
//    // But Sema won't have a valid location handy at this point, so the
//    // diagnostic will be locaiton-less.
//    // THe following follow-up diagnostic at least points out to the user
//    // where the problems started.  A bit hacky, but the best solution for
//    // now.
//    // (I tried incorporating a general diaghandler to replace all Sema
//    // diagnostics, but we need to call some general-use Sema methods
//    // in some of these calls, e.g. within defaultConstructType, and we don't
//    // want to mess with the diagnostic calls in those, so better to do it
//    // this way.) [DWR]
//    llvm::errs() << "DWR TEMP DEBUG better see refltrait_eval_failed_ars_fine\n";
//    Info.FFDiag(E->getLocStart(), diag::err_refltrait_eval_failed_args_fine);

////    //DWR TODO would be better to just add/swap the diaghandler, rather than
////    // manually copying it, but that's what we'll do for now:
////    auto thediag = Info.FFDiag(E->getLocStart(), diaghandler.getDiagID());
////    for (unsigned I = 0; I < diaghandler.getNumArgs(); ++I)
////      thediag << diaghandler.getStringArg(I);
//  }
//  llvm::errs() << "`--eval succeeded, returning res\n";
//  return res;
//}

#define NRTE_VISIT_DEBUG //uncomment when you don't want RTE EVAL DEBUG messages

#ifndef NRTE_VISIT_DEBUG
# define RTE_VISIT_DEBUG(...) llvm::errs() << "[RTE EVAL DEBUG] " << __VA_ARGS__ << "::VisitReflectionTraitExpr\n";
#else
# define RTE_VISIT_DEBUG(...)
#endif
...
public:
  ...
  //DWR MOD:
    bool VisitReflectionTraitExpr(const ReflectionTraitExpr* E) {
      // If just checking, don't do anything (TODO implement whatever checks needed here)
      if (Info.checkingForOverflow() || Info.checkingPotentialConstantExpression())
        return false;

      RTE_VISIT_DEBUG("ExprEvaluatorBase")
  //  //DWR COMMENTED THIS OUT because commented out the cast specialization
  //  // since it did not work as I'd hoped.
  //#ifndef NDEBUG
  //    // Just making sure these work, if anything else in this file
  //    // wants to call these on E to treat it as a StringLiteral.
  //    if (isStrLitType(E->getType())) {
  //      assert(E->readyToConvertToStringLiteral());
  //      assert(isa<StringLiteral>(E));
  //      assert(cast<StringLiteral>(E));
  //      assert(dyn_cast<StringLiteral>(E));
  //    } else {
  //      assert(!E->readyToConvertToStringLiteral());
  //      assert(!isa<StringLiteral>(E));
  //      assert(!dyn_cast<StringLiteral>(E));
  ...
  //#endif
      ...
      ExprResult res = EvaluateResultFromArgExprs(Info, E);
      if (res.isInvalid())
        return false;
      assert(res.get());

      APValue Result;
      if (!Evaluate(Result, Info, res.get())) {
        return false;
      }
      return DerivedSuccess(Result, E);
  ...
  //END
  //DWR ADDN:
  // Same as above, except using EvaluateAndMetaparseStrLit in
  // place of EvaluateResultFromArgExprs
  bool VisitCXXMetaparseExpr(const CXXMetaparseExpr *E) {
    // If just checking, don't do anything (TODO implement whatever checks needed here)
    if (Info.checkingForOverflow() || Info.checkingPotentialConstantExpression())
      return false;

    ExprResult res = EvaluateAndMetaparseStrLit(Info, E->getExprSrcCode(), &E->getParser(), E->getType());
    if (res.isInvalid())
      return false;
    assert(res.get());
  ...
  //END
  //DWR ADDN:
    /// Visit a __reflect_new expression.
    /// This does nothing because we will handle
    /// everything in ActOnReflectNewExpr.
    bool VisitReflectNewExpr(const ReflectNewExpr* E) {
      return true;
    }
  //END
  ...
  bool VisitStmtExpr(const StmtExpr *E) {
    ...
    for (CompoundStmt::const_body_iterator BI = CS->body_begin(),
      ...
      APValue ReturnValue;
      StmtResultB Result = { ReturnValue, nullptr };
...
//  //DWR TODO: I don't think this one is used, just the VoidExprEvaluator visit implem below,
//  // so perhaps take it out for clarity? I dunno.
//  bool VisitVoidReflectionExpr(const VoidReflectionExpr* E) {
////    llvm::errs() << "[DWR DEBUG] calling VisitVoidReflectionExpr from ExprEvaluator\n";
//    E->doit();
//    return true; /*DerivedSuccess(E->getValue(), E);*/ //is this right?
//  }
...
//DWR ADDN (got from stackoverflow)
  ...
  while (it != s.end())
  {
    ...
    if (c == '\\' && it != s.end())
    {
      switch (*it++) {
        ...
        case '"': c = '\"'; break;
        case '0': c = '\0'; break;
          // TODO all other escapes
        default:
          llvm::errs() << "The escaped char: " << *(it-1) << "\n";
          llvm_unreachable("Invalid escape sequence -- may just need to add above char to unescape switch...");
          continue;
      }
    }
    res += c;
...
//ASUTTON ADDN:
  bool VisitCXXConcatenateExpr(const CXXConcatenateExpr *E) {
    METAPARSE_DEBUG_HDR("In VisitCXXConcatenateExpr")
    const StringLiteral *FullStr;
    const Expr *frontchild = cast<Expr>(*E->child_begin());
    if ( (std::distance(E->child_begin(), E->child_end()) == 1)
        && isStrLitType(frontchild->getType())) {
      //THere is only one argument, and it is a string literal: evaluate it
      // directly into FullStr.
      if (!EvaluateAsString(FullStr, Info, frontchild)) {
        METAPARSE_DEBUG("Thought this was a string type, but evaluation failed!")
        return false;
      }
      assert(FullStr);

      METAPARSE_DEBUG("Single argument of string literal type already, so no concatenation performed, just evaluation")
    } else {
      METAPARSE_DEBUG("Performing true concatenation")
      //At least two operands, or one int, so we need to build a new string:
      SmallString<256> Buf;
      llvm::raw_svector_ostream OS(Buf);

      SmallVector<APValue, 4> Args; //ASUTTON ORIG
        ...
        if (Part.isLValue()) {
          ...
          if (Base.is<const Expr *>()) {
            ...
            if (!isa<StringLiteral>(BaseExpr))
              return Error(P);
            PartStr = cast<StringLiteral>(BaseExpr);
          } else {
          ...
          PartStr->outputString(StrOS);

          //DWR ADDN
          assert (StrBuf.size() >= 2 && StrBuf.front() == '"' && StrBuf.back() == '"'
                  && "Expected part to be surrounded in quotes!");
          if (StrBuf.size() == 2) {
            METAPARSE_DEBUG("Part is just \"\", so not concatenating; onto next one...")
            continue;
          }
          //END
          ...
          // DWR ADDN also, remove all escape characters before sending
          // to OS, otherwise with successive concatenations
          // "\n" becomes "\\n" becomes "\\\\n" etc.
          ActualStr = unescape(ActualStr);

//          llvm::errs() << "[DWR DEBUG] ActualStr: " << ActualStr << "\n";
          OS << ActualStr;
        } else if (Part.isInt()) {
  ...
  //END
      ...
      } //loop over the parts to concatenate

      // Combine into a single string
      FullStr = MakeString(Info.Ctx, Buf, E->getLocStart());
    } //if more than one argument/single argument not a string

//    assert(*FullStr->getString().end() == '\0' && "DWR: expected string to be null-terminated"); //DWR sometimes its not, with stringize macros...

    APValue Val;
    if (!Evaluate(Val, Info, FullStr)) //DWR FIXME: I think THIS is where strings lose their null-termination, somehow...
      return false;
...
//DWR ADDN (probably can comment out)
  bool VisitReflectionTraitExpr(const ReflectionTraitExpr *E) {
    RTE_VISIT_DEBUG("LValueExprEvaluatorBase")
    return ExprEvaluatorBaseTy::VisitReflectionTraitExpr(E);
  }
//END
...
//DWR ADDN (probably can comment out)
  ...
  bool VisitReflectionTraitExpr(const ReflectionTraitExpr *E) {
    RTE_VISIT_DEBUG("LValueExprEvaluator")
    return LValueExprEvaluatorBaseTy::VisitReflectionTraitExpr(E);
  }
//END
  bool VisitCastExpr(const CastExpr *E) {
    switch (E->getCastKind()) {
...
public:
  ...
  bool VisitBinaryOperator(const BinaryOperator *E);
  bool VisitReflectionTraitExpr(const ReflectionTraitExpr *E); //DWR ADDN
  bool VisitCXXMetaparseExpr(const CXXMetaparseExpr *E); //DWR ADDN
  bool VisitCastExpr(const CastExpr* E);
...
//DWR ADDN (needed to get string literals returned from reflection traits
// to be usable in other expressions):
bool PointerExprEvaluator::VisitReflectionTraitExpr(const ReflectionTraitExpr *E) {
  // If just checking, don't do anything (TODO implement whatever checks needed here)
  if (Info.checkingForOverflow() || Info.checkingPotentialConstantExpression())
    return false;

  RTE_VISIT_DEBUG("PointerExprEvaluator")
  assert(isConstCharPtrType(E->getType())
         && "Expected this for string literal-returning reflection traits");
  ...
  APValue TempResult;
  if (!Evaluate(TempResult, Info, res.get())) {
    return false;
  }

  const ConstantArrayType *CAT;

  APValue::LValueBase Base = TempResult.getLValueBase();
  const StringLiteral *StrLitResult;
  if (Base.is<const Expr *>()) {
    const Expr *BaseExpr = Base.get<const Expr *>();
    assert(isa<StringLiteral>(BaseExpr) && "Not a string literal");
    StrLitResult = cast<StringLiteral>(BaseExpr);
    assert(isa<ConstantArrayType>(StrLitResult->getType())
           && "DWR Expected String literal to have a constant array type");
    CAT = cast<ConstantArrayType>(StrLitResult->getType().getTypePtr());

//    llvm::errs() << "DWR TEMP DEBUG Evaluated string during PointerExprEvaluator: " << StrLitResult->getString() << "\n";
  }
  else {
    const ValueDecl *D = Base.get<const ValueDecl *>();
    assert(D && "DWR: Expected the LValueBase to either be an Expr or a ValueDecl");
    D->dump();
    llvm_unreachable("DWR: You should have done defaultFunctionArrayLValueConversion or "
                     "whatever during the ActOn... for the Expression resopnsible for this "
                     "Arg; right now the Evaluation result was just a ValueDecl, e.g. "
                     "a ParmVarDecl, rather than the VALUE of that Decl.  Not sure what "
                     "to do with it to get the value.  See dump above.");
  }
  ...
  // DWR NOTE: in VisitCXXMetaparseExpr these are flipped around;
  // for some reason this ordering works here and the other order
  // works there.
  Result.setFrom(Info.Ctx, TempResult);
  Result.addArray(Info, E, CAT);

  //Any more Result properties to set? So far working fine...
  return true;
}
//Same as above, except EvaluateAndMetaparseStrLit in place of EvaluateResultFromArgExprs:
bool PointerExprEvaluator::VisitCXXMetaparseExpr(const CXXMetaparseExpr *E) {

  // If just checking, don't do anything (TODO implement whatever checks needed here)
  if (Info.checkingForOverflow() || Info.checkingPotentialConstantExpression())
    return false;

//  llvm::errs() << "DWR TEMP DEBUG in PointerExprEvaluator::VisitCXXMetaparseExpr\n";

  assert(isConstCharPtrType(E->getType())
         && "Expected this for string literal-returning metaparse exprs");
  ...
  if (Base.is<const Expr *>()) {
    ...
    assert(isa<StringLiteral>(BaseExpr) && "Not a string literal");
    StrLitResult = cast<StringLiteral>(BaseExpr);
    assert(!StrLitResult->getType().isNull());
    assert(isa<ConstantArrayType>(StrLitResult->getType())
           && "DWR Expected String literal to have a constant array type");
    CAT = cast<ConstantArrayType>(StrLitResult->getType().getTypePtr());
...
//    llvm::errs() << "DWR TEMP DEBUG Evaluated string during PointerExprEvaluator: " << StrLitResult->getString() << "\n";
  ...
  Result.setFrom(Info.Ctx, TempResult);
//  Result.addArray(Info, E, CAT);
//  llvm::errs() << "`--DWR TEMP DEBUG Finished setting up string literal return from metaparse expr\n";
  //Any more Result properties to set? So far working fine...
  return true;
}
...
////DWR HACK:
//  // Usually, when an ImplicitCastExpr encloses a StringLiteral
//  // SubExpr, it will register the cast kind as a CK_ArrayToPointerDecay.
//  // HOWEVER, const char *-typed ReflectionTraitExprs should ALSO be treated
//  // as string literals -- the only reason we don't convert them during
//  // Sema::ActOnReflectionTraitExpr is so that we can do non-const reflection.
//  // But at this point, we need to pretend that they are string literals.
//  // FIXME Would be better to do this at the original ImplicitCastExpr
//  // when we first determine the SubExpr is a const char *-typed
//  // ReflectionTraitExpr, but I can't find where that happens at the moment,
//  // and this works, so pending further errors we'll leave this here.
//  // DWR TODO I think this applies to CHILDREN of a ReflectionTraitExpr as well,
//  // i.e. StringLiterals in the arguments to a ReflectionTraitExpr (rare but
//  // they do exist for reflection containers etc.)
//  if (isa<ReflectionTraitExpr>(SubExpr)) {
//    if (isConstCharPtrType(SubExpr->getType())) {
//      assert(E->getCastKind() == CK_FunctionToPointerDecay && "Hmmm, was this already done?");
//      const_cast<CastExpr *>(E)->setCastKind(CK_ArrayToPointerDecay);
//    } else
//      assert(!isStrLitType(SubExpr->getType()) && "Didn't expect const char[N] type");
//  }
////END
...
//DWR ADDN:
  bool VisitReflectionTraitExpr(const ReflectionTraitExpr *E) {
    RTE_VISIT_DEBUG("MemberPointerExprEvaluator")
    return ExprEvaluatorBaseTy::VisitReflectionTraitExpr(E);
  }
//END
  bool VisitCastExpr(const CastExpr *E);
...
namespace {
  ...
  //DWR ADDN:
    bool VisitReflectionTraitExpr(const ReflectionTraitExpr *E) {
      RTE_VISIT_DEBUG("RecordExprEvaluator")
      return ExprEvaluatorBaseTy::VisitReflectionTraitExpr(E);
    }
  //END
    bool VisitCastExpr(const CastExpr *E);
...
//DWR ADDN:
  bool VisitReflectionTraitExpr(const ReflectionTraitExpr *E) {
    RTE_VISIT_DEBUG("TemporaryExprEvaluator")
    return LValueExprEvaluatorBaseTy::VisitReflectionTraitExpr(E);
  }
//END
  bool VisitCastExpr(const CastExpr *E) {
...
namespace {
  ...
  //DWR ADDN:
    bool VisitReflectionTraitExpr(const ReflectionTraitExpr *E) {
      RTE_VISIT_DEBUG("VectorExprEvaluator")
      return ExprEvaluatorBaseTy::VisitReflectionTraitExpr(E);
    }
  //END
    bool VisitCastExpr(const CastExpr* E);
...
//DWR ADDN
  bool VisitReflectionTraitExpr(const ReflectionTraitExpr* E) {
    ...
    RTE_VISIT_DEBUG("IntExprEvaluator")
    ExprResult res = EvaluateResultFromArgExprs(Info, E);
    if (res.isInvalid())
      return false;
    assert(res.get());
    ...
    llvm::APSInt Result;
    if (!res.get()->EvaluateAsInt(Result, Info.Ctx))
      return false;
    return Success(Result, E);
  }
  //Same as above except with EvaluateAndMetaparseStrLit
  bool VisitCXXMetaparseExpr(const CXXMetaparseExpr* E) {
...
//DWR ADDN:
  bool VisitReflectionTraitExpr(const ReflectionTraitExpr *E) {
    RTE_VISIT_DEBUG("FloatExprEvaluator")
    return ExprEvaluatorBaseTy::VisitReflectionTraitExpr(E);
  }
//END
  bool VisitCastExpr(const CastExpr *E);
...
////DWR ADDN:
//  bool VisitReflectionTraitExpr(const ReflectionTraitExpr *E) {
//    RTE_VISIT_DEBUG("ComplexExprEvaluator")
//    return ExprEvaluatorBaseTy::VisitReflectionTraitExpr(E);
//  }
////END
  bool VisitCastExpr(const CastExpr *E);
...
//DWR ADDN:
  bool VisitReflectionTraitExpr(const ReflectionTraitExpr *E) {
    ...
    RTE_VISIT_DEBUG("VoidExprEvaluator")
    ExprResult res = EvaluateResultFromArgExprs(Info, E);
  ...
  bool VisitCXXMetaparseExpr(const CXXMetaparseExpr *E) {
    ...
    // NB You only get to this one via an empty exprstr; it's a special case:
    if (res.isUnset())
      return true;

    // A void expression was metaparsed, but not evaluated: visit it now to finish the job
    assert(res.get());
    return Visit(res.get());
...
////END
  ...
  bool VisitCompilerMessageExpr(const CompilerMessageExpr *E); //ASUTTON ADDN, DWR MOD
  bool VisitCompilerDiagnosticExpr(const CompilerDiagnosticExpr *E); //DWR ADDN

  /// Visit a __reflect_delete expression.  We will handle everything
  /// in ActOnReflectDeleteExpr, so no need to do anything here.
  bool VisitReflectDeleteExpr(const ReflectDeleteExpr *E) {
    return true;
  }
//END
...
bool VoidExprEvaluator::VisitCompilerMessageExpr(const CompilerMessageExpr *E) {
...
//DWR REPLACEMENT:
  ...
  if (!EvaluateAsString(Message, Info, E->getMessage()))
...
//DWR REPLACEMENT:
  llvm::errs() << unescape(NonQuote); //Don't comment this out, this is a client-defined debug message, not a temporary clang message!!
  return true;
////ASUTTON ORIG (back when this was CompilerErrorExpr)
...
//DWR ADDN:
bool VoidExprEvaluator::VisitCompilerDiagnosticExpr(const CompilerDiagnosticExpr *E) {
  // This never produces a value.
  //
  // FIXME: We probably want to evaluate the sub-expression for potential
  // errors before stopping at these conditions.
  if (Info.checkingPotentialConstantExpression()) {
      //DWR TODO: check first arg is int, second is int, third is stringliteral, remainder, if present, are structs with args necessary to build to FixitHints
    return false;
  }
  if (Info.checkingForOverflow())
    return false;

  unsigned Note2Warn1Err0;
  SourceLocation UserSuppliedLoc;
  std::string Message;
  SmallVector<FixItHint, 2> FixIts;

  // Args[0]: unsigned, 2=Note, 1=Warning, 0=Error (POSSIBLE TODO: allow user to submit different warning levels, factor them into diagnostic system)
  if (E->arg_size() > 0) {
    auto Arg = E->getArg(0);
    APSInt Res;
    if (!Arg->EvaluateAsInt(Res, Info.Ctx))
      return Error(Arg, diag::note_invalid_subexpr_in_const_expr);
    Note2Warn1Err0 = Res.getZExtValue();
    if (Note2Warn1Err0 > 2) {
      Info.FFDiag(Arg, diag::err_expected) << "either 0, 1, or 2";
      return Error(Arg, diag::err_parameter_out_of_bounds); //DWR
    }
  } else
    return Error(E, diag::err_insufficient_args);

  // Args[2]: UserSuppliedLoc
  if (E->arg_size() > 1) {
    auto Arg = E->getArg(1);
    APSInt Res;
    if (!Arg->EvaluateAsInt(Res, Info.Ctx))
      return Error(Arg, diag::note_invalid_subexpr_in_const_expr);
    intptr_t data = Res.getZExtValue();
    if (data) {
      static_assert(sizeof(SourceLocation) <= sizeof(intptr_t),
                    "DWR: I assumed SourceLocation was small, need to adjust "
                    "code to account for its additional chunks");
      UserSuppliedLoc = reinterpret_cast<const SourceLocation&>(data);
      if (UserSuppliedLoc.isInvalid())
        llvm_unreachable("Non-zero SourceLocation reflection data was supplied, but the location is invalid. "
                         "This is perhaps an issue inside Clang, with locations not being properly defined.");
    }
  }
  else
    assert(UserSuppliedLoc.isInvalid()); //sanity check;
...
//DWR REPLACEMENT:
    const StringLiteral *StrLitMsg;
    if (!EvaluateAsString(StrLitMsg, Info, Arg))
      return Error(Arg, diag::note_invalid_subexpr_in_const_expr);
//END
////ASUTTON ORIG:
//    APValue Result;

//    if (!Evaluate(Result, Info, Arg))
//      return Error(Arg, diag::note_invalid_subexpr_in_const_expr);
//    // TODO: This should never be the case.
//    if (!Result.isLValue())
//      return Error(Arg, diag::note_invalid_subexpr_in_const_expr);
...
//      if (!isa<StringLiteral>(BaseExpr))
//        return Error(Arg);
//      StrLitMsg = cast<StringLiteral>(BaseExpr);
//    } else {
//      // FIXME: Actually make this work.
//      // const ValueDecl *D = Base.get<const ValueDecl *>();
//      return Error(Arg);
...
////END
    ...
    SmallString<256> Buf;
    llvm::raw_svector_ostream OS(Buf);
    StrLitMsg->outputString(OS);
    Message = std::string(Buf.str(), 1, Buf.size() - 2); //remove quotes
    Message = unescape(Message); // TODO combine the above quote removal into the unescape function
  }

  // Args[3...n] FixItHints
  if (E->arg_size() > 3) {
    if (Note2Warn1Err0 == 2) {
      Info.FFDiag(E->getArg(3), diag::err_expected) << "user-defined diagnostic notes to be defined without FixItHints";
      return false;
    }

    std::string InsertionOrReplacementText; //allocating outside loop for a little efficiency
    for (unsigned I = 3; I != E->arg_size(); ++I) {
      InsertionOrReplacementText = "";
      auto Arg = E->getArg(I);
      auto Ty = Arg->getType();
      APValue Result;
      if (!Evaluate(Result, Info, Arg))
      ...
      if (Result.isLValue()) {
        ...
        APValue RVal;
        if (!handleLValueToRValueConversion(Info, Arg, Ty, LVal, RVal)) {
          Result.dump();
          return false;
        }
        Result = RVal;
      ...
      if (!Result.isStruct()) {
        Result.dump();
        Info.FFDiag(Arg, diag::err_expected) << "a struct object (containing the FixItHint data as its four fields)";
        return false;
      }
      if (Result.getStructNumFields() != 4) {
        Info.FFDiag(Arg, diag::err_expected) << "this struct to have exactly four fields; instead it has"
                                             << std::to_string(Result.getStructNumFields());
        return false;
      }

      SourceLocation FixItLocA, FixItLocB, FixItLocC;
      unsigned IsTokenRangeVal; //2 = unset
      assert(FixItLocA.isInvalid()); //sanity check

      const RecordDecl *UserFixitHint = Ty->getAs<RecordType>()->getDecl();
      auto CurFieldDeclIter = UserFixitHint->field_begin();

      //Field [0]: LocA (should always be a valid SourceLocation)
      APValue &CurField = Result.getStructField(CurFieldDeclIter->getFieldIndex());
      if (!CurField.isInt()) {
        CurField.dump();
        Info.FFDiag(Arg, diag::err_expected) << "field 0 to be integer (holding reflected SourceLocation data)";
        return false;
      }
      intptr_t field0data = CurField.getInt().getZExtValue();
      if (field0data) {
        FixItLocA = reinterpret_cast<const SourceLocation&>(field0data);
        if (FixItLocA.isInvalid())
          llvm_unreachable("Non-zero SourceLocation reflection data was supplied, but the location is invalid. "
                           "This is perhaps an issue inside Clang, with locations not being properly defined.");
      } else {
        Info.FFDiag(Arg, diag::err_expected) << "field 0 to be non-zero integer, holding valid SourceLocation data "
                                                "(only the second SourceLocation input can be supplied with zero, "
                                                "in the case of Insertion FixItHints)";
        return false;
      }

      //Field [1]: LocB (should be zero for an insertion)
      CurField = Result.getStructField((++CurFieldDeclIter)->getFieldIndex());
      if (!CurField.isInt()) {
        CurField.dump();
        Info.FFDiag(Arg, diag::err_expected) << "field 1 to be integer (holding reflected SourceLocation data)";
        return false;
      }
      intptr_t field1data = CurField.getInt().getZExtValue();
      if (field1data) {
        FixItLocB = reinterpret_cast<const SourceLocation&>(field1data);
        if (FixItLocB.isInvalid())
          llvm_unreachable("Non-zero SourceLocation reflection data was supplied, but the location is invalid. "
                           "This is probably an issue inside Clang, with locations not being properly defined.");
      }

      //Field [2]: either:
      //   --a third location (an insertion point for the range defined by the first two), OR
      //   --a string literal containing the insertion or replacement source code
      CurField = Result.getStructField((++CurFieldDeclIter)->getFieldIndex());
//      std::string data = CurField.getAsString(Info.Ctx, Info.Ctx.getPointerType(Info.Ctx.getConstType(Info.Ctx.CharTy)));

      if (CurField.isInt()) {
        intptr_t field2data = CurField.getInt().getZExtValue();
        if (field2data) {
          FixItLocC = reinterpret_cast<const SourceLocation&>(field2data);
          if (FixItLocC.isInvalid())
            llvm_unreachable("Non-zero SourceLocation reflection data was supplied, but the location is invalid. "
                             "This is probably an issue inside Clang, with locations not being properly defined.");
        }
      } else {
        // Field 2 is not interpretable as a SourceLocation, so it better be a string literal
        // giving us the InsertionOrReplacementText:
        if (CurField.isArray()) {
          if (!CurField.getArrayInitializedElt(0).isInt()) {
            CurField.dump();
            Info.FFDiag(Arg, diag::err_expected) << "this array to be a const char[N]";
            return false;
          }
          for (unsigned I = 0; I != CurField.getArraySize(); ++I)
            InsertionOrReplacementText.push_back(CurField.getArrayInitializedElt(I).getInt().getExtValue());
        } else if (CurField.isLValue()) {
          if (auto strLitVal = dyn_cast<StringLiteral>(CurField.getLValueBase().get<const Expr*>()))
            InsertionOrReplacementText = strLitVal->getString();
          else {
            CurField.dump();
            Info.FFDiag(Arg, diag::err_expected) << "this LValue to be a const char *";
            return false;
          }
        } else {
          CurField.dump();
          Info.FFDiag(Arg, diag::err_expected) << "field 2 to either be a third SourceLocation"
                                                  "(specifying an InsertionFromRange) or a string literal "
                                                  "(a const char * or const char[n], "
                                                  "containing the insertion/replacement text)";
          return false;
        }
        // NOTE: we do NOT remove quotes from or unescape InsertionOrReplacementText
      }


      //Field [3]: IsTokenRangeVal (should be 2 if no CharSourceRange input given)
      CurField = Result.getStructField((++CurFieldDeclIter)->getFieldIndex());
      if (!CurField.isInt()) {
        CurField.dump();
        Info.FFDiag(Arg, diag::err_expected) << "field 3 to be integer (holding IsTokenRange value or 2)";
        return false;
      }
      IsTokenRangeVal = CurField.getInt().getZExtValue();
      if (IsTokenRangeVal > 2) {
        CurField.dump();
        Info.FFDiag(Arg, diag::err_expected) << "field 3 to have value 0, 1, or 2 (IsTokenRange value or 2 if no CharSourceRange given)";
        return false;
      }

      // We have evaluated the inputs; now let's interpret them and add the appropriate FixItHints to our FixIts vector:

      assert(FixItLocA.isValid()); //sanity check
      if (!FixItLocB.isValid()) {
        // Loc A was supplied, B was not
        if (InsertionOrReplacementText.empty()) {
          Info.FFDiag(Arg, diag::warn_expected) << "nonempty insertion text (OR a valid locB if this is not an insertion)";
          continue; //no text to insert, so don't bother creating a fixit
        }
        assert(IsTokenRangeVal == 2 && "expected the IsTokenRange val to be 2, i.e. unset, for an Insertion");
        FixIts.emplace_back(FixItHint::CreateInsertion(FixItLocA, InsertionOrReplacementText));
      }
      else if (FixItLocC.isValid()) {
        // Locs A, B, and C were supplied
        assert (InsertionOrReplacementText.empty() && "expected empty insertion text for an insertion from range");
        assert (IsTokenRangeVal < 2 && "Expected the user to supply a CharSourceRange, i.e. with an IsTokenRange val, "
                                       "for CreateInsertionFromRange");
        FixIts.emplace_back(FixItHint::CreateInsertionFromRange(FixItLocA, CharSourceRange({FixItLocB, FixItLocC}, IsTokenRangeVal) ));
      } else {
        // Locs A and B were supplied, C was not
        if (InsertionOrReplacementText.empty()) {
          // Either text was not supplied, or it was supplied empty; either way a removal
          // (since a removal = a replacement with empty str)
          if (IsTokenRangeVal < 2)  //a CharSourceRange was supplied
            FixIts.emplace_back(FixItHint::CreateRemoval( CharSourceRange({FixItLocA, FixItLocB}, IsTokenRangeVal) ));
          else                      //a SourceRange was supplied
            FixIts.emplace_back(FixItHint::CreateRemoval( SourceRange({FixItLocA, FixItLocB}) ));
        } else {
          // Non-empty text was supplied
          if (IsTokenRangeVal < 2)  //a CharSourceRange was supplied
            FixIts.emplace_back(FixItHint::CreateReplacement( CharSourceRange({FixItLocA, FixItLocB}, IsTokenRangeVal),
                                                              InsertionOrReplacementText ));
          else                      //a SourceRange was supplied
            FixIts.emplace_back(FixItHint::CreateReplacement( SourceRange({FixItLocA, FixItLocB}),
                                                              InsertionOrReplacementText ));
        }
      }
    } //for loop over supplied FixItHints
  } // if any FixItHints were supplied
  else
    assert(FixIts.empty()); //sanity check

  //DWR MOD (added in warning/message stuff in addition to error):

  unsigned userDefdDiagID;
  switch (Note2Warn1Err0) {
    case 0: userDefdDiagID = diag::err_user_defined_error;    break;
    case 1: userDefdDiagID = diag::warn_user_defined_warning; break;
    case 2: userDefdDiagID = diag::note_user_defined_note;    break;
    default:
      llvm_unreachable("out of range, should have already checked this above");
  }

  { //scope so that diag is dispatched before adding note_is_user_defined_diag


    auto D = Info.Ctx.getDiagnostics().Report(
          (UserSuppliedLoc.isValid() ? UserSuppliedLoc : E->getExprLoc()),
          userDefdDiagID )
        << Message;
    for (auto F : FixIts)
      D << F;
  ...
  std::string nwestr;
  switch (Note2Warn1Err0) {
    case 0: nwestr = "error";   break;
    case 1: nwestr = "warning"; break;
    case 2: nwestr = "note";    break;
  default:
    llvm_unreachable("out of range, should have already checked this above");
  }

  Info.Ctx.getDiagnostics().Report(E->getExprLoc(),
                diag::note_is_user_defined_diag) << nwestr;

  return Note2Warn1Err0; //i.e. if its an error you return false, otherwise return true
}
...
////DWR ADDN:
//  // We leave class-returning ReflectionTraitExpr's as dependent types during
//  // Sema::ActOn... etc, and only evaluate them here.  This is because
//  // a) it permits the same degree of non-constness of class-returning
//  // RTE's as enjoyed by primitive-returning RTEs (since the type contains
//  // all the information about the RTE when it is a class -- once the type
//  // is fixed, the value cannot change) AND it enables us to do all the
//  // evaluation -- for a primitive result or a class type -- in this file,
//  // rather than splitting very similar functionality between SemaReflect.cpp
//  // and ExprConstant.cpp.
//  // If you encounter other top-level Evaluate functions where a dependent-typed
//  // RTE may be encountered, copy and paste this if statement there as well.
//  if (auto RTE = dyn_cast<ReflectionTraitExpr>(E)) {
//    EvalAndSetRTETypeIfNecc(Info, const_cast<ReflectionTraitExpr *>(RTE));
//    assert(!RTE->getType().isNull());
//    assert(!RTE->getType()->isDependentType());
//  }
////END
...
//  //DWR ADDN (see Evaluate explanation)
//  if (auto RTE = dyn_cast<ReflectionTraitExpr>(E)) {
//    EvalAndSetRTETypeIfNecc(Info, const_cast<ReflectionTraitExpr *>(RTE));
//    assert(!RTE->getType().isNull());
//    assert(!RTE->getType()->isDependentType());
...
//DWR ADDN
/// ConstexprDecls' CallExprs use this for evaluation.
bool Expr::EvaluateAsVoid(EvalResult &Result, const ASTContext &Ctx) const {
  EvalInfo Info(Ctx, Result, EvalInfo::EM_ConstantFold); //DWR: experiment with different EMs here
...
//DWR ADDN
/// DON'T CALL THIS VERSION when you have an EvalInfo &Info handy -- instead call the
/// static EvaluateAsString(const StringLiteral *&StrLitResult, EvalInfo &Info, const Expr *E) version!!!
/// (DWR FIXME eliminate this probably...)
bool Expr::EvaluateAsString(const StringLiteral *&StrLitResult, const ASTContext &Ctx,
                            SideEffectsKind AllowSideEffects) const {

  EvalResult ExprResult;

  // 1. Ensure it's a const char * or const char[n]:
  if (!isStrLitType(getType())) {
//    llvm::errs() << "DWR TEMP DEBUG, in EvaluateAsString, not a string literal type...\n";
    return false;
  }

  // 2. Evaluate:
  if (isLValue()) {
    if (!EvaluateAsLValue(ExprResult, Ctx)) {
//      llvm::errs() << "DWR TEMP DEBUG, is LValue but EvaluteAsLValue failed...\n";
      return false;
    }
  } else {
    assert(isRValue());
    if (!EvaluateAsRValue(ExprResult, Ctx)) {
//      llvm::errs() << "DWR TEMP DEBUG, is RValue but EvaluteAsRValue failed...\n";
      return false;
    }
  }

  if (hasUnacceptableSideEffect(ExprResult, AllowSideEffects))
    return false;

  // 3. EvaluateAsLValue/EvaluateAsRValue worked.
  //    Fetch the StringLiteral from the evaluation result:
  APValue::LValueBase Base = ExprResult.Val.getLValueBase();
  if (Base.is<const Expr *>()) {
    const Expr *BaseExpr = Base.get<const Expr *>();
    assert(BaseExpr);
    assert(isa<StringLiteral>(BaseExpr) && "Not a string literal");
...
//  case Expr::CXXFragmentExprClass: //ASUTTON ADDN
//ASUTTON ADDN:
  case Expr::CompilerMessageExprClass:
  case Expr::CompilerDiagnosticExprClass: //DWR ADDN
  case Expr::CXXMetaparseExprClass: //DWR ADDN
...
//END
  ...
  case Expr::CXXConstantExprClass: //ASUTTON ADDN:
    return NoDiag();
  case Expr::CallExprClass:
  ...
  case Expr::ReflectionExprClass: //ASUTTON ADDN
  case Expr::ReflectionTraitExprClass: //ASUTTON ADDN
//  case Expr::ReflectionTraitTypedExprClass: //DWR ADDN
  case Expr::ReflectNewExprClass: //DWR ADDN
  case Expr::ReflectDeleteExprClass: //DWR ADDN
//  case Expr::VoidReflectionExprClass: //DWR ADDN
//  case Expr::NonvoidReflectionExprClass: //DWR ADDN
