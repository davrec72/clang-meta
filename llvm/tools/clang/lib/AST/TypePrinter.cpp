...
namespace {
  ...
  class TypePrinter {
    ...
    bool MayAppendScope = true; //DWR ADDN
  ...
  public:
    explicit TypePrinter(const PrintingPolicy &Policy, unsigned Indentation = 0
        ...
        , MayAppendScope(MayAppendScope) //DWR ADDN
    {}
...
//DWR ADDN:
/// This (hopefully) prints the type with the original nested name specifiers needed/given
/// by the user, but not the outer ones -- allows us to refer to new types within new namespaces
/// when printing in an __queue_metaparse statment outside its original scope.
//DWR FIXME: this is a bit hacky, perhaps better to add a PrintingPolicy flag about SuppressingOuterScope; plus needs to be nested to make sure it does indeed work in all instances.
std::string QualType::getAsString_NoPrependScope() const {
  std::string S;
  auto thesplit = split();
  getAsStringInternal(thesplit.Ty, thesplit.Quals, S, LangOptions(), false/*MayAppendScope*/);
  return S;
}
//END
...
void QualType::getAsStringInternal(const Type *ty, Qualifiers qs,
  ...
  llvm::raw_svector_ostream StrOS(Buf);
  TypePrinter(policy
          , 0/*dflt indentation*/, MayAppendScope //DWR ADDN
