...
CXXRecordDecl::DefinitionData::DefinitionData(CXXRecordDecl *D)
    : UserDeclaredConstructor(false), UserDeclaredSpecialMembers(0),
      ...
      IsParsingBaseSpecifiers(false), HasODRHash(false),
      IsInstantiation(false), //DWR ADDN
      Definition(D) {}
...
ConstexprDecl::ConstexprDecl(DeclContext *DC, SourceLocation ConstexprLoc, CDContextVars &cdcv, FunctionDecl *Fn)
  : Decl(Constexpr, DC, ConstexprLoc), Representation(Fn), Call(nullptr)
  , cdcv_p(new CDContextVars(cdcv)) //DWR ADDN
{
  if (cdcv.kind == CDContextVars::CDCV_cls)
    setAccess(cdcv.cls.AS);
}

ConstexprDecl::ConstexprDecl(DeclContext *DC, SourceLocation ConstexprLoc, CDContextVars &cdcv, CXXRecordDecl *Class)
  : Decl(Constexpr, DC, ConstexprLoc), Representation(Class), Call(nullptr)
  , cdcv_p(new CDContextVars(cdcv)) //DWR ADDN
{
  if (cdcv.kind == CDContextVars::CDCV_cls)
    setAccess(cdcv.cls.AS);
...
ConstexprDecl *ConstexprDecl::Create(ASTContext &Cxt, DeclContext *DC,
                                     ...
                                     FunctionDecl *Fn) {
  return new (Cxt, DC) ConstexprDecl(DC, ConstexprLoc, cdcv, Fn);
}
...
ConstexprDecl *ConstexprDecl::Create(ASTContext &Cxt, DeclContext *DC,
                                     ...
                                     CDContextVars &cdcv, //DWR ADDN
                                     CXXRecordDecl *Class) {
  return new (Cxt, DC) ConstexprDecl(DC, ConstexprLoc, cdcv, Class);
}
...
ConstexprDecl::~ConstexprDecl() {
    delete cdcv_p;
}

//DWR ADDN:
CDContextVars &ConstexprDecl::getcdcv() {
  assert(cdcv_p && "cdcv_p is nullptr -- must have constructed this with CreateDeserialized, "
                   "did not expect this call");
  return *cdcv_p;
}
StmtResult ConstexprDecl::getStmtResult() const {
  return sr;
}
void ConstexprDecl::setStmtResult(StmtResult newsr) {
  sr = newsr;
}
Expr *ConstexprDecl::getLambdaExpr() const {
  return LambdaExpr;
}
void ConstexprDecl::setLambdaExpr(Expr *E) {
  LambdaExpr = E;
}
//END
