...
void StringLiteral::setString(const ASTContext &C, StringRef Str,
  ...
  // DWR MOD: changed Length to Length+1 in all the allocations,
  // and added null terminator.  Note we do not change
  // the StringLiteral's Length field though.
  // We null terminate everything because this often needed during
  // metaparsing/reflection, and it doesn't seem all that costly.
  // If anyone objects to this extra cost, at least leave
  // in the extra Length + 1 allocation, and just remove
  // the null-terminating line; then wherever we've placed
  // asserts about null-termination, replace with setting it
  // to be null-terminated. But definitely need the allocation!
  switch(CharByteWidth) {
    case 1: {
      char *AStrData = new (C) char[Length + 1]; //DWR MOD
      std::memcpy(AStrData,Str.data(),Length*sizeof(*AStrData));
      AStrData[Length] = '\0'; //DWR ADDN
      StrData.asChar = AStrData;
      break;
    }
    case 2: {
      uint16_t *AStrData = new (C) uint16_t[Length + 1]; //DWR MOD
      std::memcpy(AStrData,Str.data(),Length*sizeof(*AStrData));
      AStrData[Length] = '\0'; //DWR ADDN
    ...
    case 4: {
      uint32_t *AStrData = new (C) uint32_t[Length + 1]; //DWR MOD
      std::memcpy(AStrData,Str.data(),Length*sizeof(*AStrData));
      AStrData[Length] = '\0'; //DWR ADDN
...
bool StringLiteral::classof(const Stmt *T) {
  return T->getStmtClass() == StringLiteralClass
////DWR ADDN
//      || ( (T->getStmtClass() == ReflectionTraitExprClass &&
//            cast<ReflectionTraitExpr>(T)->readyToConvertToStringLiteral()) )
////END
      ;
}
...
////DWR ADDN:
//// cast<StringLiteral>(ReflectionTraitExpr *) specializations:

///// Assuming we are readyToConvertToStringLiteral(), calls EvaluateResult,
///// gets the result as a string, and returns that.
///// Since we also changed StringLiteral::classof to include ReflectionTraitExprs
///// that are readyToConvertToStringLiteral, we should be able to use isa
///// and cast<StringLiteral>(this) / dyn_cast<StringLiteral>(this), and thereby
///// avoid any bugs/lack of functionality having to do with string-typed
///// reflection traits not being interpretable as string literals.
//template<>
//clang::StringLiteral *llvm::cast<clang::StringLiteral>(clang::ReflectionTraitExpr *E) {
////  llvm::errs() << "DWR TEMP DEBUG in cast<StringLiteral>(ReflectionTraitExpr *E) specialization\n";
//  using StringLiteral = clang::StringLiteral;
//  assert(E->readyToConvertToStringLiteral()
//         && "Did not expect this to be called before ready; "
//            "should only be used if isa<StringLiteral>(this) "
//            "(which checks if readyToConvertToStringLiteral())");
//  assert(E->getASTContext_p() && "expected this to have been set");

//  auto diagstr = new (*E->getASTContext_p()) std::string("DWR_TEST_DUMMY_STRING_CONVERSION_WORKS"); //DWR FIXME!!!!!if it works get it to work, otherwise remove all this
//  return MakeString(*E->getASTContext_p(), *diagstr, E->getBeginLoc());

////  unsigned ProblemArgIfAny = 1000;
////  ExprResult res = EvaluateResultFromArgExprs(*E->getASTContext_p(),
////                                              E->getCB_getResultFromArgs(),
////                                              E->getType(),
////                                              { E->getArgs(), E->getNumArgs() },
////                                              ProblemArgIfAny);
////  if (res.isUnset()) {
////    // We can't output any diags, best we can do is to return an unexpected string literal, SO hacky...
////    if (ProblemArgIfAny < E->getNumArgs()) {
////      auto diagstr = new (*E->getASTContext_p()) std::string(BAD_REFLTRAIT_STRLIT_PFX "_arg_" + std::to_string(ProblemArgIfAny));
////      return MakeString(*E->getASTContext_p(), *diagstr, E->getArgs()[ProblemArgIfAny]->getBeginLoc());
////    } else if (res.isInvalid()) {
////      auto diagstr = new (*E->getASTContext_p()) std::string(BAD_REFLTRAIT_STRLIT_PFX);
////      return MakeString(*E->getASTContext_p(), *diagstr, E->getBeginLoc());
////    }
////  }
////  assert(res.get());
////  assert(isa<StringLiteral>(res.get()));
////  return cast<StringLiteral>(res.get());
//}

//// Version for base clang::Stmt type (Expr * should overload here too)
//template<>
//clang::StringLiteral *llvm::cast<clang::StringLiteral>(clang::Stmt *T) {
//  if (isa<clang::ReflectionTraitExpr>(T))
//    return cast<clang::StringLiteral>(cast<clang::ReflectionTraitExpr>(T)); //call above overload
//  return (clang::StringLiteral *)T; //usual cast
//}

//// Trivial const versions:
//template<>
//const clang::StringLiteral *llvm::cast<clang::StringLiteral>(const clang::ReflectionTraitExpr *E) {
//  return cast<clang::StringLiteral>(const_cast<clang::ReflectionTraitExpr *>(E));
//}
//template<>
//const clang::StringLiteral *llvm::cast<clang::StringLiteral>(const clang::Stmt *T) {
//  return cast<clang::StringLiteral>(const_cast<clang::Stmt *>(T));
//}

////END


//ASUTTON ADDN/DWR MOD/DWR MOVED FROM ExprConstant.cpp
...
StringLiteral *
clang::MakeString(ASTContext& Ctx, StringRef Str, SourceLocation Loc) {
 QualType StrTy = Ctx.getConstantArrayType(Ctx.CharTy.withConst(),
                                           llvm::APInt(32, Str.size() + 1),
                                           ArrayType::Normal, 0);
 auto res = StringLiteral::Create(Ctx, Str, StringLiteral::Ascii, false, StrTy, Loc);
 assert(*res->getString().end() == '\0' && "Expected a null-terminated string"); //DWR ADDN
 return res;
}
//END
...
bool Expr::HasSideEffects(const ASTContext &Ctx,
  ...
  case CXXDependentIdExprClass: //ASUTTON ADDN
    dump();//DWR ADDN
  ...
  case ReflectionExprClass: //ASUTTON ADDN
  case ReflectionTraitExprClass: //ASUTTON ADDN
  //case ReflectionTraitTypedExprClass: //DWR ADDN
  //case NonvoidReflectionExprClass: //DWR ADDN
  case ReflectNewExprClass: //DWR ADDN is this right?
  case CXXConstantExprClass: //ASUTTON ADDN
  case CXXConcatenateExprClass: //DWR ADDN
  case CXXMetaparseExprClass: //DWR ADDN  (is this right?)
//  case CXXFragmentExprClass: //ASUTTON ADDN
    // These never have a side-effect.
    return false;

  // DWR POSSIBLE FIXME: originally ASUTTON had CompilerErrorExprClass
  // returning false; but if I understand this function's intent that isn't right;
  // plus now that I've expanded it to include warnings and debug messages
  // and void reflection expressions (e.g. $T->dump()), I think all these
  // should be considered as having side effects.
  // Not sure how much this matters or if I've interpreted this correctly.
  case CompilerMessageExprClass: //ASUTTON ADDN
  case CompilerDiagnosticExprClass: //DWR ADDN
  //case VoidReflectionExprClass: //DWR ADDN
  case ReflectDeleteExprClass: //DWR ADDN is this right?
    return true;
