...
///         'union'
///
/// [Meta]  nested-name-specifier[opt] metaclass-name (ASUTTON ADDN)
...
//        Meta.get(),
////END
        TUK, StartLoc, SS, Name, NameLoc, attrs, AS,
...
//      // construction completion. (ASUTTON ADDN)
...
////END
  ...
  // [ASUTTON ADDN: or metaclass-name]
...
/// [Meta]  constexpr-declaration (ASUTTON ADDN)
...
//                                       , bool IsInjected //ASUTTON ADDN
                                       ) {

//  //DWR TEMP:
//  if (VerboseMode || ParsingFromMetaSrcStr()) {
//    VerboseMode = true;
//
//    llvm::outs() << "-------In ParseCXXClassMemberDeclaration(...); Tok = " << Tok.getName()
...
//    }
//    else
//      llvm::outs() << "In ParseCXXClassMemberDeclaration(...), just noting that did NOT enconter a bad (location-invalid) token.\n";
//  }
//  //END
...
//ASUTTON ADDN:
  ...
  if (getLangOpts().CPlusPlus17 && Tok.is(tok::kw_constexpr) &&
      NextToken().is(tok::l_brace)) {
    auto cdcv = CDContextVars(attrs, AS); //DWR ADDN
...
//  //DWR TEMP
//  if (VerboseMode && NonNestedClass) {
//    llvm::outs() << "Detected NonNestedClass; dump: ";
//    TagDecl->dump();
//  }
//  //END
  ...
  if (TagDecl && NonNestedClass) {
    ...
    // delayed attributes.
    DoLateParsing();
      //^ DWR MOD: extracted stuff that was here into new DoLateParsing()
      // fcn (since it is also used in Sema::InstantiateClass when processing
      //  __queue_metaparse stmts)
