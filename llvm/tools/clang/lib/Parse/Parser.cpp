...
Parser::Parser(Preprocessor &pp, Sema &actions, bool skipFunctionBodies)
  : PP(pp), Actions(actions), Diags(PP.getDiagnostics()),
    ...
    ReflectionExpressionDepth(0), //ASUTTON ADDN:
  ...
  //DWR ADDN:
  EmptyToken.startToken();//DWR TEMP TEST
  EmptyToken.setKind(tok::eof); //DWR TEMP TEST
  assert(!ParsingFromMetaSrcStr());
#ifndef NMETAPARSEDEBUG
  ++DEBUGnumParsersConstructed;
#endif
  //END
...
//DWR ADDN:
//TODO delete the
bool Parser::ParsingFromMetaSrcStr() const {
  return PP.ParsingFromMetaSrcStr();
}
//END
...
//  llvm::outs() << "[DWR DEBUG] UNEXPECTED TOKEN! Expected " << tok::getTokenName(ExpectedTok) << ", got " << Tok.getName() << "\n"; //DWR TEMP
...
//  //DWR TEMP:
//  llvm::outs() << "Expecting semi; Tok.getName(): " << Tok.getName() << "\n";
//  if (Tok.is(tok::eof))
//    return false;
//  //END
...
//DWR ADDN (to avoid crashes when metaparsing an ill-formed expression):
  if (PP.ParsingFromMetaSrcStr() && isTokenSpecial()) {
    llvm::errs() << "DWR TEMP DEBUG; encountered special token while processing gen src str, calling setErrorWhileParsingFromMetaSrcStr(true)\n";
    PP.setErrorWhileParsingFromMetaSrcStr(true);

    // Throw a wrench in the works to trigger an error/diagnostic
    // (as opposed to crashing due to the failed assert):
    Token unknowntok;
    unknowntok.setKind(tok::unknown);
    UnconsumeToken(unknowntok);
...
/// [Meta]  metaclass-definition (ASUTTON ADDN)
///
/// [C++11] empty-declaration:
///           ';'
///
...
//ASUTTON ADDN:
  case tok::kw_constexpr: // [Meta] constexpr-declaration
    if (getLangOpts().CPlusPlus17 && NextToken().is(tok::l_brace)) {
      auto cdcv = CDContextVars(attrs, DS); //DWR ADDN
      return ParseConstexprDeclaration(cdcv, /*Nested=*/ParsingFromMetaSrcStr()); //DWR MOD
    }
    goto dont_know;
//END
////ASUTTON ADDN:
