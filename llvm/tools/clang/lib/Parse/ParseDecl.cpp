...
//ASUTTON ADDN:
  case tok::kw_constexpr: // [Meta]: constexpr-declaration
    ...
    if (getLangOpts().CPlusPlus17 && NextToken().is(tok::l_brace)) {
      auto cdcv = CDContextVars(); //DWR ADDN
...
void Parser::ParseEnumSpecifier(SourceLocation StartLoc, DeclSpec &DS,
  ...
  Decl *TagDecl = Actions.ActOnTag(
      getCurScope(), DeclSpec::TST_enum,
////ASUTTON ADDN:
//      nullptr, //=Metaclass
////END
      TUK, StartLoc, SS, Name, NameLoc,
...
////DWR ADDN:
//  } else if (Tok.getKind() == tok::kw___queue_metaparse) {
//    ParseCXXQueueMetaparseStmt();
////END
  } else {
  ...
  //DWR HACK to avoid assert fail on certain unterminated metaparsed expressions
  if (!D.isPastIdentifier() && PP.ErrorWhileParsingFromMetaSrcStr()) {
    D.SetIdentifier(nullptr, Tok.getLocation());
    D.setInvalidType(true);
    return;
  }
...
/// [Meta]  using reflection [ASUTTON ADDN]
///         using... reflection
///
...
////ASUTTON ADDN: (DWR Note: if you uncomment this you need to uncomment the closing bracef further down too.  Though, looking at code looks like maybe you could just remove the else and have a continue statement within the if.  Worry about that only if/when you want to add in the injection implem.
/// )
