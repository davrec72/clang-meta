...
#include "clang/Lex/MetaparseDebug.h" //DWR ADDN (used in DoLateParsing)
...
//DWR ADDN:
#ifndef NDEBUG
  auto DEBUGOldContext = Actions.CurContext;
#endif
//END
  ...
  assert(Actions.CurContext != DEBUGOldContext
         && "DWR TEMP: After ActOnStartOfFunctionDef should have pushed decl context!");
...
//DWR ADDN:
  assert(Actions.CurContext == DEBUGOldContext
         && "...DWR: ParseFunctionStatementBody should have restored old context, via PopDeclContext!"
            "Almost certainly this is a problem with getContainingDC (see the implems of PushDeclContext"
            "and PopDeclContext to see why).");
//END
...
//END
  ...
  //DWR ADDN:
  METAPARSE_DEBUG_IFUNDERHDR(
    METAPARSE_DEBUG_HDR("Parsing Lexed member initializers; Toks: ")
    tokvec2llvmouts(MI.Toks);
  )
  //END
  ...
  assert(Tok.getName() == OldTok.getName() && "Tok not restored!"); //DWR ADDN
}
...
//DWR ADDN:
//Extracted from ParseCXXMemberSpecification:
void Parser::DoLateParsing() {
#ifndef NDEBUG
  auto DEBUGOrigContext = Actions.CurContext;
#endif

  SourceLocation SavedPrevTokLocation = PrevTokLocation;
  Parser::ParsingClass &Class = getCurrentClass();
//  METAPARSE_DEBUG_IFUNDERHDR(METAPARSE_DEBUG("Parsing Lexed attributes"))
  ParseLexedAttributes(Class);
  assert(Actions.CurContext == DEBUGOrigContext && "CurContext changed during ParseLexedAttributes");
//  METAPARSE_DEBUG_IFUNDERHDR(METAPARSE_DEBUG("Parsing Lexed method declarations"))
  ParseLexedMethodDeclarations(Class);
  assert(Actions.CurContext == DEBUGOrigContext && "CurContext changed during ParseLexedMethodDeclarations");
  // We've finished with all pending member declarations.
  Actions.ActOnFinishCXXMemberDecls();

//  METAPARSE_DEBUG_IFUNDERHDR(METAPARSE_DEBUG("Parsing Lexed member initializers"))
  ParseLexedMemberInitializers(Class);
  assert(Actions.CurContext == DEBUGOrigContext && "CurContext changed during ParseLexedMemberInitializers");
//  METAPARSE_DEBUG_IFUNDERHDR(METAPARSE_DEBUG("Parsing Lexed method defs"))
  ParseLexedMethodDefs(Class);
  assert(Actions.CurContext == DEBUGOrigContext && "CurContext changed during ParseLexedMethodDefs");
