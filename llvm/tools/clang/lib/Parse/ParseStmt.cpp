...
/// [Meta]  injection-statement (ASUTTON ADDN)
///
///       labeled-statement:
...
//DWR ADDN:
  case tok::kw___queue_metaparse: // [Meta] metaparse-statement
    Res = ParseCXXQueueMetaparseStmt();
    SemiError = "__queue_metaparse";
    break;
//END
...
//      PP.debugDispLexerKinds("In ParseCompoundStatementBody, about to begin parse loop");
//    }
//  )
  ...
  while (!tryParseMisplacedModuleImport() && Tok.isNot(tok::r_brace) &&
    ...
    IF_METAPARSE_DEBUG( if (ParsingFromMetaSrcStr()) {
      METAPARSE_DEBUG("Initial tok: " << Tok.getName())
    } )
...
//DWR ADDN:
    if (ParsingFromMetaSrcStr()) {
      //While processing gen srcstrs, invalid toks are at the end of each source string; we have to skip over those.
      //DWR FIXME any way to avoid this, here and in the other parse loop in ApplyEffects?

      while (Tok.getLocation().isInvalid()) {
        METAPARSE_DEBUG("Invalid tok; consuming...")
        ConsumeToken();
        METAPARSE_DEBUG("Tok name after consuming: "  << Tok.getName())
        //DWR: See explanation for the analogous version of this while loop in ApplyEffects.
        //DWR FIXME: should combine the two into a single function call so you can make changes
        // to both without getting them out of sync.  Add in the below stuff as well.
      }

      // Make sure you didn't add too many l_braces, which would causing
      // the parser to eat past the ParserBrickWallRAII into the original source code.
      // (Either way you'd probably get an error, but this helps properly diagnose it).
      if (PP.NoMoreMetaparseStrsAvailable()) {
        //If the last statement was invalid, it doesn't necessarily mean you had too many
        // l_braces -- you probably just stopped lexing once you hit the error.
        // But if the last statement was VALID, and you're out of lexers without having hit
        // a final r_brace, then it must mean you had an extra l_brace in there somewhere:
        if (!R.isInvalid())
          Diag(diag::err_too_many_lbraces_expanded_in_cedecl); //DWR location?
        PP.setNoMoreMetaparseStrsAvailable(false);
        METAPARSE_DEBUG("Returning StmtError from ParseCompoundStmtBody...")
        return StmtError();
      }
    }
//END
//ASUTTON ADDN: (DWR note: it was commented out in original)
    // If the parsing of this statement resulted in injections, then add those
...
//END
  ...
  IF_METAPARSE_DEBUG(
    if (ParsingFromMetaSrcStr()) {
      PP.debugDispLexerKinds("Finished with parse loop");
    }
  )
  ...
  IF_METAPARSE_DEBUG(
    if (ParsingFromMetaSrcStr()) {
      PP.debugDispLexerKinds("After consumeClose");
    }
  )
