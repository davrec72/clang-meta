...
//#include "clang/Parse/Parser.h" //DWR TEMP (for accessing VerboseMode)
...
//ASUTTON ADDN: (DWR note: comment out above if using this)
...
//END
  ...
  FieldDecl *Field = SemaRef.CheckFieldDecl(D->getDeclName(),//ASUTTON REPLACEMENT: DNI.getName()
                                            DI->getType(), DI,
...
Decl *TemplateDeclInstantiator::VisitConstexprDecl(ConstexprDecl *D) {
  //DWR: Note an irony here: both function templates and class templates
  // will be processed here, but it is only the CLASS templates that should
  // have D->hasFunctionRepresentation() == true; ce-decls from function
  // templates will have it D->hasFunctionRepresentation() == false.
  ...
  // classes -- i.e. it's a total reversal of what you'd expect at first glance,
  // but it makes sense when you think further about it.
  // So just note that D->hasFunctionRepresentation() means
  // D was declared in a class template; and D->hasLambdaRepresentation() means
  // D was declared in a function template.
  // (This was ASUTTON's design, DWR just commenting on it)
  if (D->hasFunctionRepresentation()) { //DWR MOD
    METAPARSE_DEBUG_HDR("VisitConstexprDecl, Function Representation branch")
    assert(D->getcdcv().kind == CDContextVars::CDCV_cls
           && "Expected CE-decls from class contexts to have function representations");
...
//DWR ADDN: (without this, instantiating this template outside of a function will cause an error)
      // (Note this has nothing to do with the new __queue_metaparse statements, the bug existed before that).
      SemaRef.PushFunctionScope();
      Sema::FunctionScopeRAII FunctionScopeCleanup(SemaRef);
        //^ just calls PopFunctionScope or whatever at the end of this block
//END
//      SmallVector<std::pair<Decl *, Decl *>, 8> ExistingMappings; //ASUTTON ADDN
      NewBody = SemaRef.SubstStmt(Fn->getBody(), TemplateArgs
//                                  , ExistingMappings //ASUTTON ADDN
      );
    }
  ...
  //DWR FIXME, the below might be hacky, or at least is unclear, think it through again perhaps.  There's
    assert(D->getAccessUnsafe() != AS_none
           && "Need to set the access properly for a CE decl in a class context!");
    auto cdcv = CDContextVars(this->getAccessAttrs(), D->getAccess());
      //^ DWR: we construct a new CDContextVars using the TemplateDeclInstantiator's getAccessAttrs, ^
      // NOT D->getcdcv().cls.AS or whatever; the former is associated with the instantiation,
      // the latter with the original pattern, and is probably an invalid reference.  But honestly
      // I'm not 100% sure what the accessattrs are doing and really need to test out adding
      // attributes to template & non-template classes to see if this works.  DWR TODO/DWR FIXME.
...
//DWR ADDN:
      assert(CD->getAccessUnsafe() != AS_none
             && "Should have set access during ConstexprDecl::Create!");
//END
      //DWR: Note we should have already created a TempParseIntoClassInstantiation
      // in InstantiateClass, so no need to create one here
      SemaRef.EvaluateConstexprDecl(CD, NewFn);
    }
//DWR ADDN:
    else {
      //DWR TODO verify this branch is indeed sometimes reached.
      assert(Owner->isDependentContext() && "I assumed if NewFn was dependent, the Owner would be as well"
                                             "-- not sure its a big deal though, try commenting this out");
      CD->setDependentTrue();
      Decl::castFromDeclContext(Owner)->setInstantiationsWillNeedParsingTrue();
      METAPARSE_DEBUG("STILL a dependent context (non-lambda): will delay evaluation until (further) instantiation.")
    }
//END
    return CD;
  }
//DWR ADDN (replaces llvm_unreachable statements in ASUTTON original):
  else {
    assert(D->hasLambdaRepresentation()
           && "Is there some new Representation type in ConstexprDecl,"
              "besides Function and Lambda?");

    METAPARSE_DEBUG_HDR("VisitConstexprDecl, Lambda Representation branch")

    assert(SemaRef.CurContext == Owner);

    Expr *OldLambdaExpr = D->getLambdaExpr();
    StmtResult NewLambdaStmtRes = SemaRef.SubstStmt(OldLambdaExpr, TemplateArgs);
    ...
    CD->setLambdaExpr(NewLambdaExpr);

    Owner->addDecl(CD);

    // And evaluate it if needed.
    if (!NewLambdaExpr->isTypeDependent() && !NewLambdaExpr->isValueDependent()) {
      //^ DWR TODO verify this will occasionally be false
      //DWR: Note we should have already created a TempParseIntoFuncInstantiation
      // in InstantiateFunctionDefinition, so no need to create one here
      SemaRef.EvaluateConstexprDecl(CD, NewLambdaExpr);
    } else {
      assert(Owner->isDependentContext()
             && "I assumed if the NewLambdaExpr was type dependent, the Owner would also be dependent");
      CD->setDependentTrue();
      CD->setLambdaExpr(NewLambdaExpr);
      Decl::castFromDeclContext(Owner)->setInstantiationsWillNeedParsingTrue();
      METAPARSE_DEBUG("STILL a dependent context (LAMBDA): will delay evaluation until (further) instantiation.")
    }
...
//}
//
//Decl *TemplateDeclInstantiator::VisitCXXGeneratedTypeDecl(CXXGeneratedTypeDecl *D) {
//  llvm_unreachable("not implemented");
//}
////END ASUTTON ADDN
...
//  Record->setFragment(D->isFragment()); //ASUTTON ADDN
...
//                                    || D->isInFragment() //ASUTTON ADDN
    ...
    // (ASUTTON ADDN) Members of fragments are considered separately.
...
static bool addInstantiatedParametersToScope(Sema &S, FunctionDecl *Function,
  ...
  for (unsigned I = 0, N = PatternDecl->getNumParams(); I != N; ++I) {
    ...
    if (!PatternParam->isParameterPack()) {
      ...
      // (ASUTTON ADDN) Note that this also includes injected parameters.
      // These expanded during injection, and not instantiation.
...
//DWR ADDN:
  // Regardless of whether the PatternDecl->instantiationsWillNeedParsing(),
  // we will set the PII (the ParsingIntoInstantiation state)
  // and need to restore it before exiting; this RAII construction does that.
  // (If needsParsing..., we'll set it to PII_func; if not, PII_false.)
  SemaPIIRAII SavedPIIState(*this);

  // Construct an RAII object to save the parse state and enter the proper scope
  // IF this PatternDecl needs to access the parser (e.g. due to having constexpr decls
  // with __queue_metaparse statements).
  // Perhaps more efficient to stack allocate this, but that would require some surgery
  // on the stuff below, so not ideal.
  // This RAII object must be created before LocalInstantiationScope and anything that might
  // add to that scope (i.e. addInstantiatedParametersToScope), so that the proper Parser Scope
  // is created and LocalInstantiationScope can copy its contents to it as they are added (via
  // InstantiatedLocal(...) etc.)
  std::unique_ptr<TempParseIntoFuncInstantiation> TemporarilyParseIntoInstantiation;

  if (PatternDecl->instantiationsWillNeedParsing()) {
    TemporarilyParseIntoInstantiation = llvm::make_unique<TempParseIntoFuncInstantiation>(*this);
    assert(PII == PII_func); //just to be sure it set PII correctly
  } else {
    //If you don't need parsing, set the PII accordingly so that, in case you instantiate a
    // non-needsParsing... function WHILE instantiating a yes-needsParsing... function,
    // you make sure to turn off manipulating scopes and adding decls etc.
...
//END
  ...
  if (PatternDecl->isDefaulted())
    ...
    //DWR TODO perhaps add an assert that the params were indeed added to Scope and IdResolver, in addition
    // to LocalInstantiationScope, if PII == PII_func
    ...
    if (PatternDecl->hasSkippedBody()) {
      ...
      //DWR TODO add an assert about PII_func == Scope.copyToSemaScopeEtc
      // Instantiate the function body.
//      auto ExistingMappings = CalculateExistingMappingsFor(PatternDecl,Function); //ASUTTON ADDN
...
//                      , ExistingMappings //ASUTTON ADDN
      ...
      if (Body.isInvalid()) {
        ...
        if (TemporarilyParseIntoInstantiation) {
          METAPARSE_DEBUG("ERROR: A statement added to the function instantiation was invalid!")
    ...
    // (ASUTTON ADDN) FIXME: The pattern is non-dependent only during injection.
    //  It might be nice if we kept a flag (or extra info) determining if
    //  a declaration was injected.
    if (PatternDecl->isDependentContext()) //ASUTTON ADDN //DWR UNCOMMENTED; perhaps DWR FIXME, per ASUTTON's comment (OR can we comment this out again now? is it still relevant?)
...
//DWR ADDN:
  ...
  //DWR POSSIBLE FIXME I'm not 100% sure this is the right positioning,
  // but figured since its just after the LocalInstantiationScope
  // exiting this is a good bet.  If you get issues perhaps try
  // putting it above DeclGroupRef DG(Function), i.e. at the earliest
  // point after SubstStmt that's in the same scope as its construction.
...
//        || OldVar->isInFragment() //ASUTTON ADDN
        )
      CurrentInstantiationScope->InstantiatedLocal(OldVar, NewVar
                                                   , /*AllowReinstantiation=*/true //DWR TEMP HACK
                                                   );
    //DWR FIXME!!: in InstantiatedLocal, test whether the context is a constexpr decl or something, then get rid of the AllowReinstantiation params
  }
