...
#include "clang/Sema/Scope.h" //DWR ADDN
...
bool Sema::CodeSynthesisContext::isInstantiationRecord() const {
  ...
  case ForLoopInstantiation: //ASUTTON ADDN
    return true;

  case DefaultTemplateArgumentChecking:
  case DeclaringSpecialMember:
  case DefiningSynthesizedFunction:
//  case SourceCodeInjection: //ASUTTON ADDN
    return false;
...
Sema::InstantiatingTemplate::InstantiatingTemplate(
  ...
  if (!Invalid) {
    ...
    if (Inst.Entity) { //ASUTTON ADDN (I think -- in any case, needed for tuple-for instantiation
       ...
       ++Active)
  {
    ...
    case CodeSynthesisContext::TemplateInstantiation:
      ...
      if (isa<TypeAliasTemplateDecl>(Active->Entity))
        break;
      // Fall through.
    case CodeSynthesisContext::ForLoopInstantiation: //ASUTTON ADDN
    case CodeSynthesisContext::DefaultFunctionArgumentInstantiation:
    case CodeSynthesisContext::ExceptionSpecInstantiation:
//    case CodeSynthesisContext::SourceCodeInjection: //ASUTTON ADDN
...
Decl *TemplateInstantiator::TransformDefinition(SourceLocation Loc, Decl *D) {
  ...
  getSema().CurrentInstantiationScope->InstantiatedLocal(D, Inst
                                                         , /*AllowReinstantiation=*/true //DWR HACK
                                                         );
  //DWR NOTE re hack above: all the AllowReinstantiation param  does is turn off
  // an assert that checks whether you already instantiated D to something other
  // than Inst, and instead just sets the mapping for D to Inst regardless of whether
  // it had an old mapping.  This shouldn't change existing functionality when
  // reinstantiation is not needed, except that you'll no longer have that assert
  // checking you.  But it is necessary for for... tuple instantiation during Expr constant
  // evaluation, as we have implemented now.
  // What you should really do, if you want that assert back, is set up a bool param in
  // Decl * that instructs whether it can be reinstantiated, and pass that to
  // the InstantiatedLocal call; then for the LoopVariable in for... tuple expansion
  // set that to true.
...
//      && !CurrentInjectionContext //ASUTTON ADDN
      )
    return T;
...
//DWR ADDN/MOD: (Extracted this from inside the InstantiateClass function, and made a few small additions/changes):
//DWR TODO now that I removed FieldCollector, I bet this could be a local i.e. external function, not a method.
void
Sema::AddMembersToInstantiation(CXXRecordDecl *Instantiation,
                                CXXRecordDecl *Pattern,
                                TemplateSpecializationKind TSK,
                                SourceLocation PointOfInstantiation,
                                TemplateDeclInstantiator &Instantiator) {
  FieldCollector->StartClass(); //Add fields to common class member
  assert(FieldCollector->getCurNumFields() == 0);
  ...
  // DWR POSSIBLE FIXME: copied the below call for ActOnFields largely from Sema::ActOnFinishCXXMemberSpecification,
  // which is called when non-template classes are instantiated (right after parsing).
  // In that code is the below comment "strict aliasing violation!" -- I'm not sure
  // if that indicates a problem that needs to be solved, but if you encounter issues
  // perhaps fix this or both instances.
...
//DWR ADDN:
  //This property is used so that getContainingDC (used indirectly within the ParseLexed... function
  // calls below), so that if additional parsing is done to add decls to Instantiation,
  // the outermost/topmost/nonnested class is considered to be the Instantiation.
  // (For now, this is only used when instantiationsWillNeedParsing() = true, but I believe it could be more generally
  // useful/applicable so I have left it outside of the if (hasContexprDecls) block below.)
  Instantiation->setIsInstantiationTrue();

  // Regardless of whether the Pattern instantiationsWillNeedParsing,
  // we will set the PII (the ParsingIntoInstantiation state)
  // and need to restore it before exiting; this RAII consturction does that.
  // (If needsParsing..., we'll set it to PII_class; if not, PII_false.)
  SemaPIIRAII SavedPIIState(*this);

  if (Pattern->instantiationsWillNeedParsing()) {
    METAPARSE_DEBUG_HDR("Instantiating a template with content that needs parsing...")

    METAPARSE_DEBUG_assert(Parser::DEBUGnumParsersConstructed != 1
           || ParserBrickWallRAII::DEBUGrecursiondepth
           || PII == PII_false
              && "DWR: Given that you've only constructed one parser, "
                 "and the ParserBrickWallRAII::DEBUGrecursiondepth is zero, "
                 "the PII should be PII_false!");

    //Pass along the AccessAttrs; this is inteded to hold attributes collected while parsing possibly multiple
    //constexpr decls, though I'm not positive it's really needed -- once we start testing adding attributes
    //via expand statements we can test whether or not we really need the same object passed along.  I think
    //we might though -- might be needed to carry forwared dependencies.
    // DWR FIXME once this is tested clarify these comments and if necessary fix the code.
    ParsedAttributesWithRange AccessAttrs(TheParser->getAttrFactory());
    Instantiator.setAccessAttrs(AccessAttrs);

    //DWR NOTE: We're going to temporarily change the CurScope, Parser, and IdResolver here, in advance of
    //calling Instantiator->Visit(...) on all the decls (in AddMembersToInstantiation(...)).
    //We are ASSUMING that none of the Visit implems make use of any of these properties, save VisitConstexprDecl --
    //i.e., we're assume that instantiating the template is independent of the Scope, IdResolver, and Parser state at
    //the point of instantiation.
    //IF THIS ASSUMPTION IS WRONG, then you will probably get strange errors when instantiating non-constexpr/__queue_metaparse
    // template members.  The fix in that case will be to visit all the non-constexpr decl members first,
    // before changing the state, then only processing the constexpr decls with the additional state changes.
    //DWR FIXME test out this assumption, see if its valid, and make any necessary code changes, then remove/clarify
    // these comments.


    assert(CurContext == Instantiation
           && "Expected CurContext to be Instantiation.");
    ...
    TempParseIntoClassInstantiation TemporarilyParseIntoInstantiation(
                                      *this,
                                      Pattern->getTagKind() == TTK_Interface);
    assert(PII = PII_class);
    //DWR FIXME: remove the NonNestedClass bool from the RAII, so you don't have to supply it here.
    //Also remove the DeclContext param -- that should always be set to CurContext.
    METAPARSE_DEBUG_assert(TemporarilyParseIntoInstantiation
              .PPTotalLexers_Equals_TotalLexersAtRAIIConstruction()); //just to be sure

#ifndef NDEBUG
    auto ClassStackSize = TheParser->ClassStack.size();
#endif

    // Loop over the Pattern's member decls, calling Instantiator->Visit(...) on each; when constexpr decls
    // with __queue_metaparse stmts are encountered, the Parse... and Lookup calls will be done into,
    // and in the context/scope of, the Instantiation:
    AddMembersToInstantiation(Instantiation, Pattern, TSK, PointOfInstantiation, Instantiator);

    bool Valid = true;
    if (Instantiation->isInvalidDecl()) {
      METAPARSE_DEBUG("ERROR: A member added to the instantiation was invalid!.")
      TemporarilyParseIntoInstantiation.setInvalid();
      Valid = false;
    }
    else_METAPARSE_DEBUG("Members added to instantiation were all valid")

    // Be sure you lexed any new stuff you added, and no more:
    METAPARSE_DEBUG_assert(TemporarilyParseIntoInstantiation
              .PPTotalLexers_Equals_TotalLexersAtRAIIConstruction()
           || !Valid
           && "Expected PP.getTotalNumLexers() == PP.TotalLexersAtRAIIConstruction "
              "after finishing initial parsing of constexpr decls -- are there perhaps "
              "some dead lexers left over? Try calling SaveParseState.clearAnyDeadLexers() "
              "or whatever before this assert, or better yet just try commenting out "
              "this assert -- dead lexers shouldn't be an issue at this point, only "
              "after late-parsing.");

    assert(CurContext == Instantiation
           && "DWR: CurContext was changed somewhere BEFORE late parsing began!");
    assert(Instantiation->isInstantiation()); //just to be sure you set it properly before

    // Do late-parsing
    // for all newly introduce members/nested class members
    // (initializers, method definitions, etc).  (Adapted from ParseCXXMemberSpecification.)

//    if (WillDoLateParsingNow) {
    METAPARSE_DEBUG("Doing late-parsing of instantiated constexpr decls...")

    assert(TheParser->ClassStack.size() == ClassStackSize
           && "Class stack size was changed while adding members to instantiation!");


    TheParser->DoLateParsing();
    assert(CurContext == Instantiation
           && "DWR: CurContext was changed during late parsing!  "
              "Probably an issue with PopDeclContext()/getContainingDC(...)");

      //DWR TODO: Should something like ActOnFinishNonNestedClass go in here as it did for the other thing?


//    //DWR TEST/DWR TODO/DWR FIXME
//    llvm::outs() << "!!DWR NOTE: CLEARING MetaSrcStrVec!!\n";
//    for (auto sp : PP.MetaSrcStrVec)
//      delete sp;
//    PP.MetaSrcStrVec.clear();
//    PP.MetaSrcStrVec.push_back(new std::string("new str intended to screw things up"));
//    // DWR FIXME: I tried uncommenting this, and initially it seemed to work, but upon
//    // changing code suddenly weird lexer errors popped up, like failed
//    // assert(CurPPLexer) calls, so either this is placed poorly or I'm not
//    // understanding exactly when lexing is being done.  Probably some of the functions
//    // below do lexing at a late time.  So when everything is working
//    // and you want to test this further (to save memory),
//    // need to put it at the very end of this at least.
//    //END
...
//    }
    ...
    //DWR FIXME: are there additional function checks that should be done here?  Or are those done by the below stuff?
    // Or if not can we alter a few things so that all the function checks etc. are done together?

    //NB: RAII object destroyed at end of this block,
    // resetting Parser and CurScope etc. to original state
  } else { //a.k.a. if (!Pattern->instantiationsWillNeedParsing()):
//END DWR ADDN
    // --I.e., this is what gets called instead of the huge block above
    //    for normal C++ code without any constexpr decls.
    // --NOTE: the contents of this function (the for loop over members)
    //   used to be directly here in InstantiateClass; I extracted them into a
    //   separate function because it is used both here and in the above block.
    IF_METAPARSE_DEBUG(
      if (PII != PII_false) {
        METAPARSE_DEBUG_HDR("Note: PII was not PII_false; temporarily setting it to PII_false "
                      "while instantiating current class; RAII will restore it when done")
        METAPARSE_DEBUG_VAR(PII)
      }
    )
    PII = PII_false;
    AddMembersToInstantiation(Instantiation, Pattern, TSK, PointOfInstantiation, Instantiator);
...
#ifndef NDEBUG
  if (Pattern->instantiationsWillNeedParsing()) {
    METAPARSE_DEBUG("...Finished with all CE-decl-related instantiation tasks...")
  }
#endif

  SavedPIIState.Exit(); //return PII to original value.

//DWR NOTE: original implem continues unadulterated here...
...
#ifndef NDEBUG
  if (Pattern->instantiationsWillNeedParsing()) {
    METAPARSE_DEBUG("...Finished with instantiation.")
  }
#endif
...
//        llvm::outs() << "DWR TEMP: about to call InstantiateInClassInitializer from SemaTemplateInstantiate.cpp\n";
...
//using DeclMappingList = SmallVector<std::pair<Decl *, Decl *>, 8>; //ASUTTON ADDN

StmtResult
Sema::SubstStmt(Stmt *S, const MultiLevelTemplateArgumentList &TemplateArgs
//               , const DeclMappingList &ExistingMappings //ASUTTON ADDN
               ) {
  if (!S)
    return S;
...
LocalInstantiationScope::findInstantiationOf(const Decl *D) {
  //ASUTTON (?) MOD (refactored into lookupInstantiationOf above)
...
//DWR ADDN:
//DWR FIXME once you have this working, can probably drop the D param
inline void CopyToSemaScopeEtc(const Decl *D, Decl *Inst, Sema &SemaRef) {
#ifndef NDEBUG
  auto ND = dyn_cast<NamedDecl>(D);
#endif
  auto InstND = dyn_cast<NamedDecl>(Inst);
  assert(!ND || InstND && "Since the original decl was a NamedDecl, "
                          "I assumed the instantiated decl "
                          "would also be a NamedDecl");
  if (InstND) {
    assert(InstND->getDeclName() == ND->getDeclName()
           && "The instantiation's getDeclName is different from the original decl's! ");
    if (InstND->getDeclName()) {
//      METAPARSE_DEBUG_VAR(InstND->getDeclName().getAsString().c_str())
      SemaRef.PushOnScopeChains(InstND, SemaRef.getCurScope(), false/*don't add to CurContext*/);
    }
  }
}
//END
...
void LocalInstantiationScope::InstantiatedLocal(const Decl *D, Decl *Inst
                                                , bool AllowReinstantiation/*=false*/ //DWR ADDN
                                                ) {
  D = getCanonicalParmVarDecl(D);
...
////DWR TEMP DEBUG:
//    if (Stored.get<Decl *>() != Inst) {
//      llvm::outs() << "DWR TEMP DEBUG Stored dump, then Inst dump:\n";
//      Stored.get<Decl *>()->dump();
//      Inst->dump();
//    }
////END
//DWR ADDN (to allow reinstantiation of loop variable in for... expansion
// when performed during Expr evaluation):
    if (AllowReinstantiation)
      Stored = Inst;
    else
//END
...
//DWR ADDN (this has to do with meta-parsing;
// totally separate from for... instantiation stuff):
  if (ShouldCopyToSemaScopeEtc)
    CopyToSemaScopeEtc(D, Inst, SemaRef);
//END
}
