...
//  case DeclSpec::TST_metaclass: // FIXME: Is this correct? //ASUTTON ADDN
  {
...
QualType Sema::BuildDecltypeType(Expr *E, SourceLocation Loc,
  ...
  if (AsUnevaluated && CodeSynthesisContexts.empty() &&
      //DWR HACK so that we can build decltype'd reflection fields;
      // these all seem to have TypeDependent but !ValueDependent, and when
      // HasSideEffects is called on them we get an llvm_unreachable, can't
      // figure out why, so just prevent HasSideEffects from being called for now:
