...
#include "clang/Sema/Template.h" //ASUTTON ADDN
...
//ASUTTON ADDN:
  ...
  Decl *TransformLocalNamespaceDecl(NamespaceDecl *N);
...
//  Decl *TransformLocalCXXInjectionDecl(CXXInjectionDecl *D); //ASUTTON ADDN
...
//ASUTTON ADDN:
  ...
  /// By default, performs semantic analysis to build the new expression.
  /// Subclasses may override this routine to provide different behavior.
  ExprResult RebuildReflectionTraitExpr(SourceLocation KWLoc,
                                        ReflectionTraitKind TraitKind, //DWR ADDN
                                        ArrayRef<Expr *> Args,
                                        SourceLocation RParenLoc
//                                        , bool HasNonIntArgs
                                        ) {
      return getSema().ActOnReflectionTrait(KWLoc,
                                            TraitKind,
                                            Args,
                                            RParenLoc
//                                            , HasNonIntArgs
...
//                                            , HasNonIntArgs
  ...
  ExprResult RebuildReflectionTraitExpr(SourceLocation KWLoc,
                                        ...
                                        unsigned MemNum,
                                        bool IsPtr,
                                        function_impl<QualType(ArrayRef<intptr_t>)>
                                           CB_getTypeFromArgs,
                                        function_impl<ExprResult(ArrayRef<intptr_t>, QualType)>
                                           CB_getResultFromArgs,
                                        ArrayRef<Expr *> RemArgs,
...
//                                            , HasNonIntArgs
  ...
  /// By default, performs semantic analysis to build the new expression.
  /// Subclasses may override this routine to provide different behavior.
  ExprResult RebuildReflectNewExpr(SourceLocation TraitLoc,
                                   unsigned/*ReflectionObjKind*/ ObjKind,
                                   ArrayRef<Expr *> Args,
                                   SourceLocation RParenLoc
                                   ) {
    return getSema().ActOnReflectNew(TraitLoc,
                                     ObjKind,
                                     Args, RParenLoc);
  }
  /// \brief Build a new reflection trait expression.
  ...
  /// By default, performs semantic analysis to build the new expression.
  /// Subclasses may override this routine to provide different behavior.
  ExprResult RebuildReflectDeleteExpr(SourceLocation TraitLoc,
                                      unsigned/*ReflectionObjKind*/ ObjKind,
                                      ArrayRef<Expr *> Args,
                                      SourceLocation RParenLoc
                                      ) {
    return getSema().ActOnReflectDelete(TraitLoc,
                                        ObjKind,
                                        Args, RParenLoc);
  }
//END
//ASUTTON ADDN:
  /// \brief Build a new \c __compiler_debug expression.
  ///
  /// By default, performs semantic analysis to build the new expression.
  /// Subclasses may override this routine to provide different behavior.
  ExprResult RebuildCompilerMessageExpr(Expr *Message, SourceLocation KWLoc,
                                        SourceLocation RParenLoc) {
    return getSema().ActOnCompilerMessageExpr(KWLoc, Message, RParenLoc);
  }
//END
//DWR ADDN:
  /// \brief Build a new \c __compiler_diag expression.
  ///
  /// By default, performs semantic analysis to build the new expression.
  /// Subclasses may override this routine to provide different behavior.
  ExprResult RebuildCompilerDiagnosticExpr(SourceLocation KWLoc, ArrayRef<Expr *> Args,
                                           SourceLocation RParenLoc) {
    return getSema().ActOnCompilerDiagnosticExpr(KWLoc, Args, RParenLoc);
  }
//END
//ASUTTON ADDN:
...
////END
//DWR ADDN:
  StmtResult RebuildCXXQueueMetaparseStmt(SourceLocation Loc, Expr *NewSrcCode) {
    return getSema().BuildCXXQueueMetaparseStmt(Loc, NewSrcCode);
  }
  ExprResult RebuildCXXMetaparseExpr(SourceLocation Loc, QualType Type, Expr *ExprSrcCode, Parser &TheParser) {
    return getSema().BuildCXXMetaparseExpr(Loc, Type, ExprSrcCode, TheParser);
  }
//END
////ASUTTON ADDN:
...
//ASUTTON ADDN:
  case DeclarationName::CXXIdExprName: {
    ...
    SourceRange Range = NameInfo.getCXXIdExprNameRange();
    return getSema().BuildIdExprName(Range.getBegin(), NewArgs, Range.getEnd());
  }
//END
...
//DWR ADDN:
/// SimpleParseScopeRAII:
/// IF constructed with ShouldManipulateScope==true,
/// this RAII calls TheParser->EnterScope(...) and ->ExitScope(),
/// so that the current LocalInstantiationScope can add decls to the
/// correct current scope, and have them go out of scope properly.
/// This is needed to get Lookup to work properly when parsing
/// into a function template instantiation.  Note that the
/// CompoundScopeRAII below this, which was present in the original
/// code, has nothing to do with scope for the purposes of lookup,
/// hence the needs for this separate RAII.
class SimpleParseScopeRAII {
  Sema &SemaRef;
  bool ShouldManipulateScope;
public:
  SimpleParseScopeRAII(Sema &SemaRef)
    : SemaRef(SemaRef)
    , ShouldManipulateScope(SemaRef.getPII() == Sema::PII_func)
  {
    if (ShouldManipulateScope) {
      SemaRef.TheParserEnterScope(Scope::DeclScope | Scope::CompoundStmtScope);
      //^ DWR: I intend these scope flags to be general purpose Stmt scope flags,
      //  hopefully they will work for everything as intended.
    }
  }
  ~SimpleParseScopeRAII() {
    assert(ShouldManipulateScope == (SemaRef.getPII() == Sema::PII_func)
           && "PII changed during TransformStmt!");
    if (ShouldManipulateScope) {
      SemaRef.TheParserExitScope();
    }
  }
};
//END
...
//DWR ADDN:
  ...
  SimpleParseScopeRAII parseScopeRAII(SemaRef);
//END
...
//  assert(E->getType()->isDependentType() && "DWR: shouldn't every ReflectionTraitExpr be dependent typed?");
...
//  // If nothing changed, just retain the existing expr.
//  if (!getDerived().AlwaysRebuild() &&
//      !ArgChanged)
//    return SemaRef.MaybeBindToTemporary(E);

//  return getDerived().RebuildReflectionTraitExpr(E->getKWLoc(),
//                                                 E->getTraitKind(),
//                                                 NewArgs, //NB only change is updating args
//                                                 E->getRParenLoc());
//}
////END

//DWR ADDN:
//static inline bool HasNonIntArgs(ArrayRef<Expr *> Args) {
//  for (auto E : Args) {
...
TreeTransform<Derived>::TransformReflectionTraitExpr(ReflectionTraitExpr *E) {
  ...
  //DWR ADDN:
  // Convert each arg so we can evaluate it later we
  // get an Expr * rather than an unusable ValueDecl *:
  for (unsigned I = 0; I != NewArgs.size(); ++I) {
    ExprResult Converted = SemaRef.DefaultFunctionArrayLvalueConversion(NewArgs[I]);
    if (Converted.isInvalid())
      return ExprError();
    NewArgs[I] = Converted.get();
  }
  //END

  // Now, we just route to the proper ActOn... overload
  // (by way of Rebuild...) of the three available:

  if (!E->callbacksAreSet()) {
    // We haven't yet processed the first three args, so call the overload
    // that just takes the basic stuff:
    return getDerived().RebuildReflectionTraitExpr(E->getKWLoc(),
                                                   E->getTraitKind(),
                                                   NewArgs, //NB only change is updating args
                                                   E->getRParenLoc()
//                                                   , HasNonIntArgs(NewArgs)
                                                   );
  }

  if (E->getType()->isDependentType()) {
    // The original type of E was Dependent, implying it is a
    // class-returning reflection (and thus the type will be a client
    // refleciton template instance); call the overload that passes along
    // the CB_getTypeFromArgs so that the type may eventually be calculated
    // in ActOnReflectionTraitTyped(...):
      return getDerived().RebuildReflectionTraitExpr(E->getKWLoc(),
                                                     E->getTraitKind(),
                                                     E->getObjKind(),
                                                     E->getMemNum(),
...
//                                                     , HasNonIntArgs(NewArgs)
  ...
  //Primitive reflection: just pass along the type:
  return getDerived().RebuildReflectionTraitExpr(E->getKWLoc(),
                                                 E->getTraitKind(),
                                                 E->getObjKind(),
                                                 E->getMemNum(),
...
template <typename Derived>
ExprResult
TreeTransform<Derived>::TransformReflectNewExpr(ReflectNewExpr *E) {
  SmallVector<Expr *, 4> NewArgs;
  bool ArgChanged = false;
  if (getDerived().TransformExprs(E->getArgs(), E->getNumArgs(),
          /*IsCall*/true, NewArgs, &ArgChanged))
  ...
  return getDerived().RebuildReflectNewExpr(
      E->getTraitLoc(),
      E->getObjKind(),
      NewArgs, E->getRParenLoc()
      );
}
template <typename Derived>
ExprResult
TreeTransform<Derived>::TransformReflectDeleteExpr(ReflectDeleteExpr *E) {
  SmallVector<Expr *, 4> NewArgs;
  bool ArgChanged = false;
  if (getDerived().TransformExprs(E->getArgs(), E->getNumArgs(),
          /*IsCall*/true, NewArgs, &ArgChanged))
  ...
  return getDerived().RebuildReflectDeleteExpr(
      E->getTraitLoc(),
      E->getObjKind(),
      NewArgs, E->getRParenLoc()
      );
...
//template <typename Derived>
//ExprResult
//TreeTransform<Derived>::TransformVoidReflectionExpr(VoidReflectionExpr *E) {
//  // Nothing will ever change once we convert a ReflectionTraitExpr to this,
//  // so just retain the original:
//  return SemaRef.MaybeBindToTemporary(E);
//}
//template <typename Derived>
//ExprResult
//TreeTransform<Derived>::TransformNonvoidReflectionExpr(NonvoidReflectionExpr *E) {
//  // Nothing will ever change once we convert a ReflectionTraitExpr to this,
//  // so just retain the original:
//  return SemaRef.MaybeBindToTemporary(E);
//}
template <typename Derived>
ExprResult
TreeTransform<Derived>::TransformCompilerDiagnosticExpr(CompilerDiagnosticExpr *E) {
  bool ArgChanged = false;
  SmallVector<Expr *, 4> NewArgs;
  if (getDerived().TransformExprs(E->getArgs().begin(), E->arg_size(),
          /*IsCall*/true, NewArgs, &ArgChanged))
    return ExprError();

  // Always rebuild so that __compiler_diag diagnostics can be emitted within
  // template instantiations. (DWR: is this right? Maybe test without always rebuilding...)
  return getDerived().RebuildCompilerDiagnosticExpr(
      E->getKWLoc(), NewArgs, E->getRParenLoc());
}
//END
//ASUTTON ADDN:
template <typename Derived>
ExprResult
TreeTransform<Derived>::TransformCompilerMessageExpr(CompilerMessageExpr *E) {
  ExprResult Message = getDerived().TransformExpr(E->getMessage());
  if (Message.isInvalid())
    return ExprError();
  // Always rebuild so that __compiler_debug messages can be emitted within
  // template instantiations. (DWR: is this still right now that these are not error messages?)
  return getDerived().RebuildCompilerMessageExpr(
      Message.get(), E->getKWLoc(), E->getRParenLoc());
}
//END
//ASUTTON ADDN:
template<typename Derived>
...
////END
  ...
  //DWR: the main change here is I used TransformExprs, which does pack expansion
  // in addition to the individual TransformExpr calls.
  bool ArgChanged = false;
  if (getDerived().TransformExprs(E->getOperands(), E->getNumOperands(),
          /*IsCall*/true, NewArgs, &ArgChanged))
    return ExprError();
...
////DWR consider doing the following here instead of in ActOnCXXConcatenateExpr:
//  // Convert each arg so we can evaluate it later we
//  // get an Expr * rather than an unusable ValueDecl *:
//  for (unsigned I = 0; I != NewArgs.size(); ++I) {
//    ExprResult Converted = SemaRef.DefaultFunctionArrayLvalueConversion(NewArgs[I]);
...
//  }
  ...
  return RebuildCXXConcatenateExpr(E->getLocStart(), NewArgs);
...
template<typename Derived>
StmtResult
TreeTransform<Derived>::TransformCXXQueueMetaparseStmt(CXXQueueMetaparseStmt *S) {
  ExprResult Res = TransformExpr(S->getNewSrcCode());
  if (Res.isInvalid())
    return StmtError();

//  Res = SemaRef.DefaultFunctionArrayLvalueConversion(Res.get()); //DWR TODO try commenting this in instead of other lvalue->rvalue conversion code
//  assert(!Res.isInvalid());

  return RebuildCXXQueueMetaparseStmt(S->getLocStart(), Res.get());
}
template<typename Derived>
ExprResult
TreeTransform<Derived>::TransformCXXMetaparseExpr(CXXMetaparseExpr *E) {
  ExprResult Res = TransformExpr(E->getExprSrcCode());

  QualType NewType = getDerived().TransformType(E->getType());
...
//  assert(!Res.isInvalid());
  ...
  return RebuildCXXMetaparseExpr(E->getLocStart(), NewType, Res.get(), E->getParser());
}
//END
////ASUTTON ADDN:
//template<typename Derived>
...
TreeTransform<Derived>::TransformLambdaExpr(LambdaExpr *E) {
  METAPARSE_DEBUG_IFUNDERHDR(METAPARSE_DEBUG("DWR TEMP: In TransformLambdaExpr"))
...
QualType TreeTransform<Derived>::RebuildReflectedType(Expr *E,
                                                      SourceLocation Loc) {
  return BuildReflectedType(SemaRef, Loc, E);
}
//END
...
TreeTransform<Derived>::TransformLocalVarDecl(VarDecl *D) {
  ...
  Owner->addDecl(R);
  //DWR POSSIBLE FIXME: I think this is where you need to add to the Scope, IF processing gen src strs for function templates, to get lookup to work
...
TreeTransform<Derived>::TransformLocalCXXRecordDecl(CXXRecordDecl *D) {
  ...
  R->setImplicit(D->isImplicit());
//  R->setFragment(D->isFragment()); //ASUTTON ADDN
...
TreeTransform<Derived>::TransformLocalConstexprDecl(ConstexprDecl *D) {
  METAPARSE_DEBUG("In TransformLocalConstexprDecl...") //DWR ADDN
  ...
  Decl *New = getSema().ActOnConstexprDecl(getSema().getCurScope(),
                                           D->getLocation(), ScopeFlags, D->getcdcv());
...
template<typename Derived>
Decl *
TreeTransform<Derived>::TransformLocalNamespaceDecl(NamespaceDecl *D) {
  DeclContext *Owner = getSema().CurContext;

  // Build a new declaration.
  //
  // FIXME: We should probably try to re-link the namespace if a lookup
  // in the current context has one of the same name.
  NamespaceDecl *R = NamespaceDecl::Create(getSema().Context, Owner,
                                           D->isInline(), D->getLocation(),
                                           D->getLocation(), // Wrong (probably)
                                           D->getIdentifier(), nullptr);
  Owner->addDecl(R);

  // Transform each member in turn.
  Sema::ContextRAII Ctx(getSema(), R);
  for (Decl *OldDecl : D->decls()) {
    Decl *NewDecl = TransformLocalDecl(OldDecl);
    if (!NewDecl)
      R->setInvalidDecl(true);
  }
...
//template<typename Derived>
//Decl *
//TreeTransform<Derived>::TransformLocalCXXInjectionDecl(CXXInjectionDecl *D) {
//  ExprResult Reflection = getDerived().TransformExpr(D->getReflection());
//  if (Reflection.isInvalid())
//    return nullptr;
//
//  // Generate the injected declaration. If non-dependent, this can produce
//  // a group of declarations, each of which is added to the current context
