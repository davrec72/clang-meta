...
//ASUTTON ADDN:
//(DWR note: declared in SemaTemplate.h:)
...
//  NewClass->setGenerator(Generator); //ASUTTON ADDN
...
QualType Sema::CheckTemplateIdType(TemplateName Name,
  ...
  if (TypeAliasTemplateDecl *AliasTemplate =
    ...
    ClassTemplateSpecializationDecl *Decl =
        (CacheInstantiation==false) ? nullptr : //DWR ADDN
        ClassTemplate->findSpecialization(Converted, InsertPos);
    if (!Decl) {
      // This is the first time we have referenced this class template
      // specialization. Create the canonical declaration and add it to
      // the set of specializations.
      // (DWR: unless !CacheInstantiation, in which case you neither look
      // for a previous specialization nor add the new one.
      // We need this param to implement non-const reflections -- need to
      // reinstantiate them each time to get the updated state.)
      Decl = ClassTemplateSpecializationDecl::Create(Context,
                            ...
                            ClassTemplate->getTemplatedDecl()->getLocStart(),
                                                ClassTemplate->getLocation(),
                                                     ClassTemplate,
                                                     Converted, nullptr);
      if (CacheInstantiation) //DWR ADDN
