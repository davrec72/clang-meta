...
DeclContext *Sema::getContainingDC(DeclContext *DC) {
  ...
  // (ASUTTON ADDN) A metaprogram is also defined (implicitly) within its
  // lexical context; the next context is it's lexical context.
  if (isa<FunctionDecl>(DC) && !isLambdaCallOperator(DC)
        && !isMetaprogram(DC) //ASUTTON ADDN
     ) {
...
////END
    ...
    CXXRecordDecl *RD = dyn_cast<CXXRecordDecl>(DC); //DWR ADDN
    assert(isa<CXXRecordDecl>(DC) || !RD); //sanity check
    // A function not defined within a class will always return to its
    // lexical context.
    if (!RD) //DWR MOD: used to be if (!isa<CXXRecordDecl>(DC))
      return DC;

    assert(RD==DC); //sanity check

//DWR ADDN
// If RD is a class template instantiation, that means you are doing additional
// parsing during the instantiation (which, at least for now, means you are
// parsing source code generated by __queue_metaparse statements during template
// instantiation).
// In such a case, the "topmost" class you need to return to is the instantiation,
// whether or not it is not the outermost class, because it is the context in which
// the late parsing will be performed.
// So, we either return it now or exit the loop when we have found it (NB this test appears
// in the while loop below as well)
...
//END
    ...
    while (auto RD = dyn_cast<CXXRecordDecl>(DC->getLexicalParent())) {
      DC = RD;
//DWR ADDN (see above):
      if (RD->isInstantiation())
        break;
...
//DWR TEMP?
  ...
  if (getContainingDC(DC) != CurContext) {
    llvm::outs() << "Assert is about to fail; here's some helpful debug info:\nDC: " << DC << "\n";
    llvm::outs() << "getContainingDC(DC): " << getContainingDC(DC) << "\n";
    llvm::outs() << "getContainingDC(getContainingDC(DC)): " << getContainingDC(getContainingDC(DC)) << "\n";
    llvm::outs() << "CurContext: " << CurContext << "\n";
//    llvm::outs() << "DC->dumpDeclContext(): \n";
//    DC->dumpDeclContext();
        llvm::outs() << "getContainingDC(CurContext): " << getContainingDC(CurContext) << "\n";
    llvm::outs() << "DC->getParent(): " << DC->getParent() << "\n";
    llvm::outs() << "DC->getLexicalParent(): " << DC->getLexicalParent() << "\n";
    llvm::outs() << "DC->getParent()->getParent(): " << DC->getParent()->getParent() << "\n";
    llvm::outs() << "CurContext->getParent(): " << CurContext->getParent() << "\n";
    llvm::outs() << "CurContext->getParent()->getParent(): " << CurContext->getParent()->getParent() << "\n";
    llvm::outs() << "DC->getLexicalParent()->getDeclKindName(): " << DC->getLexicalParent()->getDeclKindName() << "\n";
    if (auto Record = dyn_cast<CXXRecordDecl>(CurContext)) {
      llvm::outs() << "CurContext IS a Record; isInstantiation() = " << Record->isInstantiation() << "\n";
    } else {
      llvm::outs() << "CurContext is NOT a Record!\n";
    }
  }
#endif
//END
...
//ASUTTON ADDN:
  case UnqualifiedIdKind::IK_IdExprOperator: {
    ...
    NameInfo.setName(Context.DeclarationNames.getCXXIdExprName(Args.size(),
                                              const_cast<Expr**>(&Args[0])));
    SourceRange Range(Name.StartLocation, Name.EndLocation);
    NameInfo.setCXXIdExprNameRange(Range);
    return NameInfo;
  }
//END
  ...
  if (D.isDecompositionDeclarator()) {
    ...
    if (!Decomp.bindings().empty()) {
      ...
      Name = II;
                ...
                && Name.getNameKind() != DeclarationName::CXXIdExprName //ASUTTON ADDN
            ) {
...
////      DiscardCleanupsInEvaluationContext(); //DWR HACK
//    }
//#endif
////END

    assert(!Cleanup.exprNeedsCleanups() && "Unaccounted cleanups in function"); //DWR TEMP COMMENTED OUT, DWR FIXME!!!
...
//DWR MOD
    ...
    if (!MaybeODRUseExprs.empty()) {
      ...
      llvm_unreachable("Leftover expressions for odr-use checking -- see dumps above");
    }
#endif
//END
...
///
/// \param Generator The generating expression, if any. (ASUTTON ADDN)
///
...
Decl *Sema::ActOnTag(Scope *S, unsigned TagSpec,
////ASUTTON ADDN:
//                     Expr *Generator,
////END
                     TagUseKind TUK,
...
//            Generator,
////END
            TUK, KWLoc, SS, Name, NameLoc, Attrs, TemplateParams,
            AS, ModulePrivateLoc,
...
////END ASUTTON ADDN
  ...
  // lexical context will be different from the semantic context.
  // (ASUTTON ADDN) This will also be the case for prototype classes.
  New->setLexicalDeclContext(CurContext);
