...
#include "clang/AST/DeclCXX.h" //DWR ADDN
...
Sema::DiagnoseEmptyLookup(Scope *S, CXXScopeSpec &SS, LookupResult &R,
  ...
  IF_METAPARSE_DEBUG
  (
     METAPARSE_DEBUG("In DiagnoseEmptyLookup; Lookup name: " << Name.getAsString().c_str())
     METAPARSE_DEBUG("IdResolver number found:" << (std::distance(IdResolver.begin(Name), IdResolver.end())))
  )
...
Sema::ActOnIdExpression(Scope *S, CXXScopeSpec &SS,
  ...
  //     -- an idexpr(...) with any dependent arguments (ASUTTON ADDN)
...
//  //DWR ADDN:
//  // Constexpr decls don't count as enclosing functions for this purpose;
//  // i.e. need this code so that the following works:
//
//  //  struct A { constexpr { __queue_metaparse "int myfcn() { int j = 2; return j+1; }"; } };
//
//  // If we don't have the below code, the compiler will complain about the use of local variable j.
//  //llvm::outs() << "Here; " << VarDC->getParent()->
//
//
//  //DWR FIXME: Set this up so you just check if the DC == VarDC, or if there is some permissible relationship between them (The DC's ancestors up until the VarDC can capture).  Test it out to make sure this works
//
//  DeclContext *DCtx = DC;
//  for (DeclContext *Ctx = VarDC; Ctx != nullptr; Ctx = Ctx->getParent(), DCtx = DCtx->getParent()) {
//    llvm::outs() << "~~~~~~~~~~~~ Ctx->getDeclKindName() = " << Ctx->getDeclKindName() << "\n";
//    llvm::outs() << "~~~~~~~~~~~~ DCtx->getDeclKindName() = " << DCtx->getDeclKindName() << "\n";
//    if (isa<ConstexprDecl>(Ctx)) {
//      assert(!Ctx->getParent() || !Ctx->getParent()->isFunctionOrMethod()
//              && "Did not expect a ConstexprDecl within a function -- "
//                 "have to adjust this code (I guess to break here, instead of returning, if constexpr parent"
//                 "is a function or method -- i.e. display the Diag below in that case) ");
//      return;
//    }
//  }
//  //END
...
//      llvm::outs() << "DWR TEMP DEBUG about to complain about uncaptured variable, here's name of DC (check it against what you thought the DC was above): " << cast<NamedDecl>(DC)->getQualifiedNameAsString() << "\n"; //DWR TEMP
...
//DWR ADDN
static bool isConstexprDeclFunc(DeclContext *DC) {
  return DC->isFunctionOrMethod() &&
      (cast<NamedDecl>(DC)->getNameAsString()
        == "__constexpr_decl");
}
//END
...
//DWR ADDN:
  assert(VarDC);
  bool origVarDCwasCEDecl = isConstexprDeclFunc(VarDC);
  // If VarDC is a __constexpr_decl, set it to its parent -- that's the
  // only way DC can possibly find VarDC, based on the way we've set this up.
  if (origVarDCwasCEDecl)
    VarDC = VarDC->getParent();
//END

////DWR TEMP DEBUG
//  if (VarDCisCEDecl) {
//    llvm::outs() << "DWR TEMP DEBUG: note VarDC is named __constexpr_decl; Var is named " << Var->getQualifiedNameAsString() << "\n";
//  }
////END
///
  do {
...
//DWR ADDN:
    assert(DC);
    if (isConstexprDeclFunc(DC)) {
      DC = DC->getParent();
      assert(DC);
//      llvm::outs() << "DC was a constexpr decl func, went to its parent context";
    }
    if (origVarDCwasCEDecl && VarDC == DC)
...
////DWR ADDN:
//    // We process variables introduced in constexpr decls, and perhaps referenced
//    // in nested  (i.e. metaparsed) constexpr decls, as a special case,
//    // since they may need be enclosed in lambdas etc. properly.
//      if (VarDCisCEDecl) {
//        assert(DC);
//        ParentDC = DC->getParent();
//        if (ParentDC) {
//          if (VarDC == ParentDC) {
//            // We've found a common context.  Don't try to
//            break;
//          }
//          else if (isConstexprDeclFunc(ParentDC)) {
//            DC = ParentDC;
//            assert(!isConstexprDeclFunc(DC->getParent()));
...
//          llvm::outs() << "DC is the translation unit (i.e. DC->getParent() is null!) Never found it!\n";
//        }
//        else if (VarDC == DC->getParent()) {
//          llvm::outs() << "---GOOD, BREAKING OUT OF LOOP\n"; //DWR TEMP DEBUG
//          break;
//        }
//        //DWR TEMP DEBUG:
//        else {
//          auto DCParND = dyn_cast<NamedDecl>(DC->getParent());
//           llvm::outs() << "DC->getParent() (named "
//                        << (DCParND ? DCParND->getQualifiedNameAsString() : "[UNNAMED]")
//                        << ") not yet VarDC (named __constexpr_decl), continuing loop...";
//        }
//        //END
//      }
////END
...
//DWR NOTE: moved ActOnCompilerMessageExpr etc. to SemaReflect.cpp
