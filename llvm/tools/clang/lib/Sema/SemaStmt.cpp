...
#include "clang/AST/DeclTemplate.h" //ASUTTON ADDN
...
//ASUTTON ADDN, DWR MOD:
  // If we have a non-empty constexpr decl, repace it with its expanded (DWR; ASUTTON: "injected") statements.
  if (DG.isSingleDecl()) {
    if (ConstexprDecl *CD = dyn_cast<ConstexprDecl>(DG.getSingleDecl())) {
      if (!CD->isDependent()) { //DWR ADDN
        //      return ReplaceWithInjectedStmts(*this, CD); //ASUTTON original
//DWR ADDN:
        METAPARSE_DEBUG_HDR("In ActOnDeclStmt for non-dep. CD;")
        auto sr = CD->getStmtResult();
        if (sr.isUsable()) {
          METAPARSE_DEBUG("CD->getStmtResult() is usable, returning it")
          return sr;
        } else {
          METAPARSE_DEBUG("CD->getStmtResult() is NOT usable, returning null stmt")
          return ActOnNullStmt(StartLoc);
        }
      }
      else_METAPARSE_DEBUG("In ActOnDeclStmt for dep. CD, will return usual value")
//END DWR ADDN
    }
  }
//END
...
static bool GetTupleSize(Sema &SemaRef, SourceLocation Loc, QualType RangeType,
                         llvm::APSInt &Size) {
//  llvm::outs() << "[DWR DEBUG] looking up tuple_size...\n";
  ...
  QualType SpecType = SemaRef.CheckTemplateIdType(TempName, Loc, TempArgs
                                                  , /*CacheInstantiation=*/false //DWR ADDN
                                                  );
    //^ DWR NOTE: by setting CacheInstantiation = false, we aim to force this function to
    // reinstantiate the tuple_size<RangeType> specialization each time, so that
    // we get an updated value if anything has changed (it only will with
    // non-const reflections like reflection containers).
    // (Note too that in our implem, GetTupleSize will only be meaningfully called within
    // the CB_getSize callback, which itself will be called during expression evaluation
    // in ExprConstant.cpp.
...
//  //  //DWR TEST HACK to avoid !MaybeODRUseExprs.empty() error:
//  SemaRef.MaybeODRUseExprs.erase(Ref.get());

//  llvm::outs() << "[DWR DEBUG] Evaluated Size successfully; val = " << Size.toString(10) << "\n";
  return true;
}
//END ASUTTON ADDN
...
//ASUTTON ADDN:
        ...
        if (!GetTupleSize(*this, RangeLoc, RangeType, TupleSize)) { //DWR changed from if (GetTupleSize to if (!GetTupleSize
...
//    llvm::outs() << "DWR TEMP DEBUG builidng pack expansion stmt\n";
...
//  llvm::outs() << "DWR TEMP DEBUG about to BuildCXXTupleExpansionStmt from ActOnCXXExpansionStmt...\n";
...
//  llvm::APSInt Size = llvm::APSInt::get(0); //ASUTTON ORIG
  std::function<std::size_t()> CB_getSize; //DWR REPLACEMENT
...
////END
//// DWR REPLACEMENT:
//    CB_getSize = [this, ColonLoc, RangeClassType]() -> std::size_t {
//      llvm::APSInt Size;
//      if (!GetTupleSize(*this, ColonLoc, RangeClassType, Size))
//        return INVALID_STD_SIZE_T;
//      return Size.getZExtValue();
//    };
////END
////DWR TEMP HACK to solve MaybeODRUseExprs issue, while still using callback:
   llvm::APSInt Size;
   if (!GetTupleSize(*this, ColonLoc, RangeClassType, Size))
     return StmtError();
   std::size_t thesizeres = Size.getZExtValue();
   CB_getSize = [=]() -> std::size_t {
     return thesizeres;
   };
////END HACK

   //DWR FIXME: using get<N> to get the Nth elem will make this O(n^2) instead of
   // O(n) for any range that uses a list structure instead of a vector/array structure,
   // which is... basically all the important ones.  decls() for example.  So this
   // MUST change -- we MUST keep our place in the loop with an iterator somehow.
    ...
    //DWR FIXME: to get this to work for std::tuples as well as your reflected tuples, how about you
    // look for _get_ as you do below, THEN IF YOU DON'T FIND IT, you lookup get in the NNS as
    // ASUTTON did it...GOOD, DWR TODO

    // Build the dependent expression [DWR MOD:] _get_<__N>()
    // ( ASUTTON ORIG: 'NNS::get<__N>(__tuple)' where 'NNS' is
    ...
    // Get the name information for NNS::_get_.
    CXXRecordDecl *RangeClass = RangeClassType->getAsCXXRecordDecl();
    NestedNameSpecifierLoc NNS =
        GetQualifiedNameForDecl(Context, RangeClass, ColonLoc);
    IdentifierInfo *Name = &PP.getIdentifierTable().get("_get_"); //DWR MOD: changed to _get_ to avoid any possible name conflicts with existing methods, for range classes

    DeclarationNameInfo DNI(Name, ColonLoc);


    // Do an initial lookup for 'NNS::_get_' where NNS is the name of the range type.
    //  (ASUTTON ORIG: lookup was for 'NNS::get' where 'NNS' is the declaration
    //   context of the range type.)
    LookupResult R(*this, DNI.getName(), ColonLoc, Sema::LookupOrdinaryName);
    if (!LookupQualifiedName(R, RangeClass
//                             ->getDeclContext() //DWR REMOVED: search in the current class, not its DeclContext
                             )) {
      CXXRecordDecl *D = RangeClassType->getAsCXXRecordDecl();
      Diag(ColonLoc, diag::err_no_member) << Name << D
//                                             ->getParent() //DWR REMOVED
                                             ;
      return StmtError();
    }
    const UnresolvedSetImpl &FoundNames = R.asUnresolvedSet();

    // Build the lookup expression 'NNS::_get_' (ASUTTON ORIG: 'NNS::get<I>').
...
////DWR TEST COMMENTED OUT (THOUGH SEE ABOVE, you will eventually want to put this back in so this works with std::tuples too)
...
////END
    ...
    ExprResult Call =
        ActOnCallExpr(getCurScope(), Fn, ColonLoc,
//                      Args,
                      MutableArrayRef<Expr*>(), //DWR REPLACEMENT: pass no arg since our new _get_ is nullary.
                      ColonLoc);
...
StmtResult Sema::FinishCXXTupleExpansionStmt(CXXTupleExpansionStmt *S,
  ...
  // DWR FIXME: pre-instantiating is problematic.  It means we can't do nested
  // for... loops in which the inner loop must instantiate its own reflections:
  // the types are always that of the first outer instantiation, which doesn't work
  // for reflection.  How can we avoid that?
  S->setBody(B);

//  return S; //DWR REPLACEMENT FOR BELOW (stopped here back when

//DWR MOVED TO ExprConstant.cpp, Visit implem
...
//DWR COMMENTED OUT: ALWAYS create the expansion statement
// (since we are using a callback to get the size, i.e. it may change)
// DWR FIXME: perhaps you should distinguish between const and non-const
// here?
  // When there are no members, return an empty compound statement.
  if (S->getSize() == 0) {
    return CompoundStmt::Create(Context, None, SourceLocation(), SourceLocation()); //DWR FIXED
  }
  ...
  Stmt *Body = CompoundStmt::Create(Context, VarAndBody, SourceLocation(), SourceLocation()); //DWR FIXED
