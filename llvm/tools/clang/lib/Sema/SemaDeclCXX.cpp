...
//    case Decl::CXXInjection: //ASUTTON ADDN
//ASUTTON ADDN:
    case Decl::Constexpr:
      continue;
//END
...
//ASUTTON ADDN:
  case Stmt::CXXQueueMetaparseStmtClass:
    // __queue_metaparse is a compile-time statement.
    return true;
//END
...
//  if (VerboseMode) llvm::outs() << "DWR TEMP in ActOnFinishCXXInClassMemberInitializer, InitExpr = " << InitExpr << "\n"; //DWR TEMP
...
//  if (VerboseMode) llvm::outs() << "DWR TEMP in ActOnFinishCXXInClassMemberInitializer, about to setInClassInitializer for " << FD->getQualifiedNameAsString() << " (ptr: " << FD << ") to : " << InitExpr << "\n"; //DWR TEMP
...
//    if (S.VerboseMode) llvm::outs() << "DWR TEMP: in computeImplicitExceptionSpec, about to call ComputeDefaultedSpecialMemberExceptionSpec with NULLPTR as last param\n"; //DWR TEMP
...
//    if (S.VerboseMode) llvm::outs() << "DWR TEMP: in computeImplicitExceptionSpec, about to call ComputeDefaultedSpecialMemberExceptionSpec with NON-NULL last param\n"; //DWR TEMP
...
//  if (S.VerboseMode) llvm::outs() << "DWR TEMP: in ComputeDefaultedSpecialMemberExceptionSpec; constructed SpecialMemberExceptionSpecInfo Info, about to call Info.visit (which will visit all bases, then all fields):\n "; //DWR TEMP
...
ExprResult Sema::BuildCXXDefaultInitExpr(SourceLocation Loc, FieldDecl *Field) {
  ...
  if (isTemplateInstantiation(ParentRD->getTemplateSpecializationKind())) {
    ...
    //DWR TEMP
    if (Lookup.empty()) {
      llvm::outs() << "Lookup results empty while in BuildCXXDefaultInitExpr for Field: " << Field->getQualifiedNameAsString() << " (ptr: " << Field << "); getInClassInitializer() for this field: " << Field->getInClassInitializer() << "\n";
    }
    //END

    assert(!Lookup.empty() && "no lookup results for field name"); //DWR separated out this assert from below one
    assert((getLangOpts().Modules || (Lookup.size() <= 2)) &&
...
//                                /*Generator=*/nullptr, //ASUTTON ADDN
...
//                      /*Metaclass=*/nullptr, //ASUTTON ADDN
                      TUK_Friend, TagLoc, SS, Name, NameLoc,
                      Attr, AS_public,
                      /*ModulePrivateLoc=*/SourceLocation(),
...
NamedDecl *Sema::ActOnFriendFunctionDecl(Scope *S, Declarator &D,
  ...
  if (!DC->isRecord()) {
    ...
    case UnqualifiedIdKind::IK_OperatorFunctionId:
    case UnqualifiedIdKind::IK_TemplateId:
    case UnqualifiedIdKind::IK_IdExprOperator: //ASUTTON ADDN
      break;
    }
