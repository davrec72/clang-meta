...
# define AK_TOOLKIT_ASSERT(CHECK) ((CHECK) ? void(0) : []{assert(!#CHECK);}())
    ...
    // # A wrapper over a string literal with alternate interface. No ownership management
    ...
    // # SIGNATURE: literal(const char (&lit)[N_PLUS_1])
    //
...
//DWR ADDN:
    // # SIGNATURE: literal<N>(const char *lit)
    // #                    ^ you must specify the size manually
    // # EXAMPLE:
    //     struct NonTmplStrLitWrpr {
    //         const char *_data;
    //         std::size_t _size;
    //
    //         template<std::size_t N>
    //         constexpr NonTmplStrLitWrpr(const char (&data)[N])
    //           : _data(data), _size(N)
    //         {}
    //     };
    //     constexpr NonTmplStrLitWrpr s("mystr"); //MUST be constexpr
    //     //auto resA = literal(s.data); //ERROR: no matching constructor
    //     auto resB = literal<s._size>(s._data);
    //     auto resC = resB + "suffix";
    //
    // # EXAMPLE (uses cestrlen function defined below):
    //    constexpr const char * cccp = "mystr B"; //MUST be constexpr
    //    auto resD = literal<cestrlen(cccp)>(cccp);
    //
    template<::std::size_t N_PLUS_1>
    constexpr string_literal<N_PLUS_1 - 1> literal(const char *lit) {
        return string_literal<N_PLUS_1 - 1>((const char (&)[N_PLUS_1])*lit);
    }

    // # A constexpr verison of strlen (though returns N+1).
    // # Can be used to calculate N value for above literal<N>(const char *data)
    // # whenever the size isn't stored alongside the data.
    constexpr std::size_t cestrlen(const char* lit) {
      return *lit ? 1 + cestrlen(lit + 1) : 1;
    }

    // # Just in case you call this when you already have the size info:
    template<::std::size_t N_PLUS_1>
    constexpr std::size_t cestrlen(const char (&lit)[N_PLUS_1]) {
      return N_PLUS_1;
    }
//END DWR ADDN
