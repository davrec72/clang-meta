...
Note that you only need to build the clang-wreflection when you change clang/tools/reflection-src-generator/GenReflectionSrc.cpp OR one of the AST headers (i.e. the reflection interface).  When just making ordinary adjustments to clang cpps, just recompile the clang target -- much faster. 
