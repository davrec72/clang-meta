...
Our implementations will then differ only in how you extract reflection properties from that point -- via e.g. `meta::some_standard_property(reflexpr(x))` queries in their case, via `__creflexpr(x)->someClangSpecificProperty()` in ours.

It also makes great sense to incorporate their excellent `unqualid(...)` feature, as that will work excellently with our `const char *`-centric implementation.  Like our `__metaparse_expr(..., T)`, `unqualid(...)` is an excellent tool for transferring information *between* "meta-levels" in-place, so you needn't always resort to `DEFERRED_META` techniques (discussed in our metaparsing example hpp).  See their tutorial at https://gitlab.com/lock3/clang/wikis/Metaprogramming-Introductory-Tutorial for how `unqualid` is used (in our implementation, the arguments will always be string literals or integers, as with `__concatenate`).
