...
Our implementations will then differ only in how you extract reflection properties from that point -- via e.g. `meta::some_property(refl)` queries in their case, via `refl->someProperty()` in ours.
