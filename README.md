...
Our implementations will then differ only in how you extract reflection properties from that point -- via e.g. `meta::some_standard_property(refl)` queries in their case, via `refl->someClangSpecificProperty()` in ours.
